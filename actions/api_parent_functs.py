...
def api_action_copy_paste(dialog, geom_type, tab_type=None):
    ...
    global_vars.canvas.xyCoordinates.connect(api_action_copy_paste_mouse_move)
    parent_vars.emit_point.canvasClicked.connect(partial(api_action_copy_paste_canvas_clicked, dialog, tab_type))
...
def api_action_copy_paste_mouse_move(point):
...
def api_action_copy_paste_canvas_clicked(dialog, tab_type, point, btn):
    """ Slot function when canvas is clicked """

    if btn == Qt.RightButton:
        api_disable_copy_paste(dialog)
        return
...
def api_disable_copy_paste(dialog):
