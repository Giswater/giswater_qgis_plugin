...
from PyQt4.QtGui import QCheckBox, QRadioButton, QAction, QWidget
...
class Info(ParentAction):
    ...
    def info_show_info(self):
        ...
        self.folderSoftware = self.root_dir + '/' + self.project_type + '/'
        # TODO: Anadir locale (variable ES, EN, CAT...)
        locale = QSettings().value('locale/userLocale').lower()
        print(locale)
        self.folderLocale = self.root_dir + '\i18n/' + 'EN/'
        self.folderUtils = self.root_dir + '\utils/'
        self.folderUpdates = self.root_dir + '\updates/'
        self.folderExemple = self.root_dir, '\example/'
        self.folderPath = ''

        # Set Listeners
        self.dlg_info_show_info.btn_schema_create.clicked.connect(partial(self.open_create_project, True))
        self.dlg_info_show_info.btn_no_ct_schema_create.clicked.connect(partial(self.open_create_project, False))
    ...
    def load_update_ddl_dml(self):
        ...
        if self.process_folder(self.folderUpdates, '') is False:
            return False
        if self.process_folder(self.folderUpdates, '') is False:
            return False
        if self.process_folder(self.folderUpdates, '') is False:
    ...
    def load_trg(self):
        ...
        if self.process_folder(self.folderUtils, self.file_pattern_trg) is False:
            ...
            self.controller.log_info(str("FK STATUS---------------"))
    ...
    def create_project_data_schema(self, chk):
        if chk.isChecked():
    ...
    def create_project_data_schema_no_ct(self, chk):
        if chk.isChecked():
    ...
    def open_create_project(self, ct):
        ...
        if ct:
            print(str("ct"))
            self.dlg_readsql_create_project.btn_accept.clicked.connect(partial(self.create_project_data_schema, chk_import_data))
        else:
            print(str("NO ct"))
            self.dlg_readsql_create_project.btn_accept.clicked.connect(partial(self.create_project_data_schema_no_ct, chk_import_data))
    ...
    def executeFiles(self, filelist, filedir):
        for file in filelist:
            try:
                ...
                # self.controller.log_info(str(f.read()))
