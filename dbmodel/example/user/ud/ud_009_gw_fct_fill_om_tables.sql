...
BEGIN
	...
	LOOP
		--visit class 9. insp links
		INSERT INTO ve_visit_link_insp (link_id, visitcat_id, startdate, enddate, user_name, expl_id, class_id, status, sediments_link, defect_link, clean_link, insp_observ, photo)
		VALUES(rec_link.link_id, 1, now(), now(), 'postgres', rec_link.expl_id, 9, 4, 2, 1, 1, 'No other problems', False);

		--visit class 10. incident links
		INSERT INTO ve_visit_incid_link (link_id, visitcat_id, startdate, enddate, user_name, expl_id, class_id, status, incident_type, incident_comment, photo)
		VALUES(rec_link.link_id, 1, now(), now(), 'postgres', rec_link.expl_id, 10, 4, 6, 'No other problems', False);
	END LOOP;
