...
BEGIN
    ...
    IF type_element_arg = 'arc' OR type_element_arg='ARC' THEN
        ...
        IF controlValue = 1 THEN
			...
			EXECUTE 'SELECT gw_fct_getmessage($${"client":{"device":4, "infoType":1, "lang":"ES"},"feature":{},
	       		"data":{"message":"3006", "function":"2304","parameters":null, "is_process":true}}$$);';
            END IF;
		...
		ELSE 
				EXECUTE 'SELECT gw_fct_getmessage($${"client":{"device":4, "infoType":1, "lang":"ES"},"feature":{},
       			"data":{"message":"1082", "function":"2304","parameters":{"arc_id":"'||element_id_arg||'"}, "is_process":true}}$$);';
		END IF;
    ...
    ELSE
		...
		IF (SELECT state FROM node WHERE (node_id = element_id_arg))=0 THEN
            EXECUTE 'SELECT gw_fct_getmessage($${"client":{"device":4, "infoType":1, "lang":"ES"},"feature":{},
       		"data":{"message":"3004", "function":"2304","parameters":{"element_id":"'||element_id_arg||'"}, "is_process":true}}$$);';
		END IF;
        ...
        ELSE 
            EXECUTE 'SELECT gw_fct_getmessage($${"client":{"device":4, "infoType":1, "lang":"ES"},"feature":{},
       		"data":{"message":"1084", "function":"2304","parameters":{"node_id":"'||element_id_arg||'"}, "is_process":true}}$$);';
        END IF;
