...
--  get input values
	...
	v_vehicle = (((p_data ->>'data')::json->>'fields')::json->>'vehicle_id')::integer;

    -- Check if exist some other workday opened, and close
	EXECUTE 'SELECT team_id, lot_id, endtime, user_id, vehicle_id FROM (SELECT * FROM om_visit_lot_x_user WHERE user_id=''' || v_user ||''' ORDER BY id DESC) a LIMIT 1' INTO v_record;

	IF v_record.user_id IS NULL OR v_record.endtime IS NOT NULL THEN

		-- Insert start work day
		INSERT INTO om_visit_lot_x_user (team_id, lot_id, the_geom, vehicle_id) VALUES (v_team, v_lot, v_thegeom, v_vehicle);
	...
	ELSE
		...
		--EXECUTE 'SELECT team_id, lot_id FROM (SELECT * FROM om_visit_lot_x_user WHERE user_id=''' || v_user ||''' ORDER BY id DESC) a LIMIT 1' INTO v_record;
