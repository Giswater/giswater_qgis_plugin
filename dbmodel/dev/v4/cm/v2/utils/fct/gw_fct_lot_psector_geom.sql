...
BEGIN 
	...
	SELECT epsg, project_type INTO epsg_val, v_projecttype FROM sys_version LIMIT 1;
		...
		ELSIF v_projecttype = 'UD' THEN 
            ...
            (WITH polygon AS
                (SELECT st_collect(f.the_geom) g
                FROM
                    (SELECT the_geom FROM arc JOIN om_visit_lot_x_arc ON om_visit_lot_x_arc.arc_id=arc.arc_id
                    WHERE lot_id=p_lot_id
                    UNION
                    SELECT the_geom FROM node JOIN om_visit_lot_x_node ON om_visit_lot_x_node.node_id=node.node_id
                    WHERE lot_id=p_lot_id AND source = 'ORPHAN'
                    UNION
                    SELECT the_geom FROM connec JOIN om_visit_lot_x_connec ON om_visit_lot_x_connec.connec_id=connec.connec_id
                    WHERE lot_id=p_lot_id
                    UNION
                    SELECT the_geom FROM gully JOIN om_visit_lot_x_gully ON om_visit_lot_x_gully.gully_id=gully.gully_id
                    WHERE lot_id=p_lot_id AND source = 'ORPHAN'
                    )f
                )
                SELECT CASE
                WHEN st_geometrytype(st_concavehull(g, 0.85)) = 'ST_Polygon'::text THEN st_buffer(st_concavehull(g, 0.85), 3)::geometry(Polygon,25831)
                ELSE st_expand(st_buffer(g, 3::double precision), 1::double precision)::geometry(Polygon,25831)
                END AS the_geom
                FROM polygon
            )a;
