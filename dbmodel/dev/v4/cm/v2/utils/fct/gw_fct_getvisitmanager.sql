...
BEGIN
	...
	v_vehicle = ((p_data ->>'data')::json->>'fields')::json->>'vehicle_id'::text;
	...
	v_formtabs := '[';
		...
		IF v_isusermanager THEN
			...
			IF v_activedatatab THEN
				...
				IF v_record.endtime IS NULL AND v_record.user_id IS NOT NULL THEN
					v_disable_widget_name = '{data_startbutton, data_team_id, data_lot_id, data_vehicle_id}';
				ELSE
				...
				v_feature := '{"tableName":"visit_manager"}';
				...
				LOOP
					...
					IF (aux_json->>'widgetname') = v_disable_widget_name[1] OR (aux_json->>'widgetname') = v_disable_widget_name[2] OR (aux_json->>'widgetname') = v_disable_widget_name[3] OR (aux_json->>'widgetname') = v_disable_widget_name[4] THEN
						v_fields[array_index] := gw_fct_json_object_set_key(v_fields[array_index], 'disabled', True);
					END IF;

					-- get last used team if exists
					...
					-- manage if last team not exists or permisions are not anymore allowed
					IF v_team IS NULL OR v_team NOT IN (SELECT team_id::text FROM om_team_x_user WHERE user_id = current_user) THEN
					...
					IF (aux_json->>'widgettype')='combo' THEN
						...
						ELSIF (aux_json->>'columnname')='lot_id' AND v_team IS NOT NULL THEN
							...
							select array_to_json(array_agg((value))) FROM (select * from json_array_elements_text(((((v_child_result->>'body')::json->>'data')::json->>1)::json->>'comboIds')::json)) a INTO v_childs_result;
							v_fields[array_index] := gw_fct_json_object_set_key(v_fields[array_index], 'comboIds', v_childs_result);
							select array_to_json(array_agg((value))) FROM (select * from json_array_elements_text(((((v_child_result->>'body')::json->>'data')::json->>1)::json->>'comboNames')::json)) a INTO v_childs_result;
							...
							END IF;
						ELSIF (aux_json->>'columnname')='vehicle_id' AND v_team IS NOT NULL THEN
							...
							IF v_lot IS NULL THEN
								v_fields[array_index] := gw_fct_json_object_set_key(v_fields[array_index], 'selectedId', COALESCE(v_fieldvalue, ''));
							ELSE
								v_fields[array_index] := gw_fct_json_object_set_key(v_fields[array_index], 'selectedId', COALESCE(v_vehicle::text, ''));
			...
			SELECT gw_fct_getlist (p_data) INTO v_fields_json_aux;

			v_fields_json_aux = (((v_fields_json_aux->>'body')::json->>'data')::json->>'fields');

			-- Join results from config_form_fields and config_form_list and sort by orderby
			v_fields_json = v_fields_json::jsonb || v_fields_json_aux::jsonb;

			IF v_fields_json IS NOT NULL THEN
				EXECUTE 'SELECT (''['' || STRING_AGG(a::text, '','' ORDER BY (a->>''orderby'')) || '']'') FROM json_array_elements($$'||v_fields_json||'$$) a' into v_fields_json;
			END IF;
		...
		-- header form
		SELECT name INTO v_realusername FROM cat_users WHERE id=current_user;

		IF v_isusermanager THEN
			IF v_realusername IS NOT NULL THEN
				v_formheader :=concat('VISIT MANAGER - ',UPPER(v_realusername));
			ELSE
