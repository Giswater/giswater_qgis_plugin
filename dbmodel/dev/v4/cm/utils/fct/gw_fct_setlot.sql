...
BEGIN
	...
	IF v_result IS NOT NULL THEN
		...
		-- Check if all UM have been visited before close Lot
		IF (SELECT feature_type FROM config_visit_class WHERE id=v_visitclass_id::integer) = 'UNIT' THEN
			SELECT count(*) INTO v_count FROM om_visit_lot_x_unit WHERE lot_id=v_id::integer AND status <>4;
			IF v_count > 0 THEN
				EXECUTE 'SELECT gw_fct_getmessage($${"client":{"device":4, "infoType":1, "lang":"ES"},"feature":{},
				"data":{"message":"3195", "function":"2862","debug_msg":""}}$$);'INTO v_message;
				v_data = p_data->>'data';
			...
			END IF;
		END IF;
		IF ((((v_message->>'body')::json->>'data')::json->>'info')::json->>'level')::integer != 2 THEN
