...
CREATE OR REPLACE FUNCTION sample_ud.gw_trg_edit_man_arc() RETURNS trigger LANGUAGE plpgsql AS $$
DECLARE 
    arc_table varchar;
    man_table varchar;
    v_sql varchar;
...
BEGIN
    ...
    IF TG_OP = 'INSERT' THEN
        ...
        -- MANAGEMENT INSERT
        IF arc_table = 'man_conduit' THEN   
            INSERT INTO man_conduit VALUES (NEW.arc_id, NEW.add_info);
        END IF;              
    ...
    ELSIF TG_OP = 'UPDATE' THEN
        ...
        IF man_table = 'man_conduit' THEN   
            UPDATE man_conduit SET arc_id=NEW.arc_id, add_info=NEW.add_info WHERE arc_id=OLD.arc_id;
        END IF;         
        RETURN NEW;
...
CREATE TRIGGER gw_trg_edit_man_arc_conduit INSTEAD OF INSERT OR DELETE OR UPDATE ON "sample_ud".v_edit_man_conduit
FOR EACH ROW EXECUTE PROCEDURE "sample_ud".gw_trg_edit_man_arc('man_conduit');
