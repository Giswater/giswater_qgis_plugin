...
v_version text;
v_result json;CREATE OR REPLACE FUNCTION SCHEMA_NAME.gw_fct_create_dwf_scenario_empty(p_data json)
 RETURNS json
 LANGUAGE plpgsql
AS $function$
...
BEGIN
	...
	PERFORM setval('SCHEMA_NAME.cat_dwf_scenario_id_seq'::regclass,(SELECT max(dscenario_id) FROM cat_dscenario) ,true);
