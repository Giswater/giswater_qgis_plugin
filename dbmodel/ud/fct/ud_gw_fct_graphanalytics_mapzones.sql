	...
	IF v_floodonlymapzone IS NULL THEN
		...
		EXECUTE '
			...
			FROM (
				...
				FROM (
					SELECT DISTINCT ON (vc.connec_id) vc.connec_id, c.conneccat_id, c.state, c.expl_id, NULL AS mapzone_id, c.the_geom, ''Disconnected''::text AS descript
					FROM temp_pgr_arc ta
					JOIN v_temp_connec vc USING (arc_id)
					JOIN connec c USING (connec_id)
					WHERE ta.mapzone_id = 0
					UNION
					SELECT DISTINCT ON (vc.connec_id) vc.connec_id, c.conneccat_id, c.state, c.expl_id, NULL AS mapzone_id, c.the_geom, ''Conflict''::text AS descript
					FROM temp_pgr_arc ta
					JOIN v_temp_connec vc USING (arc_id)
					JOIN connec c USING (connec_id)
	...
	ELSIF v_commitchanges IS TRUE THEN
		...
		ELSE
			...
			v_querytext = '
				...
				AND link.'||quote_ident(v_mapzone_field)||' <> links.mapzone_id
			';
			RAISE NOTICE 'v_querytext:: UPDATE link with temp_pgr_arc join v_temp_link_gully mapzone_id %', v_querytext;
			EXECUTE v_querytext;
			...
			';
			RAISE NOTICE 'v_querytext:: UPDATE gully with temp_pgr_arc join v_temp_gully mapzone_id %', v_querytext;
			EXECUTE v_querytext;
