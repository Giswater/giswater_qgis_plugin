...
BEGIN
	...
	IF (SELECT count(*) FROM selector_inp_dscenario WHERE cur_user = current_user) > 0 THEN
		...
		UPDATE temp_rpt_inp_raingage t SET form_type = d.form_type FROM inp_dscenario_raingage d
		...
		UPDATE temp_rpt_inp_raingage t SET intvl = d.intvl FROM inp_dscenario_raingage d
		...
		UPDATE temp_rpt_inp_raingage t SET scf = d.scf FROM inp_dscenario_raingage d
		WHERE t.rg_id = d.rg_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.scf IS NOT NULL AND result_id = result_id_var;
		UPDATE temp_rpt_inp_raingage t SET rgage_type = d.rgage_type FROM inp_dscenario_raingage d
		...
		UPDATE temp_rpt_inp_raingage t SET timser_id = d.timser_id FROM inp_dscenario_raingage d
		...
		UPDATE temp_rpt_inp_raingage t SET fname = d.fname FROM inp_dscenario_raingage d
		...
		UPDATE temp_rpt_inp_raingage t SET sta = d.sta FROM inp_dscenario_raingage d
		WHERE t.rg_id = d.rg_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.sta IS NOT NULL AND result_id = result_id_var;
		UPDATE temp_rpt_inp_raingage t SET units = d.units FROM inp_dscenario_raingage d
		...
		UPDATE temp_t_arc_flowregulator t SET close_time = d.close_time FROM inp_dscenario_flwreg_orifice d 
		...
		FROM v_edit_inp_dscenario_lids
