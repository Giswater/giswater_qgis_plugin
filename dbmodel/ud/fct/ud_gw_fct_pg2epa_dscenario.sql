...
BEGIN
	...
	IF (SELECT count(*) FROM selector_inp_dscenario WHERE cur_user = current_user) > 0 THEN
		...
		UPDATE temp_rpt_inp_raingage t SET form_type = d.form_type FROM v_edit_inp_dscenario_raingage d
		...
		UPDATE temp_rpt_inp_raingage t SET intvl = d.intvl FROM v_edit_inp_dscenario_raingage d
		...
		UPDATE temp_rpt_inp_raingage t SET scf = d.scf FROM v_edit_inp_dscenario_raingage d
		WHERE t.rg_id = d.rg_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.scf IS NOT NULL AND result_id = result_id_var;
		UPDATE temp_rpt_inp_raingage t SET rgage_type = d.rgage_type FROM v_edit_inp_dscenario_raingage d
		...
		UPDATE temp_rpt_inp_raingage t SET timser_id = d.timser_id FROM v_edit_inp_dscenario_raingage d
		...
		UPDATE temp_rpt_inp_raingage t SET fname = d.fname FROM v_edit_inp_dscenario_raingage d
		...
		UPDATE temp_rpt_inp_raingage t SET sta = d.sta FROM v_edit_inp_dscenario_raingage d
		WHERE t.rg_id = d.rg_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.sta IS NOT NULL AND result_id = result_id_var;
		UPDATE temp_rpt_inp_raingage t SET units = d.units FROM v_edit_inp_dscenario_raingage d
		...
		UPDATE temp_t_arc t SET n = d.n FROM (SELECT * FROM v_edit_inp_dscenario_conduit a 
		JOIN cat_arc c ON c.id = a.arccat_id  
		JOIN cat_mat_arc b ON c.matcat_id = b.id) d
		WHERE t.arc_id = d.arc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.arccat_id IS NOT NULL;

		-- when material is informed by user
		UPDATE temp_t_arc t SET n = d.n FROM (SELECT * FROM v_edit_inp_dscenario_conduit a JOIN cat_mat_arc b ON a.matcat_id = id WHERE a.matcat_id IS NOT NULL)d
		...
		-- update outfalls
		UPDATE temp_t_node t SET addparam = gw_fct_json_object_set_key(addparam::json, 'outfall_type', d.outfall_type) FROM v_edit_inp_dscenario_outfall d 
		WHERE t.node_id = d.node_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.outfall_type IS NOT NULL;		
		UPDATE temp_t_node t SET addparam = gw_fct_json_object_set_key(addparam::json, 'stage', d.stage) FROM v_edit_inp_dscenario_outfall d 
		WHERE t.node_id = d.node_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.stage IS NOT NULL;		
		UPDATE temp_t_node t SET addparam = gw_fct_json_object_set_key(addparam::json, 'curve_id', d.curve_id) FROM v_edit_inp_dscenario_outfall d 
		WHERE t.node_id = d.node_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.curve_id IS NOT NULL;		
		UPDATE temp_t_node t SET addparam = gw_fct_json_object_set_key(addparam::json, 'timser_id', d.timser_id) FROM v_edit_inp_dscenario_outfall d 
		WHERE t.node_id = d.node_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.timser_id IS NOT NULL;		
		UPDATE temp_t_node t SET addparam = gw_fct_json_object_set_key(addparam::json, 'gate', d.gate) FROM v_edit_inp_dscenario_outfall d 
		WHERE t.node_id = d.node_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.gate IS NOT NULL;		
		
		-- update storage
		UPDATE temp_t_node t SET addparam = gw_fct_json_object_set_key(addparam::json, 'storage_type', d.storage_type) FROM v_edit_inp_dscenario_storage d 
		WHERE t.node_id = d.node_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.storage_type IS NOT NULL;		
		UPDATE temp_t_node t SET addparam = gw_fct_json_object_set_key(addparam::json, 'curve_id', d.curve_id) FROM v_edit_inp_dscenario_storage d 
		WHERE t.node_id = d.node_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.curve_id IS NOT NULL;		
		UPDATE temp_t_node t SET addparam = gw_fct_json_object_set_key(addparam::json, 'a1', d.a1) FROM v_edit_inp_dscenario_storage d 
		WHERE t.node_id = d.node_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.a1 IS NOT NULL;		
		UPDATE temp_t_node t SET addparam = gw_fct_json_object_set_key(addparam::json, 'a2', d.a2) FROM v_edit_inp_dscenario_storage d 
		WHERE t.node_id = d.node_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.a2 IS NOT NULL;		
		UPDATE temp_t_node t SET addparam = gw_fct_json_object_set_key(addparam::json, 'a0', d.a0) FROM v_edit_inp_dscenario_storage d 
		WHERE t.node_id = d.node_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.a0 IS NOT NULL;		
		UPDATE temp_t_node t SET addparam = gw_fct_json_object_set_key(addparam::json, 'fevap', d.fevap) FROM v_edit_inp_dscenario_storage d 
		WHERE t.node_id = d.node_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.fevap IS NOT NULL;		
		UPDATE temp_t_node t SET addparam = gw_fct_json_object_set_key(addparam::json, 'sh', d.sh) FROM v_edit_inp_dscenario_storage d 
		WHERE t.node_id = d.node_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.sh IS NOT NULL;		
		UPDATE temp_t_node t SET addparam = gw_fct_json_object_set_key(addparam::json, 'hc', d.hc) FROM v_edit_inp_dscenario_storage d 
		WHERE t.node_id = d.node_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.hc IS NOT NULL;		
		UPDATE temp_t_node t SET addparam = gw_fct_json_object_set_key(addparam::json, 'imd', d.imd) FROM v_edit_inp_dscenario_storage d 
		WHERE t.node_id = d.node_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.imd IS NOT NULL;		
		UPDATE temp_t_node t SET addparam = gw_fct_json_object_set_key(addparam::json, 'y0', d.y0) FROM v_edit_inp_dscenario_storage d 
		WHERE t.node_id = d.node_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.y0 IS NOT NULL;		
		UPDATE temp_t_node t SET addparam = gw_fct_json_object_set_key(addparam::json, 'ysur', d.ysur) FROM v_edit_inp_dscenario_storage d 
		WHERE t.node_id = d.node_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.ysur IS NOT NULL;		
		UPDATE temp_t_node t SET addparam = gw_fct_json_object_set_key(addparam::json, 'apond', d.apond) FROM v_edit_inp_dscenario_storage d 
		WHERE t.node_id = d.node_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.apond IS NOT NULL;		

		-- update flwreg orifice
		UPDATE temp_t_arc_flowregulator t SET ori_type = d.ori_type FROM v_edit_inp_dscenario_flwreg_orifice d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.ori_type IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET offsetval = d.offsetval FROM v_edit_inp_dscenario_flwreg_orifice d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.offsetval IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET cd = d.cd FROM v_edit_inp_dscenario_flwreg_orifice d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.cd IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET orate = d.orate FROM v_edit_inp_dscenario_flwreg_orifice d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.orate IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET flap = d.flap FROM v_edit_inp_dscenario_flwreg_orifice d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.flap IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET shape = d.shape FROM v_edit_inp_dscenario_flwreg_orifice d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.shape IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET geom1 = d.geom1 FROM v_edit_inp_dscenario_flwreg_orifice d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.geom1 IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET geom2 = d.geom2 FROM v_edit_inp_dscenario_flwreg_orifice d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.geom2 IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET geom3 = d.geom3 FROM v_edit_inp_dscenario_flwreg_orifice d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.geom3 IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET geom4 = d.geom4 FROM v_edit_inp_dscenario_flwreg_orifice d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.geom4 IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET close_time = d.close_time FROM v_edit_inp_dscenario_flwreg_orifice d 
		...
		UPDATE temp_t_arc_flowregulator t SET outlet_type = d.outlet_type FROM v_edit_inp_dscenario_flwreg_outlet d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.outlet_type IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET offsetval = d.offsetval FROM v_edit_inp_dscenario_flwreg_outlet d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.offsetval IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET curve_id = d.curve_id FROM v_edit_inp_dscenario_flwreg_outlet d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.curve_id IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET cd1 = d.cd1 FROM v_edit_inp_dscenario_flwreg_outlet d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.cd1 IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET cd2 = d.cd2 FROM v_edit_inp_dscenario_flwreg_outlet d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.cd2 IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET flap = d.flap FROM v_edit_inp_dscenario_flwreg_outlet d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.flap IS NOT NULL;
		
		-- update flwreg pump
		UPDATE temp_t_arc_flowregulator t SET curve_id = d.curve_id FROM v_edit_inp_dscenario_flwreg_pump d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.curve_id IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET status = d.status FROM v_edit_inp_dscenario_flwreg_pump d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.status IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET startup = d.startup FROM v_edit_inp_dscenario_flwreg_pump d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.startup IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET shutoff = d.shutoff FROM v_edit_inp_dscenario_flwreg_pump d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.shutoff IS NOT NULL;

		-- update flwreg weir
		UPDATE temp_t_arc_flowregulator t SET weir_type = d.weir_type FROM v_edit_inp_dscenario_flwreg_weir d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.weir_type IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET offsetval = d.offsetval FROM v_edit_inp_dscenario_flwreg_weir d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.offsetval IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET cd = d.cd FROM v_edit_inp_dscenario_flwreg_weir d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.cd IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET ec = d.ec FROM v_edit_inp_dscenario_flwreg_weir d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.ec IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET cd2 = d.cd2 FROM v_edit_inp_dscenario_flwreg_weir d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.cd2 IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET flap = d.flap FROM v_edit_inp_dscenario_flwreg_weir d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.flap IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET geom1 = d.geom1 FROM v_edit_inp_dscenario_flwreg_weir d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.geom1 IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET geom2 = d.geom2 FROM v_edit_inp_dscenario_flwreg_weir d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.geom2 IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET geom3 = d.geom3 FROM v_edit_inp_dscenario_flwreg_weir d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.geom3 IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET geom4 = d.geom4 FROM v_edit_inp_dscenario_flwreg_weir d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.geom4 IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET surcharge = d.surcharge FROM v_edit_inp_dscenario_flwreg_weir d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.surcharge IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET road_width = d.road_width FROM v_edit_inp_dscenario_flwreg_weir d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.road_width IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET road_surf = d.road_surf FROM v_edit_inp_dscenario_flwreg_weir d 
		WHERE t.arc_id = d.nodarc_id AND dscenario_id IN (SELECT unnest(v_userscenario)) AND d.road_surf IS NOT NULL;
		UPDATE temp_t_arc_flowregulator t SET coef_curve = d.coef_curve FROM v_edit_inp_dscenario_flwreg_weir d 
