...
BEGIN
    ...
    SELECT node_id INTO v_exists_id FROM anl_flow_node WHERE node_id = v_node_id AND cur_user="current_user"() AND context='Flow exit';
    ...
    IF NOT FOUND THEN
        ...
        (v_node_id, (SELECT expl_id FROM v_edit_node WHERE node_id = v_node_id), 'Flow exit', (SELECT the_geom FROM v_edit_node WHERE node_id = v_node_id));
        ...
        LOOP
            ...
            INSERT INTO anl_node (node_id, nodecat_id, expl_id, fprocesscat_id, the_geom) 
            SELECT connec_id, connec_type, expl_id, 121, the_geom FROM v_edit_connec WHERE arc_id=rec_table.arc_id;

            INSERT INTO anl_node (node_id, nodecat_id, expl_id, fprocesscat_id, the_geom) 
            SELECT gully_id, gully_type, expl_id, 121, the_geom FROM v_edit_gully WHERE arc_id=rec_table.arc_id;
