...
BEGIN
	...
	IF TG_OP = 'INSERT' THEN
		...
		ELSIF v_dscenario_type = 'LID-USAGE' THEN
			...
			WHERE dscenario_id = NEW.dscenario_id AND hydrology_id=NEW.hydrology_id AND subc_id=NEW.subc_id AND lidco_id=NEW.lidco_id );END IF;
	...
	ELSIF TG_OP = 'UPDATE' THEN
		...
		ELSIF v_dscenario_type = 'JUNCTION' THEN
		 ...
		 ELSIF v_dscenario_type = 'LID-USAGE' THEN
			UPDATE inp_dscenario_lid_usage SET dscenario_id=NEW.dscenario_id, hydrology_id=NEW.hydrology_id, subc_id=NEW.subc_id, lidco_id=NEW.lidco_id, 
			...
			WHERE dscenario_id = OLD.dscenario_id AND hydrology_id=OLD.hydrology_id AND subc_id=OLD.subc_id AND lidco_id=OLD.lidco_id;
