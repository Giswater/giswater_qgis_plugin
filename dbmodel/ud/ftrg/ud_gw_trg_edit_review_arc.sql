...
DECLARE
	...
	v_rev_arc_geom1_tol double precision;
	v_rev_arc_geom2_tol double precision;
...
BEGIN
	...
	v_rev_arc_geom1_tol :=(SELECT "value" FROM config_param_system WHERE "parameter"='rev_arc_geom1_tol');
	v_rev_arc_geom2_tol :=(SELECT "value" FROM config_param_system WHERE "parameter"='rev_arc_geom2_tol');	
		...
		IF (NEW.expl_id IS NULL) THEN
			...
			IF (NEW.expl_id IS NULL) THEN
				...
				IF (NEW.expl_id IS NULL) THEN
					...
					"data":{"error":"2012", "function":"2470","debug_msg":"'||NEW.arc_id::text||'"}}$$);'; 
				END IF;		
			END IF;
		END IF;
    ...
    ELSIF TG_OP = 'UPDATE' THEN
		...
		IF 	abs(rec_arc.y1-NEW.y1)>v_rev_arc_y1_tol OR  (rec_arc.y1 IS NULL AND NEW.y1 IS NOT NULL) OR
			...
			abs(rec_arc.geom1-NEW.geom1)>v_rev_arc_geom1_tol OR  (rec_arc.geom1 IS NULL AND NEW.geom1 IS NOT NULL) OR
			abs(rec_arc.geom2-NEW.geom2)>v_rev_arc_geom2_tol OR  (rec_arc.geom2 IS NULL AND NEW.geom2 IS NOT NULL) OR
