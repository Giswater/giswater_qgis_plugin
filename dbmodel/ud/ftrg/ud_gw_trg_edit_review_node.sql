...
--get value from edit_review_auto_field_checked
    ...
    IF TG_OP = 'INSERT' THEN
		...
		IF (NEW.expl_id IS NULL) THEN
			...
			IF (NEW.expl_id IS NULL) THEN
				...
				IF (NEW.expl_id IS NULL) THEN
					...
					"data":{"message":"2012", "function":"2466","parameters":{"feature_id":"'||NEW.node_id::text||'"}}}$$);';
				END IF;
			END IF;
		END IF;
		...
		review_obs, expl_id, the_geom, field_checked, field_date)
		VALUES (NEW.node_id, NEW.top_elev, NEW.ymax, NEW.node_type, NEW.matcat_id, NEW.nodecat_id, NEW.annotation, NEW.observ,
		NEW.review_obs, NEW.expl_id, NEW.the_geom, NEW.field_checked, NEW.field_date);
