...
BEGIN
	...
	IF TG_OP = 'INSERT' THEN
		...
		IF v_table = 'cat_dwf' THEN
			INSERT INTO cat_dwf(id, idval, startdate, enddate, observ, active, expl_id, log)
			VALUES (NEW.id, NEW.idval, NEW.startdate, NEW.enddate, NEW.observ, NEW.active, NEW.expl_id, NEW.log) RETURNING id INTO v_id;

			UPDATE config_param_user SET value=v_id WHERE parameter='inp_options_dwfscenario_current' AND cur_user=current_user;
		ELSE
	...
	ELSIF TG_OP = 'UPDATE' THEN
   		IF v_table = 'cat_dwf' THEN
   			UPDATE cat_dwf SET id=NEW.id, idval=NEW.idval, startdate=NEW.startdate, enddate=NEW.enddate, observ=NEW.observ, active=NEW.active, expl_id=NEW.expl_id, log=NEW.log
			WHERE id=OLD.id;
   		ELSE
			UPDATE inp_dwf
	...
	ELSIF TG_OP = 'DELETE' THEN
		IF v_table = 'cat_dwf' THEN
			DELETE FROM cat_dwf WHERE id=OLD.id;
   		ELSE
			DELETE FROM inp_dwf
			WHERE node_id=OLD.node_id AND dwfscenario_id = OLD.dwfscenario_id;
		END IF;
