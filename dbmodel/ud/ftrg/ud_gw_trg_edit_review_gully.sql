...
--get value from edit_review_auto_field_checked
	...
	IF (NEW.field_checked is TRUE) THEN
		...
		IF abs(rec_gully.top_elev-NEW.top_elev)>v_rev_gully_top_elev_tol OR  (rec_gully.top_elev IS NULL AND NEW.top_elev IS NOT NULL) OR
			...
			rec_gully.gullycat_id != NEW.gullycat_id	OR  (rec_gully.gullycat_id IS NULL AND NEW.gullycat_id IS NOT NULL) OR
	...
	IF TG_OP = 'INSERT' THEN
		...
		IF (NEW.expl_id IS NULL) THEN
			...
			IF (NEW.expl_id IS NULL) THEN
				...
				IF (NEW.expl_id IS NULL) THEN
					...
					"data":{"message":"2012", "function":"2478","parameters":{"feature_id":"'||NEW.gully_id::text||'"}}}$$);';
				END IF;
			END IF;
		END IF;
		...
		INSERT INTO review_gully (gully_id, top_elev, ymax, sandbox, matcat_id, gully_type, gullycat_id, units, groove, siphon, connec_arccat_id,
				annotation, observ, review_obs, expl_id, the_geom, field_checked, field_date)
		VALUES (NEW.gully_id, NEW.top_elev, NEW.ymax, NEW.sandbox, NEW.matcat_id, NEW.gully_type, NEW.gullycat_id, NEW.units, NEW.groove, NEW.siphon,
		...
		--looking for insert values on audit table
	  	IF NEW.field_checked=TRUE THEN
				...
				INSERT INTO review_audit_gully (gully_id, new_top_elev, new_ymax, new_sandbox, new_matcat_id, new_gully_type, new_gullycat_id, new_units,
    ...
    ELSIF TG_OP = 'UPDATE' THEN
		...
		UPDATE review_gully SET top_elev=NEW.top_elev, ymax=NEW.ymax, sandbox=NEW.sandbox, matcat_id=NEW.matcat_id,gully_type=NEW.gully_type,
				gullycat_id=NEW.gullycat_id,units=NEW.units, groove=NEW.groove, siphon=NEW.siphon, connec_arccat_id=NEW.connec_arccat_id,
		...
		IF (NEW.field_checked is TRUE) THEN
			...
			ELSE
				...
				new_matcat_id, new_gully_type, old_gully_type, old_gullycat_id,new_gullycat_id,old_units, new_units, old_groove, new_groove, old_siphon,
