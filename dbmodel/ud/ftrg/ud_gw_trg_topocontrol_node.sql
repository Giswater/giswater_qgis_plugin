			...
			IF (NEW.state=2 AND v_node.node_id IS NOT NULL) THEN
				...
				LOOP
					...
					ELSE
						...
						v_mantable = (SELECT man_table FROM cat_feature_arc c JOIN cat_feature cf ON cf.id = n.id JOIN sys_feature_cat s ON cf.sys_feature_cat = s.id JOIN v_edit_arc ON c.id=arc_type WHERE arc_id=v_arc.arc_id);
