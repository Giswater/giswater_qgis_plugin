...
BEGIN
	...
	IF TG_OP = 'INSERT' THEN
		...
		IF v_matfromcat THEN
			...
			drainzone_id, parent_id, expl_id2, adate, adescript, visitability, label_quadrant, brand_id, model_id, serial_number, streetname, streetname2, initoverflowpath, lock_level)
			...
			NEW.drainzone_id, NEW.parent_id, NEW.expl_id2, NEW.adate, NEW.adescript, NEW.visitability, NEW.label_quadrant, NEW.brand_id, NEW.model_id, NEW.serial_number, NEW.streetname, NEW.streetname2, NEW.initoverflowpath, NEW.lock_level);
		ELSE
			...
			NEW.asset_id, NEW.pavcat_id, NEW.drainzone_id, NEW.parent_id, NEW.expl_id2, NEW.adate, NEW.adescript, NEW.visitability, NEW.label_quadrant, NEW.brand_id, NEW.model_id, NEW.serial_number, NEW.streetname, NEW.streetname2, NEW.initoverflowpath, NEW.lock_level);
		END IF;
		...
		IF v_man_table='man_conduit' THEN
			...
			INSERT INTO man_conduit VALUES (NEW.arc_id, NEW.bottom_mat);
	...
	ELSIF TG_OP = 'UPDATE' THEN
		...
		IF v_matfromcat THEN
			...
			initoverflowpath=NEW.initoverflowpath, lock_level=NEW.lock_level
		...
		IF v_man_table='man_conduit' THEN
			...
			UPDATE man_conduit SET arc_id=NEW.arc_id, bottom_mat=NEW.bottom_mat WHERE arc_id=OLD.arc_id;
