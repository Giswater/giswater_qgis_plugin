...
BEGIN
	...
	IF TG_OP = 'INSERT' THEN
		...
		IF v_table = 'FLWREG-ORIFICE' THEN
			...
			INSERT INTO inp_frorifice (nodarc_id, node_id, order_id, to_arc, flwreg_length, ori_type, offsetval, cd, orate,
	 	...
	 	ELSIF v_table = 'FLWREG-OUTLET' THEN
			...
			INSERT INTO inp_froutlet (nodarc_id, node_id, order_id, to_arc, flwreg_length, outlet_type, offsetval, curve_id, cd1, cd2)
	 	...
	 	ELSIF v_table = 'FLWREG-PUMP' THEN
			...
			INSERT INTO inp_frpump (nodarc_id, node_id, order_id, to_arc, flwreg_length, curve_id, status, startup, shutoff)
	 	...
	 	ELSIF v_table = 'FLWREG-WEIR' THEN
			...
			INSERT INTO inp_frweir (nodarc_id, node_id, order_id, to_arc, flwreg_length, weir_type, offsetval, cd, ec,
	...
	ELSIF TG_OP = 'UPDATE' THEN
		...
		IF v_table = 'FLWREG-ORIFICE' THEN
			...
			UPDATE inp_frorifice SET nodarc_id=concat(NEW.node_id,'OR',NEW.order_id), node_id=NEW.node_id, order_id=NEW.order_id, to_arc=NEW.to_arc,
	 	...
	 	ELSIF v_table = 'FLWREG-OUTLET' THEN
			...
			UPDATE inp_froutlet SET nodarc_id=concat(NEW.node_id,'OT',NEW.order_id), node_id=NEW.node_id, order_id=NEW.order_id, to_arc=NEW.to_arc,
	 	...
	 	ELSIF v_table = 'FLWREG-PUMP' THEN
			...
			UPDATE inp_frpump SET nodarc_id=concat(NEW.node_id,'PU',NEW.order_id), node_id=NEW.node_id, order_id=NEW.order_id, to_arc=NEW.to_arc,
			flwreg_length= NEW.flwreg_length, curve_id=NEW.curve_id, status=NEW.status, startup=NEW.startup, shutoff=NEW.shutoff
			WHERE nodarc_id=OLD.nodarc_id;

	 	ELSIF v_table = 'FLWREG-WEIR' THEN

			UPDATE inp_frweir SET nodarc_id=concat(NEW.node_id,'WE',NEW.order_id), node_id=NEW.node_id, order_id=NEW.order_id, to_arc=NEW.to_arc, flwreg_length= NEW.flwreg_length,
	...
	ELSIF TG_OP = 'DELETE' THEN
		...
		IF v_table = 'FLWREG-ORIFICE' THEN
			DELETE FROM inp_frorifice WHERE nodarc_id=OLD.nodarc_id;

	 	ELSIF v_table = 'FLWREG-OUTLET' THEN
			DELETE FROM inp_froutlet WHERE nodarc_id=OLD.nodarc_id;

	 	ELSIF v_table = 'FLWREG-PUMP' THEN
			DELETE FROM inp_frpump WHERE nodarc_id=OLD.nodarc_id;

	 	ELSIF v_table = 'FLWREG-WEIR' THEN
			DELETE FROM inp_frweir WHERE nodarc_id=OLD.nodarc_id;
		END IF;
