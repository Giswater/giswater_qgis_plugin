...
CREATE VIEW "sample_ud".v_edit_inp_junction AS
SELECT 
node.node_id, node.top_elev, node.ymax, node.elev, node.nodecat_id, node.sector_id, node."state", node.annotation, node.observ, node.comment, node.dma_id, node.rotation, node.link, node.est_top_elev, node.est_ymax, node.verified, node.the_geom,
inp_junction.y0, inp_junction.ysur, inp_junction.apond
FROM ("sample_ud".node
JOIN "sample_ud".inp_junction ON (((inp_junction.node_id)::text = (node.node_id)::text)));


CREATE VIEW "sample_ud".v_edit_inp_divider AS
SELECT 
node.node_id, node.top_elev, node.ymax, node.elev, node.nodecat_id, node.sector_id, node."state", node.annotation, node.observ, node.comment, node.dma_id, node.rotation, node.link, node.est_top_elev, node.est_ymax, node.verified, node.the_geom,
...
CREATE VIEW "sample_ud".v_edit_inp_outfall AS
SELECT 
node.node_id, node.top_elev, node.ymax, node.elev, node.nodecat_id, node.sector_id, node."state", node.annotation, node.observ, node.comment, node.dma_id, node.rotation, node.link, node.est_top_elev, node.est_ymax, node.verified, node.the_geom,
...
CREATE VIEW "sample_ud".v_edit_inp_storage AS
SELECT 
node.node_id, node.top_elev, node.ymax, node.elev, node.nodecat_id, node.sector_id, node."state", node.annotation, node.observ, node.comment, node.dma_id, node.rotation, node.link, node.est_top_elev, node.est_ymax, node.verified, node.the_geom,
...
CREATE VIEW "sample_ud".v_edit_inp_conduit AS
SELECT 
...
CREATE VIEW "sample_ud".v_edit_inp_orifice AS
SELECT
...
FROM ("sample_ud".arc
JOIN "sample_ud".inp_orifice ON (((arc.arc_id)::text = (inp_orifice.arc_id)::text)));


CREATE VIEW "sample_ud".v_edit_inp_outlet AS
SELECT 
arc.arc_id, arc.y1, arc.y2, arc.arccat_id, arc.sector_id, arc."state", arc.annotation, arc.observ, arc.comment, arc.dma_id, arc.custom_length, arc.rotation, arc.link, arc.est_y1, arc.est_y2, arc.verified, arc.the_geom,
...
CREATE VIEW "sample_ud".v_edit_inp_pump AS
SELECT 
arc.arc_id, arc.y1, arc.y2, arc.arccat_id, arc.sector_id, arc."state", arc.annotation, arc.observ, arc.comment, arc.dma_id, arc.custom_length, arc.rotation, arc.link, arc.est_y1, arc.est_y2, arc.verified, arc.the_geom,
...
CREATE VIEW "sample_ud".v_edit_inp_weir AS
SELECT 
arc.arc_id, arc.y1, arc.y2, arc.arccat_id, arc.sector_id, arc."state", arc.annotation, arc.observ, arc.comment, arc.dma_id, arc.custom_length, arc.rotation, arc.link, arc.est_y1, arc.est_y2, arc.verified, arc.the_geom,
...
FROM ("sample_ud".arc
JOIN "sample_ud".inp_weir ON (((arc.arc_id)::text = (inp_weir.arc_id)::text)));
