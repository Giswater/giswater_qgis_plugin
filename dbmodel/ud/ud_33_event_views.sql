...
CREATE OR REPLACE VIEW sample_ud.v_event_x_storage AS
SELECT
event_x_storage.id,
...
node.the_geom
FROM sample_ud.event_x_storage
JOIN sample_ud.event ON event.id::text = event_x_storage.event_id::text
JOIN sample_ud.node ON node.node_id::text = event_x_storage.node_id::text
GROUP BY
event_x_storage.id,
event_x_storage.event_id,
event_x_storage.node_id,
...
CREATE OR REPLACE VIEW sample_ud.v_event_x_outfall AS
SELECT
event_x_outfall.id,
...
node.the_geom
FROM sample_ud.event_x_outfall
JOIN sample_ud.event ON event.id::text = event_x_outfall.event_id::text
JOIN sample_ud.node ON node.node_id::text = event_x_outfall.node_id::text
GROUP BY
event_x_outfall.id,
event_x_outfall.event_id,
event_x_outfall.node_id,
...
CREATE OR REPLACE VIEW sample_ud.v_event_x_conduit AS
SELECT 
event_x_conduit.id,
...
arc.the_geom
FROM sample_ud.event_x_conduit
JOIN sample_ud.event ON event.id::text = event_x_conduit.event_id::text
JOIN sample_ud.arc ON arc.arc_id::text = event_x_conduit.arc_id::text
GROUP BY
event_x_conduit.id,
event_x_conduit.event_id,
event_x_conduit.arc_id,
...
CREATE OR REPLACE VIEW sample_ud.v_event_x_gully AS
SELECT 
event_x_gully.id,
...
gully.the_geom
FROM sample_ud.event_x_gully
JOIN sample_ud.event ON event.id::text = event_x_gully.event_id::text
JOIN sample_ud.gully ON gully.gully_id::text = event_x_gully.gully_id::text
GROUP BY
event_x_gully.id,
event_x_gully.event_id,
event_x_gully.gully_id,
