				...
				EXECUTE 'SET search_path TO '||quote_literal(TG_TABLE_SCHEMA)||', public';
					...
					IF TG_OP = 'INSERT' THEN
							...
							IF NEW.field_checked=TRUE THEN
								INSERT INTO review_audit_node (node_id, the_geom, top_elev, ymax, node_type, nodecat_id, annotation, observ, verified, field_checked,"operation", "user", date_field, office_checked) 
								VALUES(NEW.node_id, NEW.the_geom, NEW.top_elev, NEW.ymax, NEW.node_type, NEW.nodecat_id,NEW.annotation, NEW.observ, 'REVISED', NEW.field_checked, 'INSERT', user, CURRENT_TIMESTAMP, 
								NEW.office_checked);
					...
					ELSIF TG_OP = 'UPDATE' THEN
						...
						IF EXISTS (SELECT node_id FROM review_audit_node WHERE node_id=NEW.node_id) THEN
									...
									nodecat_id=NEW.nodecat_id, annotation=NEW.annotation, observ=NEW.observ, verified='REVISED', field_checked=NEW.field_checked, "operation"='UPDATE',"user"=user, date_field=CURRENT_TIMESTAMP, office_checked=NEW.office_checked
						...
						ELSE
							IF NEW.the_geom=OLD.the_geom THEN
								INSERT INTO review_audit_node (node_id, the_geom, top_elev, ymax, node_type, nodecat_id, annotation, observ, verified, field_checked,"operation", "user", date_field, office_checked,moved_geom) 
								VALUES (NEW.node_id, NEW.the_geom, NEW.top_elev, NEW.ymax, NEW.node_type, NEW.nodecat_id, NEW.annotation, NEW.observ, 'REVISED', NEW.field_checked, 'INSERT', user, CURRENT_TIMESTAMP, 
