...
BEGIN
    ...
    IF TG_OP = 'INSERT' THEN
        ...
        IF (NEW.sector_id IS NULL) THEN
            ...
            IF (NEW.sector_id IS NULL) THEN
                RETURN audit_function(1010,1202); 
            END IF;
        END IF;
        ...
        IF (NEW.dma_id IS NULL) THEN
            ...
            IF (NEW.dma_id IS NULL) THEN
                RETURN audit_function(1014,1202); 
            END IF;
        END IF;
		...
		IF (NEW.expl_id IS NULL) THEN
			...
			IF (NEW.expl_id IS NULL) THEN
				...
				IF (NEW.expl_id IS NULL) THEN
					PERFORM audit_function(2012,1202);
				END IF;		
			END IF;
		END IF;
        ...
        -- FEATURE INSERT
				...
				muni_id, streetaxis_id, postcode, streetaxis_02_id, postnumber, postnumber_02, descript, link, verified, the_geom, undelete,label_x,label_y, 
				...
				NEW.muni_id, NEW.streetaxis_id, NEW.postcode, NEW.streetaxis_02_id, NEW.postnumber, NEW.postnumber_02, NEW.descript, NEW.link, NEW.verified, NEW.the_geom,NEW.undelete,NEW.label_x,NEW.label_y, 
	...
	END IF;
		...
		postnumber=NEW.postnumber, postnumber_02=NEW.postnumber_02, descript=NEW.descript, link=NEW.link, verified=NEW.verified, undelete=NEW.undelete,label_x=NEW.label_x,label_y=NEW.label_y, label_rotation=NEW.label_rotation,
