...
BEGIN
 ...
 IF gully_geometry = 'gully' THEN
        INSERT INTO gully (gully_id, top_elev, "ymax",sandbox, matcat_id, gratecat_id, units, groove, arccat_id, siphon, arc_id, sector_id, "state",annotation, "observ", "comment", rotation,dma_id, 
					...
					NEW.feature_id,NEW.label_x, NEW.label_y,NEW.label_rotation, NEW.code, expl_id_int , NEW.publish, NEW.inventory, NEW.end_date, NEW.streetaxis_id, NEW.postnumber, NEW.connec_length, NEW.connec_depth);
        ...
        INSERT INTO gully (gully_id, top_elev, "ymax",sandbox, matcat_id, gratecat_id, units, groove, arccat_id, siphon, arc_id,sector_id, "state", annotation, "observ", "comment", rotation,
					...
					NEW.label_x, NEW.label_y,NEW.label_rotation,  NEW.code, expl_id_int, NEW.publish, NEW.inventory, NEW.end_date, NEW.streetaxis_id, NEW.postnumber);
        END IF;     
    ...
    ELSIF TG_OP = 'UPDATE' THEN
       ...
       -- UPDATE values
		IF gully_geometry = 'gully' THEN
			...
			end_date=NEW.end_date, streetaxis_id=NEW.streetaxis_id, postnumber=NEW.postnumber,  connec_length=NEW.connec_length, connec_depth=NEW.connec_depth, expl_id=NEW.expl_id
