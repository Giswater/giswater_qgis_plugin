-- DROP FUNCTION SCHEMA_NAME.gw_fct_cso_calculation(json);
...
--modo calculo
SELECT SCHEMA_NAME.gw_fct_cso_calculation($${"client":{"device":4, "lang":"es_ES", "epsg":25830}, "form":{}, "feature":{}, 
...
BEGIN
	...
	v_sourcetables := p_data -> 'data' -> 'parameters' -> 'calculation' ->> 'sourceTables';
	...
	IF v_drainzone_id IS NOT NULL OR v_drainzone_id !='' THEN
		--v_filter_drainzone = 'AND b.drainzone_id IN ('||v_drainzone_id||')';
	 	v_filter_drainzone = ''||v_expl_id||' = ANY(b.expl_id::int4[])';
	ELSE
		v_filter_drainzone = 'AND '||v_expl_id||' = ANY(b.expl_id::int4[])';
	END IF;
	...
	LEFT JOIN drainzone b ON graphconfig->''use''->0->>''nodeParent'' = a.node_id::text
	...
	IF (SELECT EXISTS(SELECT 1 FROM cso_inp_rainfall)) IS FALSE THEN
		return '{"status": "Failed", "message":{"level":1, "text":"The selected exploitation does not have rainfall in the inp_timeseries"}}'::json;
	END IF;

	if (select count(*) from cso_inp_rainfall) = 0 then
		raise exception 'There are no rainfall in the cso_inp_rainfall';
	end if;
	...
	IF v_step_calculation THEN -- INIT CALCULATION PROCESS
		...
		ELSIF v_sourcetables = 'SWMM' THEN
			...
			IF (SELECT EXISTS (SELECT idval FROM cat_dwf WHERE expl_id = v_expl_id)) IS NOT TRUE THEN
	
				INSERT INTO cat_dwf (idval, expl_id) 
			...
			ELSE
				...
				SELECT id INTO v_dwfscenario_id FROM cat_dwf WHERE expl_id = v_expl_id LIMIT 1;
			...
			WITH sums AS (
				SELECT 
				n.drainzone_outfall,
				sum(st_area(a.the_geom)) AS thyssen_plv_area,
				...
				LEFT JOIN drainzone b ON b.drainzone_id = ANY(n.drainzone_outfall)
				LEFT JOIN inp_dwf e ON a.outlet_id = e.node_id::TEXT
				WHERE e.dwfscenario_id = '||v_dwfscenario_id||'
				GROUP BY n.drainzone_outfall
			), aliv AS (
				SELECT 
				a.node_id, 
				b.drainzone_outfall,
				...
				a.vmax AS vret
				FROM cso_inp_weir a
				LEFT JOIN node b ON a.node_id = b.node_id
			)
			SELECT
			aliv.node_id,
			aliv.q_max,
			aliv.vret,
			sums.*
			FROM sums JOIN aliv USING (drainzone_outfall)';
			...
			mean_coef_runoff = a.mean_coef_runof,
			demand = a.demand 
			FROM ('||v_sql||')a 
			WHERE t.drainzone_id = ANY(a.drainzone_outfall)';
	...
	FROM drainzone a 
	JOIN inp_timeseries c ON c.expl_id = ANY(a.expl_id)
	...
	LOOP
		...
		FOR rec_rainfall in EXECUTE 'select *, split_part(rf_tstep, '':'', 1)::int+1 AS hourly from cso_inp_rainfall order by rf_name, rf_tstep'
		LOOP
			...
			v_residual_pattern = coalesce(v_residual_pattern, 1);
	...
	/*
		...
		ELSE
			...
			LOOP			
				...
				FROM cso_out_vol WHERE drainzone_id IN ('||v_drainzone_array||') AND RIGHT(rf_name, 2)::integer = '||v_i||' 
				ON CONFLICT (dscenario_id, node_id, order_id) DO NOTHING;';
...
*/
	...
	-- caudal residual que llega a ese punto (dwf_pd80_percent.v_cso_drainzone)
	UPDATE drainzone t SET addparam = a.new_addparam FROM (
		WITH mec AS (
			SELECT drainzone_id,
				CASE
	...
	v_result_info = concat ('{"geometryType":"", "values":',v_result, '}');
  	...
  	--polygon;
	  	...
	  	FROM (
		    ...
		    FROM (SELECT drainzone_id AS id, NULL AS descript, expl_id, efficiency, the_geom FROM v_cso_drainzone) 
	   	row) features;
