...
CREATE OR REPLACE FUNCTION SCHEMA_NAME.gw_fct_getprint(p_active_composer text, p_device integer)
  RETURNS json AS
$BODY$
DECLARE
...
-- Create tabs array
    ...
    -- Get fields
    EXECUTE 'SELECT array_agg(row_to_json(a)) FROM (SELECT id, label, name, type, "dataType", placeholder, widgetfunction AS "changeAction" FROM config_web_fields WHERE table_id = ''F32'' order by orderby) a'
        INTO fields_array;
