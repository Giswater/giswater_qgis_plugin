...
VALUES(902, 'DB Activity', 'SELECT date, user_name, count, action  FROM SCHEMA_NAME.v_log_ws',
...
CREATE OR REPLACE VIEW SCHEMA_NAME.v_log_ws AS
...
GRANT ALL ON TABLE SCHEMA_NAME.v_log_ws TO role_master;


CREATE OR REPLACE VIEW SCHEMA_NAME.v_log_ud AS
SELECT user_name,  count (*) , action, date FROM
(SELECT user_name, substring(query,0,30)  as action, (substring(date_trunc('day',(tstamp))::text,0,12))::date AS date from SCHEMA_NAME.log
where schema = 'ud')a
group by user_name, date, action
...
GRANT ALL ON TABLE SCHEMA_NAME.v_log_ud TO role_master;
