...
--					Update direction if necessary
					IF (z2 > z1) THEN
						...
						EXECUTE 'UPDATE arc SET z1 = ' || arcrec.z2 || ', z2 = ' || arcrec.z1 || ' WHERE arc_id = ' || quote_literal(arcrec."arc_id"); 
...
--			Material catalog ID
		...
		INSERT INTO inp_conduit VALUES(NEW.arc_id,NEW.barrels,NEW.culvert,NEW.kentry,NEW.kexit,NEW.kavg,NEW.flap,NEW.q0,NEW.qmax);
		RETURN NEW;
    ...
    ELSIF TG_OP = 'UPDATE' THEN
		...
		UPDATE inp_conduit SET arc_id=NEW.arc_id,barrels=NEW.barrels,culvert=NEW.culvert,kentry=NEW.kentry,kexit=NEW.kexit,kavg=NEW.kavg,flap=NEW.flap,q0=NEW.q0,qmax=NEW.qmax WHERE arc_id=OLD.arc_id;
		RETURN NEW;
...
CREATE OR REPLACE FUNCTION SCHEMA_NAME.update_v_inp_edit_divider() RETURNS trigger LANGUAGE plpgsql AS $$
...
--		Trigger error				
		...
		INSERT INTO node VALUES(NEW.node_id,NEW.top_elev,NEW.ymax,'DIVIDER'::TEXT,NEW.sector_id,NEW.the_geom);
	...
	ELSIF TG_OP = 'UPDATE' THEN
		UPDATE node SET node_id=NEW.node_id,top_elev=NEW.top_elev,elev=NEW.elev,ymax=NEW.ymax,sector_id=NEW.sector_id,the_geom=NEW.the_geom WHERE node_id=OLD.node_id;
...
--			Sector ID
	...
	ELSIF TG_OP = 'UPDATE' THEN
		UPDATE arc SET arc_id=NEW.arc_id,z1=NEW.z1,z2=NEW.z2,categ_type=NEW.categ_type,systm_type=NEW.systm_type,sector_id=NEW.sector_id,the_geom=NEW.the_geom WHERE arc_id=OLD.arc_id;
...
CREATE OR REPLACE FUNCTION SCHEMA_NAME.update_v_inp_edit_outfall() RETURNS trigger LANGUAGE plpgsql AS $$
...
--		Trigger error				
		...
		INSERT INTO node VALUES(NEW.node_id,NEW.top_elev,NEW.ymax,'OUTFALL'::TEXT,NEW.sector_id,NEW.the_geom);
...
CREATE OR REPLACE FUNCTION SCHEMA_NAME.update_v_inp_edit_outlet() RETURNS trigger LANGUAGE plpgsql AS $$
...
--			Sector ID
	   ...
	   INSERT INTO arc VALUES(NEW.arc_id,'','',NEW.z1,NEW.z2,DEFAULT,DEFAULT,'OUTLET'::TEXT,NEW.sector_id,NEW.the_geom);
...
--			Sector ID
       ...
       INSERT INTO arc VALUES(NEW.arc_id,'','',NEW.z1,NEW.z2,DEFAULT,DEFAULT,'PUMP'::TEXT,NEW.sector_id,NEW.the_geom);
...
CREATE OR REPLACE FUNCTION SCHEMA_NAME.update_v_inp_edit_storage() RETURNS trigger LANGUAGE plpgsql AS $$
...
--		Trigger error				
		...
		INSERT INTO node VALUES(NEW.node_id,NEW.top_elev,NEW.ymax,'STORAGE'::TEXT,NEW.sector_id,NEW.the_geom);
