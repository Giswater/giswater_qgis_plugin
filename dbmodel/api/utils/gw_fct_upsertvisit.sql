...
--        Insert into element visit
        EXECUTE 'INSERT INTO ' || quote_ident('om_visit_x_' || element_type) || ' (visit_id,' || quote_ident( element_type || '_id') || ') VALUES ($1, $2)'
            ...
            is_done := FALSE;
    ELSE
        --get value of visicat_id
        EXECUTE 'SELECT visitcat_id FROM om_visit
        WHERE ' || id_visit|| ' = id;' INTO query_result_def_visit;
...
--    Control NULL's
    ...
    query_result_def_visit := COALESCE(query_result_def_visit, '');
