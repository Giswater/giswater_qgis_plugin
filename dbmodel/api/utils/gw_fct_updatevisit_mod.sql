CREATE OR REPLACE FUNCTION "SCHEMA_NAME"."gw_fct_updatevisit_mod"(visit_id int8, column_name varchar, value_new varchar) RETURNS pg_catalog.json AS $BODY$
DECLARE
...
--    Control NULL's
    ...
    raise notice '%',return_multiplier_aux;

--    Return
    RETURN ('{"status":"Accepted", "apiVersion":'|| api_version ||', "geometry":"'||return_multiplier_aux||'"}')::json;    
