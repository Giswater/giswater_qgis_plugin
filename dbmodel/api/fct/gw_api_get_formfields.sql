...
BEGIN
	...
	IF p_formname!='infoplan' THEN 
		...
		EXECUTE 'SELECT array_agg(row_to_json(a)) FROM (SELECT '||v_label||', column_id, concat('||quote_literal(p_tabname)||',''_'',column_id) AS widgetname, widgettype, 
			...
			dv_orderby_id AS "orderById", dv_isnullvalue AS "isNullValue", stylesheet, widgetcontrols, widgetcontrols->>''autoupdateReloadFields'' as "reloadfields", 
	...
	IF (p_tgop ='UPDATE' OR p_tgop = 'SELECT') AND p_tablename IS NOT NULL AND p_idname IS NOT NULL AND p_id IS NOT NULL THEN
		EXECUTE 'SELECT row_to_json(a) FROM (SELECT * FROM ' || quote_ident(p_tablename) || ' WHERE ' || quote_ident(p_idname) || ' = CAST(' || quote_literal(p_id) || ' AS ' ||
	...
	LOOP
		...
		ELSIF (p_tgop ='UPDATE' OR p_tgop = 'SELECT') THEN
			v_selected_id := v_featurevalues->>(aux_json->>'parentId');	
		...
		IF (aux_json->>'queryTextFilter') IS NOT NULL AND v_selected_id IS NOT NULL THEN	
			query_text= 'SELECT (array_agg(id)) FROM ('|| (aux_json->>'queryText') || (aux_json->>'queryTextFilter')||' '||quote_literal(v_selected_id)||' ORDER BY '||v_orderby||') a';
		...
		IF (aux_json->>'dv_querytext_filterc') IS NOT NULL AND v_selected_id IS NOT NULL THEN
			query_text= 'SELECT (array_agg(idval)) FROM ('|| (aux_json->>'queryText') ||(aux_json->>'dv_querytext_filterc')||' '||quote_literal(v_selected_id)||' ORDER BY '||v_orderby||') a';
			execute query_text INTO v_array;
		ELSE 	
