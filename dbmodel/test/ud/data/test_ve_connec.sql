...
-- connec -> ve_connec_cjoin
INSERT INTO ve_connec_cjoin
(connec_id, code, sys_code, customer_code, top_elev, y1, y2, conneccat_id, connec_type, sys_type, matcat_id, state, state_type, expl_id, macroexpl_id, sector_id, macrosector_id, drainzone_id, demand, connec_depth, connec_length, arc_id, annotation, observ, "comment", omzone_id, macroomzone_id, soilcat_id, function_type, category_type, fluid_type, location_type, workcat_id, workcat_id_end, builtdate, enddate, ownercat_id, muni_id, postcode, district_id, streetaxis_id, postnumber, postcomplement, streetaxis2_id, postnumber2, postcomplement2, region_id, province_id, descript, svg, rotation, link, verified, "label", label_x, label_y, label_rotation, accessibility, diagonal, publish, inventory, uncertain, num_value, pjoint_id, pjoint_type, created_at, created_by, updated_at, updated_by, the_geom, workcat_id_plan, asset_id, is_operative, adate, adescript, plot_code, placement_type)
VALUES('-901', '-901', '-901', '-901', 33.2200, NULL, NULL, 'CON-CC040_I', 'CJOIN', 'CONNEC', NULL, 1, 2, 2, 1, 2, 2, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 3, NULL, 'soil1', NULL, NULL, 0, NULL, 'work1', NULL, '2024-08-23', NULL, 'owner1', 2, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, 1, 1, NULL, NULL, NULL, '', '1', NULL, NULL, NULL, NULL, NULL, NULL, true, true, false, NULL, NULL, NULL, '2024-08-23 09:38:22.000', 'postgres', NULL, NULL, 'SRID=25831;POINT (418456.45053275774 4577991.751653385)'::public.geometry, NULL, NULL, true, NULL, NULL, NULL, NULL);
SELECT is((SELECT count(*)::integer FROM ve_connec_cjoin WHERE code = '-901'), 1, 'INSERT: ve_connec_cjoin "-901" was inserted');
SELECT is((SELECT count(*)::integer FROM connec WHERE code = '-901'), 1, 'INSERT: connec "-901" was inserted');

UPDATE ve_connec_cjoin SET top_elev = 33.0000 WHERE code = '-901';
SELECT is((SELECT top_elev FROM ve_connec_cjoin WHERE code = '-901'), 33.0000, 'UPDATE: ve_connec_cjoin "-901" was updated');
SELECT is((SELECT top_elev FROM connec WHERE code = '-901'), 33.0000, 'UPDATE: connec "-901" was updated');

DELETE FROM ve_connec_cjoin WHERE code = '-901';
SELECT is((SELECT count(*)::integer FROM ve_connec_cjoin WHERE code = '-901'), 0, 'DELETE: ve_connec_cjoin "-901" was deleted');
