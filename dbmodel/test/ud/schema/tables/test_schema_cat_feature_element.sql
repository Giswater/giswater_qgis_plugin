...
-- Check primary key
SELECT col_is_pk('cat_feature_element', 'id', 'Column id should be primary key'); 

-- Check check columns
SELECT col_has_check('cat_feature_element', 'epa_default', 'Table should have check on epa_default');
...
SELECT fk_ok('cat_feature_element', 'id', 'cat_feature', 'id', 'Table should have foreign key from id to cat_feature.id');

-- Check indexes
SELECT has_index('cat_feature_element', 'cat_feature_element_pkey', ARRAY['id'], 'Table should have index on id');
