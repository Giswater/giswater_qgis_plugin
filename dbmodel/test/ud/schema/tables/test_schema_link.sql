...
SELECT columns_are(
    'link',
    ARRAY[
        'link_id', 'code', 'sys_code', 'top_elev1', 'y1', 'exit_id',
        'exit_type', 'top_elev2', 'y2', 'feature_type', 'feature_id', 'link_type',
        'linkcat_id', 'state', 'state_type', 'expl_id', 'muni_id', 'sector_id',
        'dma_id', 'dwfzone_id', 'omzone_id', 'drainzone_outfall', 'dwfzone_outfall', 'location_type',
        '_fluid_type', 'fluid_type', 'custom_length', 'sys_slope', 'annotation', 'observ',
...
SELECT col_type_is('link', 'y1', 'numeric(12,4)', 'Column y1 should be numeric(12,4)');
...
SELECT col_type_is('link', 'y2', 'numeric(12,4)', 'Column y2 should be numeric(12,4)');
...
SELECT col_type_is('link', 'link_type', 'varchar(30)', 'Column link_type should be varchar(30)');
...
SELECT col_type_is('link', 'dwfzone_id', 'int4', 'Column dwfzone_id should be int4');
SELECT col_type_is('link', 'omzone_id', 'int4', 'Column omzone_id should be int4');
SELECT col_type_is('link', 'drainzone_outfall', 'int4[]', 'Column drainzone_outfall should be int4[]');
SELECT col_type_is('link', 'dwfzone_outfall', 'int4[]', 'Column dwfzone_outfall should be int4[]');
SELECT col_type_is('link', 'location_type', 'varchar(50)', 'Column location_type should be varchar(50)');
SELECT col_type_is('link', '_fluid_type', 'varchar(50)', 'Column _fluid_type should be varchar(50)');
SELECT col_type_is('link', 'fluid_type', 'int4', 'Column fluid_type should be int4');
SELECT col_type_is('link', 'custom_length', 'numeric(12,2)', 'Column custom_length should be numeric(12,2)');
SELECT col_type_is('link', 'sys_slope', 'numeric(12,3)', 'Column sys_slope should be numeric(12,3)');
...
SELECT col_type_is('link', 'private_linkcat_id', 'varchar(30)', 'Column private_linkcat_id should be varchar(30)');
...
SELECT fk_ok('link', 'dwfzone_id', 'dwfzone', 'dwfzone_id', 'FK dwfzone_id → dwfzone.dwfzone_id');
...
SELECT fk_ok('link', 'private_linkcat_id', 'cat_arc', 'id', 'FK private_linkcat_id → cat_arc.id');
...
SELECT fk_ok('link', 'link_type', 'cat_feature_link', 'id', 'FK link_type → cat_feature_link.id');
SELECT fk_ok('link', 'location_type', 'man_type_location', 'location_type', 'FK location_type → man_type_location.location_type');
SELECT fk_ok('link', 'dma_id', 'dma', 'dma_id', 'FK dma_id → dma.dma_id');
SELECT fk_ok('link', 'omunit_id', 'omunit', 'omunit_id', 'FK omunit_id → omunit.omunit_id');
