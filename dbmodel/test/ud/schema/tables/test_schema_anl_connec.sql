...
SELECT col_type_is('anl_connec', 'dwfzone_id', 'int4', 'Column dwfzone_id should be int4');
SELECT col_type_is('anl_connec', 'drainzone_id', 'int4', 'Column drainzone_id should be int4');

-- Check foreign keys
SELECT has_fk('anl_connec', 'Table anl_connec should have foreign keys');

SELECT fk_ok('anl_connec', 'dwfzone_id', 'dwfzone', 'dwfzone_id', 'FK dwfzone_id → dwfzone.dwfzone_id');
SELECT fk_ok('anl_connec', 'drainzone_id', 'drainzone', 'drainzone_id', 'FK drainzone_id → drainzone.drainzone_id');
