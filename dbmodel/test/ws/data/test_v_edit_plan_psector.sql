...
INSERT INTO v_edit_plan_psector ( "name", descript, priority, text1, text2, observ, rotation, "scale", atlas_id, gexpenses, vat, other, the_geom, expl_id, psector_type, active, archived, ext_code, status, text3, text4, text5, text6, num_value, workcat_id, parent_id)
...
SELECT is((SELECT count(*)::integer FROM v_edit_plan_psector WHERE name = '-901'), 1, 'INSERT: v_edit_plan_psector -901 was inserted');
SELECT is((SELECT count(*)::integer FROM plan_psector WHERE name = '-901'), 1, 'INSERT: plan_psector -901 was inserted');


UPDATE v_edit_plan_psector SET descript = 'updated descript' WHERE name = '-901';
SELECT is((SELECT descript FROM v_edit_plan_psector WHERE name = '-901'), 'updated descript', 'UPDATE: v_edit_plan_psector -901 was updated');
SELECT is((SELECT descript FROM plan_psector WHERE name = '-901'), 'updated descript', 'UPDATE: plan_psector -901 was updated');


DELETE FROM v_edit_plan_psector WHERE name = '-901';
SELECT is((SELECT count(*)::integer FROM v_edit_plan_psector WHERE name = '-901'), 0, 'DELETE: v_edit_plan_psector -901 was deleted');
