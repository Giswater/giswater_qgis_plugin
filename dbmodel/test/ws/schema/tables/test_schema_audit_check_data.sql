...
SELECT columns_are(
    'audit_check_data',
    ARRAY[
        'id', 'fid', 'result_id', 'table_id', 'column_id', 'criticity',
        'enabled', 'error_message', 'tstamp', 'cur_user', 'feature_type', 'feature_id',
        'addparam', 'fcount'
    ],
...
-- Check column types
SELECT col_type_is('audit_check_data', 'id', 'int4', 'Column id should be int4');
...
SELECT col_type_is('audit_check_data', 'tstamp', 'timestamp without time zone', 'Column tstamp should be timestamp without time zone');
