...
SELECT col_type_is('cat_feature_connec', 'epa_default', 'varchar(30)', 'Column epa_default should be varchar(30)');

-- Check foreign keys
SELECT has_fk('cat_feature_connec', 'Table cat_feature_connec should have foreign keys');

SELECT fk_ok('cat_feature_connec', 'id', 'cat_feature', 'id', 'FK id → cat_feature.id');
