...
SELECT columns_are(
    'man_type_category',
    ARRAY[
        'category_type', 'feature_type', 'featurecat_id', 'observ', 'active'
    ],
    'Table man_type_category should have the correct columns'
);

-- Check primary key
SELECT col_is_pk('man_type_category', ARRAY['category_type'], 'Column category_type should be primary key');

-- Check column types
SELECT col_type_is('man_type_category', 'category_type', 'varchar(50)', 'Column category_type should be varchar(50)');
SELECT col_type_is('man_type_category', 'feature_type', 'text[]', 'Column feature_type should be text[]');
