...
-- Check column types
SELECT col_type_is('inp_virtualvalve', 'arc_id', 'int4', 'Column arc_id should be int4');
...
SELECT col_type_is('inp_virtualvalve', 'init_quality', 'float8', 'Column init_quality should be float8');

-- Check foreign keys
SELECT has_fk('inp_virtualvalve', 'Table inp_virtualvalve should have foreign keys');

SELECT fk_ok('inp_virtualvalve', 'curve_id', 'inp_curve', 'id', 'FK curve_id → inp_curve.id');
SELECT fk_ok('inp_virtualvalve', 'arc_id', 'arc', 'arc_id', 'FK arc_id → arc.arc_id');
