...
SELECT columns_are(
    ...
    ARRAY[
        ...
        'demand_pattern_id', 'emitter_coeff'
    ],
...
SELECT col_type_is('inp_inlet', 'head', 'float8', 'Column head should be float8');
SELECT col_type_is('inp_inlet', 'mixing_model', 'varchar(18)', 'Column mixing_model should be varchar(18)');
SELECT col_type_is('inp_inlet', 'mixing_fraction', 'float8', 'Column mixing_fraction should be float8');
SELECT col_type_is('inp_inlet', 'reaction_coeff', 'float8', 'Column reaction_coeff should be float8');
SELECT col_type_is('inp_inlet', 'init_quality', 'float8', 'Column init_quality should be float8');
SELECT col_type_is('inp_inlet', 'source_type', 'varchar(18)', 'Column source_type should be varchar(18)');
SELECT col_type_is('inp_inlet', 'source_quality', 'float8', 'Column source_quality should be float8');
...
SELECT col_type_is('inp_inlet', 'emitter_coeff', 'float8', 'Column emitter_coeff should be float8');

-- Check foreign keys
SELECT has_fk('inp_inlet', 'Table inp_inlet should have foreign keys');

SELECT fk_ok('inp_inlet', 'curve_id', 'inp_curve', 'id', 'FK curve_id → inp_curve.id');
SELECT fk_ok('inp_inlet', 'node_id', 'node', 'node_id', 'FK node_id → node.node_id');
SELECT fk_ok('inp_inlet', 'pattern_id', 'inp_pattern', 'pattern_id', 'FK pattern_id → inp_pattern.pattern_id');
