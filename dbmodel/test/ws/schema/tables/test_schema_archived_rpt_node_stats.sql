...
-- Check column types
SELECT col_type_is('archived_rpt_node_stats', 'node_id', 'varchar(16)', 'Column node_id should be varchar(16)');
SELECT col_type_is('archived_rpt_node_stats', 'result_id', 'varchar(30)', 'Column result_id should be varchar(30)');
SELECT col_type_is('archived_rpt_node_stats', 'node_type', 'varchar(30)', 'Column node_type should be varchar(30)');
SELECT col_type_is('archived_rpt_node_stats', 'sector_id', 'int4', 'Column sector_id should be int4');
SELECT col_type_is('archived_rpt_node_stats', 'nodecat_id', 'varchar(30)', 'Column nodecat_id should be varchar(30)');
...
SELECT col_type_is('archived_rpt_node_stats', 'the_geom', 'geometry(point, SRID_VALUE)', 'Column the_geom should be geometry(point, SRID_VALUE)');

-- Check foreign keys
SELECT has_fk('archived_rpt_node_stats', 'Table archived_rpt_node_stats should have foreign keys');

SELECT fk_ok('archived_rpt_node_stats', 'result_id', 'rpt_cat_result', 'result_id', 'FK result_id → rpt_cat_result.result_id');
