...
-- Check column types
SELECT col_type_is('config_graph_mincut', 'node_id', 'int4', 'Column node_id should be int4');
SELECT col_type_is('config_graph_mincut', 'parameters', 'json', 'Column parameters should be json');
SELECT col_type_is('config_graph_mincut', 'active', 'bool', 'Column active should be bool');

-- Check foreign keys
SELECT has_fk('config_graph_mincut', 'Table config_graph_mincut should have foreign keys');

SELECT fk_ok('config_graph_mincut', 'node_id', 'node', 'node_id', 'FK node_id → node.node_id');
