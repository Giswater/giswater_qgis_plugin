...
SELECT columns_are(
    'man_tap',
    ARRAY[
        'connec_id', 'linked_connec', 'drain_diam', 'drain_exit', 'drain_gully', 'drain_distance',
        'arq_patrimony', 'com_state'
    ],
    'Table man_tap should have the correct columns'
);

-- Check column types
SELECT col_type_is('man_tap', 'connec_id', 'int4', 'Column connec_id should be int4');
SELECT col_type_is('man_tap', 'linked_connec', 'int4', 'Column linked_connec should be int4');
...
SELECT col_type_is('man_tap', 'arq_patrimony', 'bool', 'Column arq_patrimony should be bool');
...
SELECT fk_ok('man_tap', 'connec_id', 'connec', 'connec_id', 'FK connec_id → connec.connec_id');
SELECT fk_ok('man_tap', 'linked_connec', 'connec', 'connec_id', 'FK linked_connec → connec.connec_id');
