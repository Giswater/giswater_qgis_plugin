...
SELECT columns_are(
    'audit_check_project',
    ARRAY[
        'id', 'table_id', 'table_host', 'table_dbname', 'table_schema', 'fid',
        'criticity', 'enabled', 'message', 'tstamp', 'cur_user', 'observ',
        'table_user'
    ],
...
-- Check column types
SELECT col_type_is('audit_check_project', 'id', 'int4', 'Column id should be int4');
...
SELECT col_type_is('audit_check_project', 'tstamp', 'timestamp without time zone', 'Column tstamp should be timestamp without time zone');
...
SELECT fk_ok('audit_check_project', 'fid', 'sys_fprocess', 'fid', 'FK fid → sys_fprocess.fid');
