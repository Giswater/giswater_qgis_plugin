...
BEGIN
	...
	IF v_upsertattributes THEN 
		...
		ELSIF v_fprocesscat=30 THEN -- sector
			...
			FROM connec n JOIN audit_log_data a ON feature_id=node_id WHERE fprocesscat_id=30 AND user_name=current_user;
			
			-- update staticpressure on parent tables
			UPDATE node SET staticpressure=log_message::float FROM audit_log_data WHERE feature_id=node_id AND fprocesscat_id=47 AND user_name=current_user;
			--UPDATE connec SET staticpressure = arc.sector_id FROM arc WHERE arc.arc_id=connec.arc_id;
