...
BEGIN
	...
	IF v_buildupmode = 1 THEN
		...
		IF v_count > 0 THEN
			INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
			VALUES (v_fprocesscat_id, v_result_id, 4, concat('SUPPLY MODE: There is/are ',v_count,' pump_type = 1 with null values on curve_id column. Default user value for curve of pumptype = 1 ( ',v_curvedefault,' ) have been chosen.'));					
			v_count=0;
		END IF;	
	ELSE
	...
	ELSIF v_demandtype > 1 THEN --(2 or 3)
		...
		ELSE
			INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message)
			VALUES (v_fprocesscat_id, v_result_id, 1, concat('INFO: There is/are ', v_count, ' dma''s on this exportation. The dma-period table (ext_rtc_scada_dma_period) it''s filled.'));
			v_count=0;
		END IF;
	END IF;
	...
	IF v_patternmethod = 23 OR v_patternmethod = 25 THEN 			
		...
		ELSIF v_count_2 > v_count THEN
			INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message)
			VALUES (v_fprocesscat_id, v_result_id, 2, concat('WARNING: According with the pattern method used, there is/are ', v_count_2, 
		...
		IF v_patternmethod = 23 THEN  -- check for unitary patterns
			...
			IF v_count > 0 THEN
				...
				INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
				VALUES (v_fprocesscat_id, v_result_id, 3, concat('SELECT * FROM audit_check_data WHERE fprocesscat_id=61 AND user_name=current_user'));		
			...
			ELSE 
				...
				INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
				VALUES (v_fprocesscat_id, v_result_id, 1, concat('INFO: According with the pattern method, all patterns related to dma''s on this exportation are unitary patterns.'));
		...
		ELSIF v_patternmethod = 25 THEN -- check for volume patterns
			...
			ELSE
				INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
				VALUES (v_fprocesscat_id, v_result_id, 1, concat('INFO: According with the pattern method, all patterns related to dma''s on this exportation are volume patterns.'));					
			END IF;
		END IF;
	...
	IF v_patternmethod = 24 OR v_patternmethod = 25 OR v_patternmethod = 26 THEN -- node period, dma interval, dma interval-node period
		...
		IF v_count > 0 THEN
			...
			INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
			VALUES (v_fprocesscat_id, v_result_id, 2, concat('SELECT * FROM audit_check_data WHERE fprocesscat_id=62 AND cur_user=current_user'));		
		...
		ELSE
			INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
			VALUES (v_fprocesscat_id, v_result_id, 1, concat('INFO: According with the pattern method, all patterns related to hydrometer''s on this exportation are unitary patterns.'));
		END IF;
