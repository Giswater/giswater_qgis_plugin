...
BEGIN
	...
	ELSIF p_only_mandatory_nodarc IS FALSE THEN
		v_querytext = 'SELECT a.node_id FROM rpt_inp_node a JOIN inp_valve ON a.node_id=inp_valve.node_id WHERE result_id ='||quote_literal(result_id_var)||' 
				 UNION 
				SELECT a.node_id FROM rpt_inp_node a JOIN inp_pump ON a.node_id=inp_pump.node_id WHERE result_id ='||quote_literal(result_id_var)||' 
				 UNION 
	...
	SELECT * INTO v_node FROM rpt_inp_node WHERE node_id = v_node_id AND result_id=result_id_var;
		...
		IF ST_GeometryType(v_arcreduced_geom) != 'ST_LineString' THEN
			...
			PERFORM audit_function(2022,2316,v_error);
		END IF;
                UPDATE rpt_inp_arc a SET the_geom = v_arcreduced_geom, node_2 = (SELECT concat(v_node_id, '_n2a_1')) WHERE a.arc_id = v_arc2.arc_id AND result_id=result_id_var;
		...
		IF ST_GeometryType(v_arcreduced_geom) != 'ST_LineString' THEN
			...
			PERFORM audit_function(2022,2316,v_error);
		END IF;
                UPDATE rpt_inp_arc a SET the_geom = v_arcreduced_geom, node_1 = (SELECT concat(v_node_id, '_n2a_1')) WHERE a.arc_id = v_arc2.arc_id AND result_id=result_id_var;
		...
		IF ST_GeometryType(v_arcreduced_geom) != 'ST_LineString' THEN
			...
			PERFORM audit_function(2022,2316,v_error);
		END IF;
                UPDATE rpt_inp_arc a SET the_geom = v_arcreduced_geom, node_2 = (SELECT concat(a.node_2, '_n2a_1')) WHERE a.arc_id = v_arc2.arc_id AND result_id=result_id_var;
	...
	v_newarc.epa_type := v_node.epa_type;
        ...
        IF (v_node.addparam::json->>'minorloss') != '' then 
		v_newarc.minorloss := (v_node.addparam->>'minorloss')::numeric;
        END IF;
	IF (v_node.addparam::json->>'diameter') != '' then 
		v_newarc.diameter := (v_node.addparam->>'diameter')::numeric;
        END IF;    
        v_newarc.status := v_node.addparam::json->>'status';
        v_newarc.addparam := gw_fct_json_object_delete_keys(v_node.addparam::json, 'minorloss', 'diameter', 'status');
       
        -- Inserting new arc into arc table
        INSERT INTO rpt_inp_arc (result_id, arc_id, node_1, node_2, arc_type, arccat_id, epa_type, sector_id, state, state_type, diameter, roughness, annotation, length, status, the_geom, minorloss, addparam)
	...
	v_newarc.state, v_newarc.state_type, v_newarc.diameter, v_newarc.roughness, v_newarc.annotation, v_newarc.length, v_status, v_newarc.the_geom, v_newarc.minorloss, v_newarc.addparam);
