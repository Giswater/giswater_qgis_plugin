...
BEGIN
	...
	-- reverse geometries when node acts as node1 from arc but must be node2
	...
	-- reverse geometries when node acts as node2 from arc but must be node1
	EXECUTE 'UPDATE rpt_inp_arc SET the_geom = st_reverse(the_geom) , node_1 = node_2 , node_2 = node_1 WHERE arc_id IN (SELECT arc_id FROM (
		SELECT c.arc_id, n.node_id as node_1, n.to_arc
	...
	EXECUTE 'INSERT INTO rpt_inp_arc (result_id, arc_id, node_1, node_2, arc_type, arccat_id, epa_type, sector_id, expl_id, state, state_type, diameter, roughness, annotation, length, status, the_geom, minorloss, addparam)
			...
			concat (a.addparam::json->>''nodeParent'', ''_n2a_1'') as node_1,
			concat (a.addparam::json->>''nodeParent'', ''_n2a_2'') as node_2,
			''NODE2ARC'', 
			...
			a.annotation,
			st_length2d(st_makeline(a.the_geom, n.the_geom)) as length,
			...
			n.addparam
			FROM 	(SELECT * FROM rpt_inp_node WHERE result_id = '||quote_literal(result_id_var)||' AND node_type = ''NODE2ARC'' AND addparam::json->>''arcPosition''=''1-2'') a
				JOIN rpt_inp_node n ON n.node_id = a.addparam::json->>''nodeParent''
				AND n.result_id = '||quote_literal(result_id_var);


	-- new arcs when numarcs = 2
	EXECUTE 'INSERT INTO rpt_inp_arc (result_id, arc_id, node_1, node_2, arc_type, arccat_id, epa_type, sector_id, expl_id, state, state_type, diameter, roughness, annotation, length, status, the_geom, minorloss, addparam)
			SELECT DISTINCT ON (a.addparam::json->>''nodeParent'')
			a.result_id,
			concat (a.addparam::json->>''nodeParent'', ''_n2a'') as arc_id,
