...
/*example
select SCHEMA_NAME.gw_fct_pg2epa_nod2arc_double ('r1')
*/
...
BEGIN
	...
	v_query_text = 'SELECT rpt_inp_arc.arc_id, rpt_inp_arc.node_1, the_geom FROM rpt_inp_arc
			JOIN inp_pump a ON concat(node_id,''_n2a'')=arc_id
			JOIN inp_curve_id b ON b.id=a.curve_id
			WHERE epa_type=''PUMP'' AND result_id = '||quote_literal(v_result)||' AND isdoublen2a IS TRUE';			

	FOR v_arc_id, v_node_1, v_geom IN EXECUTE v_query_text
	LOOP
		-- New node
		SELECT * INTO v_record FROM rpt_inp_node WHERE node_id = v_node_1 AND result_id=v_result;
		...
		INSERT INTO rpt_inp_node (result_id, node_id, elevation, elev, node_type, nodecat_id, epa_type, sector_id, state, state_type, annotation, demand, the_geom, expl_id, pattern_id)
		...
		-- New arc (PRV)
		SELECT * INTO v_record_a2 FROM rpt_inp_arc WHERE arc_id = v_arc_id AND result_id=v_result;
		v_record_a2.arc_id = concat (v_record_a2.arc_id, '_4');
		v_record_a2.arc_type =  'DOUBLE-NODE2ARC';
		...
		INSERT INTO rpt_inp_arc	(result_id, arc_id, node_1, node_2, arc_type, arccat_id, epa_type, sector_id, state, state_type, annotation, diameter, roughness, length, status, the_geom, expl_id, flw_code, minorloss)
		...
		v_record_a2.annotation, v_record_a2.diameter, v_record_a2.roughness, v_record_a2.length, v_record_a2.status, v_record_a2.the_geom, v_record_a2.expl_id, v_record_a2.flw_code, v_record_a2.minorloss);

		-- New arc (GPV)
		SELECT * INTO v_record_a1 FROM rpt_inp_arc WHERE arc_id = v_arc_id AND result_id=v_result;
		v_record_a1.arc_id = concat (v_record_a1.arc_id, '_5');
		v_record_a1.arc_type =  'DOUBLE-NODE2ARC';
		...
		v_record_a1.annotation, v_record_a1.diameter, v_record_a1.roughness, v_record_a1.length, v_record_a1.status, v_record_a1.the_geom, v_record_a1.expl_id, v_record_a1.flw_code, v_record_a1.minorloss);

		-- Deleting old arc
		DELETE FROM rpt_inp_arc WHERE arc_id = v_arc_id AND result_id=v_result;
