...
BEGIN
    ...
    -- Reset values
    DELETE FROM anl_review_node WHERE cur_user="current_user"() AND context='Topological consistency';

    
    -- Computing process
    INSERT INTO anl_review_node (node_id, node_type, state, num_arcs, expl_id, context, the_geom)
	
    SELECT node_id, nodetype_id, node.state, COUNT(*), node.expl_id, 'Topological consistency'::text, node.the_geom FROM node INNER JOIN arc ON arc.node_1 = node.node_id OR arc.node_2 = node.node_id 
    ...
    UNION
    SELECT node_id, node_type, node.state, COUNT(*),  node.expl_id, 'Topological consistency'::text, node.the_geom FROM node INNER JOIN arc ON arc.node_1 = node.node_id OR arc.node_2 = node.node_id 
