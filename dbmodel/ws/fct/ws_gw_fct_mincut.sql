    ...
    IF type_element_arg = 'arc' OR type_element_arg='ARC' THEN
		...
		ELSIF v_mincutversion = 3 THEN
			...
			ELSE
				...
				SELECT COUNT(*) INTO controlValue FROM config_mincut_inlet WHERE active IS TRUE AND node_id = node_1_aux;
			...
			ELSE
				-- Check if extreme if being a inlet
				SELECT COUNT(*) INTO controlValue FROM config_mincut_inlet WHERE active IS TRUE AND node_id = node_2_aux;
				IF controlValue = 0 THEN
					-- Compute the tributary area using DFS
