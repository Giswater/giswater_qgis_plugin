...
BEGIN
	...
	FROM (
		SELECT  vnode_id, arc_id, locate, vnode_elevation
			...
			FROM (
				SELECT node_1 as vnode_id, arc_id,  0 as locate, null::numeric as vnode_elevation FROM temp_t_arc WHERE arc_type NOT IN ('NODE2ARC', 'LINK')
				)z
		UNION
			SELECT  vnode_id, arc_id, locate, elevation
			FROM (
			SELECT node_2 as vnode_id, arc_id,  1 as locate, null::numeric as elevation FROM temp_t_arc WHERE arc_type NOT IN ('NODE2ARC', 'LINK') 
				)z
		UNION	
	...
	WHERE node_type = 'CONNEC' AND st_dwithin ( arc.the_geom, temp_t_node.the_geom, 0.01) AND c.connec_id = temp_t_node.node_id;
	DELETE FROM temp_t_node WHERE epa_type = 'TODELETE';

	RAISE NOTICE '4 - new nodes from links on temp_t_node table ';
	INSERT INTO temp_t_node (node_id, elevation, elev, node_type, nodecat_id, epa_type, sector_id, state, state_type, annotation, the_geom, addparam, dma_id, presszone_id, dqa_id, minsector_id)
	SELECT 
		vnode_id as node_id, 
	...
	RAISE NOTICE '5 - new nodes from connecs over arc on temp_t_node table ';
	INSERT INTO temp_t_node (node_id, elevation, elev, node_type, nodecat_id, epa_type, sector_id, state, state_type, annotation, the_geom, addparam, dma_id, presszone_id, dqa_id, minsector_id)
	SELECT 
		vnode_id as node_id, 
		t.elevation::numeric(12,3), -- elevation it's interpolated elevation against node1 and node2 of pipe
	...
	RAISE NOTICE '7 - new arcs on temp_t_arc table';
	...
	RAISE NOTICE '8 - delete only trimmed arc on temp_t_arc table';
	-- step 1
