...
BEGIN
	...
	LOOP 
		...
		v_count2 = (SELECT count(*) FROM  temp_node);

		RAISE NOTICE 'Starting pg2epa vnode trim arcs - % (%)',v_offset , v_count2;
		...
		EXIT WHEN v_count2 < v_offset + v_limit;

	END LOOP;
	
	RAISE NOTICE 'delete only trimmed arc on temp_arc table  - step 1';
	UPDATE temp_arc SET epa_type ='PIPENOTUSED' 
		FROM (SELECT DISTINCT arcparent AS arc_id FROM temp_arc WHERE arcparent !='') a
		WHERE a.arc_id = temp_arc.arc_id;

	RAISE NOTICE 'delete only trimmed arc on temp_arc table - step 2';
	DELETE FROM temp_arc WHERE epa_type ='PIPENOTUSED';
	...
	DELETE FROM temp_node a USING temp_node b WHERE a.id < b.id AND a.node_id = b.node_id;
