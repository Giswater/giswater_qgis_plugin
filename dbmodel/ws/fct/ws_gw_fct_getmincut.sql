...
BEGIN
	...
	v_x := (p_data ->> 'data')::json->> 'xcoord';
	v_y := (p_data ->> 'data')::json->> 'ycoord';
	v_srid := (p_data ->> 'data')::json->> 'srid';
    ...
    IF v_feature_type='arc' THEN 
        v_mincut_class=1;
    ELSIF v_feature_type='node' 
        THEN v_mincut_class=1;
    ELSIF v_feature_type='connec' 
        THEN v_mincut_class=2;
    ELSIF v_feature_type='hydrometer' 
    ...
    IF mincut_id_arg IS NOT NULL THEN
        ...
        EXECUTE 'SELECT 
            ...
            exec_depth,
            exec_appropiate                
            FROM om_mincut LEFT JOIN ext_municipality USING (muni_id) WHERE id = $1'
        INTO mincut_data
        USING mincut_id_arg; 
		...
		-- Mincut fields
        json_array[0] := gw_fct_createwidgetjson('mincut_id', 'mincut_id', 'text', 'text', '', TRUE, mincut_data.mincut_id::TEXT);
        json_array[1] := gw_fct_createwidgetjson('WorkOrder', 'work_order', 'text', 'text', '', v_state0, mincut_data.work_order);
        json_array[2] := gw_fct_createcombojson('State', 'mincut_state', 'combo', 'string', '', TRUE, '(SELECT * FROM om_typevalue WHERE typevalue = ''mincut_state'')b','id','idval',mincut_data.mincut_state);
        json_array[3] := gw_fct_createcombojson('Type', 'mincut_type', 'combo', 'string', '', v_state0, 'om_mincut_cat_type','id','id',mincut_data.mincut_type);
        json_array[4] := gw_fct_createcombojson('Cause', 'anl_cause', 'combo', 'string', '', v_state0, '(SELECT * FROM om_typevalue WHERE typevalue = ''mincut_cause'')b','id','idval',mincut_data.anl_cause);
        ...
        json_array[6] := gw_fct_createwidgetjson('Description', 'anl_descript', 'textarea', 'text', '', v_state0, mincut_data.anl_descript);
        ...
        json_array[11] := gw_fct_createwidgetjson('Iniciar procediment de tall', 'gw_fct_setmincut_start', 'button', 'text', '', FALSE,'');
        ...
        json_array[25] := gw_fct_createwidgetjson('Municipality', 'muni_id', 'text', 'text', '', TRUE, mincut_data.muni_id);
        json_array[26] := gw_fct_createwidgetjson('Postcode', 'postcode', 'text', 'text', '', TRUE, mincut_data.postcode);
        json_array[27] := gw_fct_createwidgetjson('Street axis', 'streetaxis_id', 'text', 'text', '', TRUE, mincut_data.streetaxis_id);
        json_array[28] := gw_fct_createwidgetjson('Postnumber', 'postnumber', 'text', 'text', '', TRUE, mincut_data.postnumber);
    ...
    ELSE
        ...
        point_geom := ST_SetSRID(ST_Point(v_x, v_y),v_srid);
        ...
        json_array[2] := gw_fct_createcombojson('State', 'mincut_state', 'combo', 'string', '', v_state0, '(SELECT * FROM om_typevalue WHERE typevalue = ''mincut_state'')b','id','idval',v_mincut_new_state::TEXT);
        json_array[3] := gw_fct_createcombojson('Type', 'mincut_type', 'combo', 'string', '', v_state0, 'om_mincut_cat_type','id','id',v_mincut_new_type::TEXT);
        json_array[4] := gw_fct_createcombojson('Cause', 'anl_cause', 'combo', 'string', '', v_state0, '(SELECT * FROM om_typevalue WHERE typevalue = ''mincut_cause'')b','id','idval',v_mincut_new_cause);
