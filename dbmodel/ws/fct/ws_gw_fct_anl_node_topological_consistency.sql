...
BEGIN
    ...
    SELECT node_id, cat_node.nodetype_id, node.state, COUNT(*), node.expl_id, 'Node topological consistency'::text, node.the_geom FROM node INNER JOIN arc ON arc.node_1 = node.node_id OR arc.node_2 = node.node_id 
