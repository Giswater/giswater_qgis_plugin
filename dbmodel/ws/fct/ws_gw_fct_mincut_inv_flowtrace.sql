			...
			*/
			...
			IF rec_result IS NOT NULL THEN
				...
				IF (select value::boolean from config_param_system where parameter='om_mincut_valve2tank_traceability') IS TRUE THEN 
					...
					LOOP 
						INSERT INTO audit_log_data(fprocesscat_id, feature_id, log_message, cur_user) VALUES (29, rec_result.edge, concat(result_id_arg, ':', rec_valve.node_id,'-',rec_tank.node_id), current_user);
					END LOOP;			
				END IF;
