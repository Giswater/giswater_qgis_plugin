...
BEGIN
	...
	ELSE 
		...
		IF v_netscenario_type = 'DMA' THEN
			...
			SELECT v_scenarioid, dma_id, name, pattern_id, graphconfig, the_geom FROM dma WHERE expl_id = v_expl_id
		...
		ELSIF v_netscenario_type = 'PRESSZONE' THEN
			...
			SELECT v_scenarioid, presszone_id, name, head, graphconfig, the_geom FROM presszone WHERE expl_id = v_expl_id
