...
BEGIN
	...
	IF TG_OP = 'INSERT' THEN
		...
		IF v_table = 'DMA' THEN
			...
			VALUES (NEW.netscenario_id, NEW.dma_id, NEW.name, NEW.pattern_id, NEW.graphconfig, NEW.the_geom, NEW.active) ON CONFLICT (netscenario_id, dma_id) DO NOTHING;
		...
		ELSIF v_table = 'PRESSZONE' THEN
			...
			INSERT INTO plan_netscenario_presszone (netscenario_id, presszone_id, presszone_name, head, graphconfig, the_geom, active, presszone_type)
			VALUES (NEW.netscenario_id, NEW.presszone_id, NEW.name, NEW.head, NEW.graphconfig, NEW.the_geom, NEW.active, NEW.presszone_type) ON CONFLICT (netscenario_id, presszone_id) DO NOTHING;
	...
	ELSIF TG_OP = 'UPDATE' THEN
   	IF v_table = 'DMA' THEN
		...
		ELSIF v_table = 'PRESSZONE' THEN
			...
			SET presszone_id=NEW.presszone_id, presszone_name=NEW.name,  the_geom=NEW.the_geom, head=NEW.head, graphconfig=NEW.graphconfig::json, active=NEW.active, presszone_type=NEW.presszone_type
