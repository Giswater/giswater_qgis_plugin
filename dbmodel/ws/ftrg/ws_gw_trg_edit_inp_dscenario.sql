 ...
 v_dscenario_type = TG_ARGV[0];
  ...
  IF TG_OP = 'INSERT' THEN
		...
		ELSIF v_dscenario_type = 'RESERVOIR' THEN
			...
			-- default values
			IF NEW.pattern_id_id IS NULL OR NEW.pattern_id_id='' THEN NEW.pattern_id_id = (SELECT pattern_id FROM v_edit_inp_reservoir WHERE node_id = NEW.node_id);END IF;
			...
			VALUES (NEW.dscenario_id, NEW.node_id, NEW.pattern_id_id, NEW.head, NEW.init_quality, NEW.source_type, NEW.source_quality, NEW.source_pattern_id);
		...
		ELSIF v_dscenario_type = 'PUMP' THEN
			...
			IF NEW.pattern_id_id IS NULL OR NEW.pattern_id_id='' THEN NEW.pattern_id_id = (SELECT pattern_id FROM v_edit_inp_pump WHERE node_id = NEW.node_id);END IF;
			...
			VALUES (NEW.dscenario_id, NEW.node_id, NEW.power, NEW.curve_id, NEW.speed, NEW.pattern_id_id, NEW.status, NEW.effic_curve_id, NEW.energy_price, NEW.energy_pattern_id);
		...
		ELSIF v_dscenario_type = 'VIRTUALPUMP' THEN
			...
			IF NEW.pattern_id_id IS NULL OR NEW.pattern_id_id='' THEN NEW.pattern_id_id = (SELECT pattern_id FROM v_edit_inp_virtualpump WHERE arc_id = NEW.arc_id);END IF;
			...
			VALUES (NEW.dscenario_id, NEW.arc_id, NEW.power, NEW.curve_id, NEW.speed, NEW.pattern_id_id, NEW.status, NEW.effic_curve_id, NEW.energy_price, NEW.energy_pattern_id);
		...
		ELSIF v_dscenario_type = 'JUNCTION' THEN
			...
			IF NEW.pattern_id_id IS NULL OR NEW.pattern_id_id='' THEN NEW.pattern_id_id = (SELECT pattern_id FROM v_edit_inp_junction WHERE node_id = NEW.node_id);END IF;
			...
			VALUES (NEW.dscenario_id, NEW.node_id, NEW.demand, NEW.pattern_id_id, NEW.init_quality, NEW.source_type, NEW.source_quality, NEW.source_pattern_id);
		...
		ELSIF v_dscenario_type = 'CONNEC' THEN
			...
			IF NEW.pattern_id_id IS NULL OR NEW.pattern_id_id='' THEN NEW.pattern_id_id = (SELECT pattern_id FROM v_edit_inp_connec WHERE connec_id = NEW.connec_id);END IF;
			...
			VALUES (NEW.dscenario_id, NEW.connec_id, NEW.demand, NEW.pattern_id_id, NEW.custom_roughness, NEW.custom_length, NEW.custom_dint
		...
		ELSIF v_dscenario_type = 'INLET' THEN
			...
			IF NEW.pattern_id_id IS NULL OR NEW.pattern_id_id='' THEN NEW.pattern_id_id = (SELECT pattern_id FROM v_edit_inp_inlet WHERE node_id = NEW.node_id);END IF;
			...
			mixing_model, mixing_fraction, reaction_coeff, init_quality, source_type, source_quality, source_pattern_id)
			VALUES (NEW.dscenario_id, NEW.node_id, NEW.initlevel, NEW.minlevel, NEW.maxlevel, NEW.diameter, NEW.minvol, NEW.curve_id, NEW.overflow, NEW.pattern_id_id, NEW.head,
			NEW.mixing_model, NEW.mixing_fraction, NEW.reaction_coeff, NEW.init_quality, NEW.source_type, NEW.source_quality, NEW.source_pattern_id);
		...
		ELSIF v_dscenario_type = 'PUMP_ADDITIONAL' THEN
			...
			IF NEW.pattern_id_id IS NULL OR NEW.pattern_id_id='' THEN NEW.pattern_id_id = (SELECT pattern_id FROM v_edit_inp_pump_additional WHERE node_id = NEW.node_id);END IF;
			...
			VALUES (NEW.dscenario_id, NEW.node_id, NEW.order_id, NEW.power, NEW.curve_id, NEW.speed, NEW.pattern_id_id, NEW.status, NEW.effic_curve_id, NEW.energy_price, NEW.energy_pattern_id);
	...
	ELSIF TG_OP = 'UPDATE' THEN
		...
		ELSIF v_dscenario_type = 'RESERVOIR' THEN
			UPDATE inp_dscenario_reservoir SET dscenario_id=NEW.dscenario_id, node_id=NEW.node_id, pattern_id=NEW.pattern_id_id, head=NEW.head,
		...
		ELSIF v_dscenario_type = 'JUNCTION' THEN
			UPDATE inp_dscenario_junction SET dscenario_id=NEW.dscenario_id, node_id=NEW.node_id, demand=NEW.demand, pattern_id=NEW.pattern_id_id,
		...
		ELSIF v_dscenario_type = 'CONNEC' THEN
			...
			demand=NEW.demand, pattern_id=NEW.pattern_id_id, demand_type=NEW.demand_type, peak_factor=NEW.peak_factor,
		...
		ELSIF v_dscenario_type = 'INLET' THEN
			...
			maxlevel=NEW.maxlevel, diameter=NEW.diameter, minvol=NEW.minvol, curve_id=NEW.curve_id, overflow = NEW.overflow, pattern_id = NEW.pattern_id_id, head = NEW.head,
			mixing_model=NEW.mixing_model, mixing_fraction=NEW.mixing_fraction, reaction_coeff=NEW.reaction_coeff,  init_quality=NEW.init_quality, source_type=NEW.source_type,
			source_quality=NEW.source_quality, source_pattern_id=NEW.source_pattern_id
			WHERE dscenario_id=OLD.dscenario_id AND node_id=OLD.node_id;
