...
BEGIN
    ...
    ELSIF TG_OP = 'UPDATE' THEN
        ...
        ELSIF v_epatype = 'valve' THEN
			...
			init_quality=NEW.init_quality, head = NEW.head, pattern_id = NEW.pattern_id, demand = NEW.demand, demand_pattern_id = NEW.demand_pattern_id, emitter_coeff = NEW.emitter_coeff WHERE node_id=OLD.node_id;
        ...
        ELSIF v_epatype = 'shortpipe' THEN
			UPDATE inp_shortpipe SET minorloss=NEW.minorloss, bulk_coeff = NEW.bulk_coeff, wall_coeff = NEW.wall_coeff, custom_dint=NEW.custom_dint,
			head = NEW.head, pattern_id = NEW.pattern_id, demand = NEW.demand, demand_pattern_id = NEW.demand_pattern_id, emitter_coeff = NEW.emitter_coeff WHERE node_id=OLD.node_id;
