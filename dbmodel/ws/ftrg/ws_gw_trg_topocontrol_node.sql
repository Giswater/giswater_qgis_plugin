...
BEGIN
	...
	IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE ' THEN
		...
		IF (NEW.state=2 AND node_rec.node_id IS NOT NULL) THEN
			...
			FOR v_arc IN (SELECT arc_id, node_1 as node_id FROM arc WHERE node_1=node_rec.node_id AND state >0 
			UNION SELECT arc_id, node_2 FROM arc WHERE node_2=node_rec.node_id AND state >0)		
			LOOP
