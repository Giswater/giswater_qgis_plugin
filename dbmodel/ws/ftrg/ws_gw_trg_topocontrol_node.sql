	...
	IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE ' THEN
		...
		ELSIF (NEW.state=2) AND (v_node_proximity_control IS TRUE) THEN
			...
			IF node_rec.node_id IS NOT NULL THEN
				...
				ELSE
					INSERT INTO audit_log_data (fprocesscat_id, feature_id, log_message) VALUES (4, NEW.node_id, 'Node with state 2 over another node with state=2 on same alternative it is not allowed');
				END IF;
			END IF;		
		END IF;
		...
		IF (NEW.state=2) THEN	
			...
			-- check for existing nodes (1 and 2)
			FOR node_rec IN SELECT * FROM node JOIN plan_psector_x_node USING (node_id) WHERE ST_DWithin(NEW.the_geom, node.the_geom, v_node_proximity) AND node.node_id != NEW.node_id AND node.state>0
			LOOP 
