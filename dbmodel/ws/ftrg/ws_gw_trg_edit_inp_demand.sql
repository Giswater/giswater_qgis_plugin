...
BEGIN
	...
	IF TG_OP = 'INSERT' THEN
		...
		NEW.id=nextval('SCHEMA_NAME.inp_dscenario_demand_id_seq'::regclass);
	
		INSERT INTO inp_dscenario_demand (id, feature_id, demand, pattern_id, demand_type, dscenario_id, feature_type) 
		VALUES (NEW.id,NEW.feature_id, NEW.demand, NEW.pattern_id, NEW.demand_type, NEW.dscenario_id, NEW.feature_type);
		RETURN NEW;

	ELSIF TG_OP = 'UPDATE' THEN
		UPDATE inp_dscenario_demand SET id=NEW.id, feature_id=NEW.feature_id, demand=NEW.demand, pattern_id=NEW.pattern_id, 
