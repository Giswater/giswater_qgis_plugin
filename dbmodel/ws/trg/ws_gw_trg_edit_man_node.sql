...
-- FEATURE INSERT      
		...
		VALUES (NEW.node_id, NEW.code, NEW.elevation, NEW.depth, NEW.nodecat_id, NEW.epa_type, NEW.sector_id, NEW.state, NEW.state_type, NEW.annotation, NEW.observ, NEW.comment,NEW.dma_id, NEW.presszonecat_id,
		...
		NEW.streetaxis_id, NEW.streetaxis2_id, NEW.postcode,NEW.postnumber,NEW.postnumber2, NEW.descript, NEW.rotation, NEW.verified, NEW.undelete,NEW.label_x,NEW.label_y,NEW.label_rotation, 
	...
	ELSIF man_table='man_valve' THEN	
		...
		NEW.exit_type, NEW.exit_code, NEW.drive_type, NEW.cat_valve2, NEW.arc_id);
...
-- MANAGEMENT UPDATE
	...
	ELSIF man_table ='man_pump' THEN
		...
		UPDATE man_pump 
		SET max_flow=NEW.max_flow, min_flow=NEW.min_flow, nom_flow=NEW.nom_flow, "power"=NEW.power, pressure=NEW.pressure, elev_height=NEW.elev_height, name=NEW.name
		WHERE node_id=OLD.node_id;
	...
	ELSIF man_table ='man_hydrant' THEN
		...
		UPDATE man_hydrant 
		SET fire_code=NEW.fire_code, communication=NEW.communication, valve=NEW.valve, valve_diam=NEW.valve_diam
		WHERE node_id=OLD.node_id;			
	...
	ELSIF man_table ='man_valve' THEN
		...
		exit_type=NEW.exit_type, exit_code=NEW.exit_code, drive_type=NEW.drive_type, cat_valve2=NEW.cat_valve2, arc_id=NEW.arc_id
		WHERE node_id=OLD.node_id;	
	...
	ELSIF man_table ='man_register_pol' THEN
		...
		ELSE
				...
				UPDATE polygon 
				SET the_geom=NEW.the_geom,pol_id=NEW.pol_id
				WHERE pol_id=OLD.pol_id;
		END IF;
