...
EXECUTE 'SET search_path TO '||quote_literal(TG_TABLE_SCHEMA)||', public';
      ...
      IF TG_OP = 'INSERT' THEN
		IF NEW.field_checked=TRUE THEN
			...
			INSERT INTO review_audit_connec (connec_id, the_geom, elevation, "depth", connec_type, connecat_id, annotation, verified, field_checked,"operation","user",date_field, office_checked) 
			VALUES(NEW.connec_id, NEW.the_geom, NEW.elevation, NEW."depth", NEW.connec_type, NEW.connecat_id, NEW.annotation, 'REVISED', NEW.field_checked,'UPDATE',user,CURRENT_TIMESTAMP, NEW.office_checked);
      ...
      ELSIF TG_OP = 'UPDATE' THEN
		...
		ELSE
				IF NEW.the_geom=OLD.the_geom THEN
					...
					VALUES (NEW.connec_id, NEW.the_geom, NEW.elevation, NEW."depth", NEW.connec_type, NEW.connecat_id, NEW.annotation, 'REVISED', NEW.field_checked,'INSERT', user, CURRENT_TIMESTAMP, NEW.office_checked,'FALSE');
