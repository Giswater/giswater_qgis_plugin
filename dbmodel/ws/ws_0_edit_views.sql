...
-- EPA INSERT	
			...
			ELSIF (NEW.epa_type = 'PUMP') THEN INSERT INTO  inp_pump VALUES(NEW.node_id,null,null,null,null,null);
			ELSIF (NEW.epa_type = 'VALVE') THEN INSERT INTO  inp_valve VALUES(NEW.node_id,null,null,null,null,null,null,null,null);
			ELSIF (NEW.epa_type = 'SHORT_PIPE') THEN INSERT INTO  inp_shortpipe VALUES(NEW.node_id,null,null,null);
			END IF;
...
-- MANAGEMENT INSERT
			...
			IF (querystring='man_junction') THEN INSERT INTO  man_junction VALUES(NEW.node_id,null);
			ELSIF (querystring='man_tank') THEN INSERT INTO  man_tank VALUES(NEW.node_id,null,null,null);
			ELSIF (querystring='man_hdyrant') THEN INSERT INTO  man_hdyrant VALUES(NEW.node_id,null);
			ELSIF (querystring='man_valve') THEN INSERT INTO  man_valve VALUES(NEW.node_id,null);
			ELSIF (querystring='man_pump') THEN INSERT INTO  man_pump VALUES(NEW.node_id,null);
			ELSIF (querystring='man_filter') THEN INSERT INTO  man_filter VALUES(NEW.node_id,null);
			ELSIF (querystring='man_meter') THEN INSERT INTO  man_meter VALUES(NEW.node_id,null);
			END IF;
	RETURN NEW;
    ...
    ELSIF TG_OP = 'UPDATE' THEN
			...
			UPDATE 	node SET node_id=NEW.node_id, elevation=NEW.elevation, "depth"=NEW."depth", nodecat_id=NEW.nodecat_id, epa_type=NEW.epa_type, sector_id=NEW.sector_id, "state"=NEW."state", annotation=NEW.annotation, "observ"=NEW."observ", 
...
-- MAN INSERT
	...
	ELSIF TG_OP = 'UPDATE' THEN
		...
		UPDATE arc 	SET arc_id=NEW.arc_id, arccat_id=NEW.arccat_id, epa_type=NEW.epa_type, sector_id=NEW.sector_id, "state"=NEW."state", annotation= NEW.annotation, "observ"=NEW."observ", "comment"=NEW."comment", custom_length=NEW.custom_length, 
						dma_id=NEW.dma_id, soilcat_id=NEW.soilcat_id, category_type=NEW.category_type, fluid_type=NEW.fluid_type, location_type=NEW.location_type, workcat_id=NEW.workcat_id, buildercat_id=NEW.buildercat_id, builtdate=NEW.builtdate,
						ownercat_id=NEW.ownercat_id, adress_01=NEW.adress_01, adress_02=NEW.adress_02, adress_03=NEW.adress_03, descript=NEW.descript,
						rotation=NEW.rotation, link=NEW.link, verified=NEW.verified, the_geom=NEW.the_geom WHERE arc_id=OLD.arc_id;
		RETURN NEW;
