...
CREATE OR REPLACE FUNCTION am.gw_fct_admin_sys_version_register(p_data json)
RETURNS json AS
$BODY$
DECLARE
	v_gwversion text;
...
BEGIN
	...
	END IF;
	INSERT INTO am.sys_version (giswater, project_type, postgres, postgis, language, epsg, addparam)
	VALUES (v_gwversion, upper(COALESCE(v_projecttype, 'AM')), version(), postgis_version(), v_language, v_epsg, v_addparam);
