...
CREATE OR REPLACE FUNCTION cibs.gw_fct_admin_sys_version_register(p_data json)
RETURNS json AS
$BODY$
DECLARE
	v_gwversion text;
...
BEGIN
	...
	SELECT * INTO v_prev FROM cibs.sys_version ORDER BY id DESC LIMIT 1;
	...
	END IF;
	INSERT INTO cibs.sys_version (giswater, project_type, postgres, postgis, language, epsg, addparam)
	VALUES (v_gwversion, upper(COALESCE(v_projecttype, 'CIBS')), version(), postgis_version(), v_language, v_epsg, v_addparam);
