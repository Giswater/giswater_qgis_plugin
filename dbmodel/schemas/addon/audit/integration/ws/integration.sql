...
INSERT INTO config_report
(id, alias, query_text, addparam, filterparam, sys_role, descript, active, device)
VALUES(903, 'Check health WS', 'SELECT * FROM audit.v_fidlog_index WHERE schema = ''PARENT_SCHEMA''', '{"orderBy":"1", "orderType": "DESC"}'::json, NULL, 'role_plan', NULL, true, '{4}') ON CONFLICT (id) DO NOTHING;
...
"widgettype":"combo","datatype":"text","layoutorder":2, "dvquerytext":"Select distinct on (type) type AS id, type AS idval FROM audit.v_fidlog_aux WHERE schema = ''PARENT_SCHEMA'' ORDER BY type DESC"}]'::json, 'role_plan', NULL, true, '{4}') ON CONFLICT (id) DO NOTHING;
