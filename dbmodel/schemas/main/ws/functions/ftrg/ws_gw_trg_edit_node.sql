...
BEGIN
	...
	IF TG_OP = 'INSERT' THEN
		...
		IF v_man_table='man_tank' THEN
			...
			VALUES (NEW.node_id, NEW.vmax, NEW.vutil, NEW.area,NEW.chlorination, NEW.name, NEW.hmax, NEW.automated, NEW.length, NEW.width, COALESCE(NEW.shape, 0), NEW.fence_type, NEW.fence_length, NEW.inlet_arc, NEW.invert_level);
	...
	-- static pressure
	  ...
	  IF (NEW.nodecat_id != OLD.nodecat_id) THEN
		...
		ELSIF v_man_table ='man_tank' THEN
			...
			hmax=NEW.hmax, automated=NEW.automated, length=NEW.length, width=NEW.width, shape=COALESCE(NEW.shape, 0), fence_type=NEW.fence_type, fence_length=NEW.fence_length, inlet_arc=NEW.inlet_arc
			WHERE node_id=OLD.node_id;
