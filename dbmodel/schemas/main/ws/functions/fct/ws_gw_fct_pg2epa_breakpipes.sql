...
BEGIN
	...
	PERFORM setval('SCHEMA_NAME.temp_table_id_seq', (SELECT max(vnode_id) FROM temp_t_link), true);
	...
	LOOP
		...
		LOOP
			...
			INSERT INTO temp_t_table(fid, geom_point)
	...
	UPDATE temp_t_table a SET fid = 296 FROM temp_t_link WHERE st_dwithin(st_endpoint(the_geom), geom_point, v_removevnodebuffer);
