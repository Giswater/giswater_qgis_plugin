    ...
    WHILE (array_length(stack, 1) > 0) LOOP
        ...
        IF node_id_arg IS NULL THEN
          EXECUTE 'SELECT gw_fct_getmessage($${"client":{"device":4, "infoType":1, "lang":"ES"},"feature":{},
            "data":{"message":"3006", "function":"2306","parameters":null, "is_process":true}}$$);';
        END IF;

        -- Get node public.geometry
        SELECT the_geom, node_type INTO node_aux, v_node_type FROM ve_node WHERE (node_id = node_id_arg) AND (is_operative IS TRUE);

        -- Check node_id being a valve
        SELECT node_id INTO exists_id FROM om_mincut_valve WHERE node_id = node_id_arg 
        AND result_id=result_id_arg AND ((unaccess = FALSE AND broken = FALSE) OR (closed = TRUE));
        
        IF FOUND THEN
            UPDATE om_mincut_valve SET proposed = TRUE WHERE node_id=node_id_arg AND result_id=result_id_arg;
	    ...
	    SELECT COUNT(*) INTO controlValue FROM config_graph_mincut WHERE active IS TRUE AND  node_id = node_id_arg;
		...
		END IF;
                ...
                FOR rec_table IN SELECT * FROM ve_arc 
