...
AS WITH plan_netscenario_current AS (
 ...
 SELECT n.netscenario_id,
   ...
   FROM plan_netscenario_node n
     ...
     JOIN plan_netscenario_current pnc ON n.netscenario_id = pnc.netscenario_id
  WHERE (EXISTS (SELECT 1 FROM vf_exploitation vfe WHERE vfe.expl_id = p.expl_id)) AND (EXISTS ( SELECT 1
           FROM sel_expl
          WHERE sel_expl.expl_id = p.expl_id));
...
AS WITH sel_expl AS (
 ...
 SELECT DISTINCT ON (netscenario_id) netscenario_id,
  ...
  WHERE (EXISTS (SELECT 1 FROM vf_exploitation vfe WHERE vfe.expl_id = plan_netscenario.expl_id)) AND (EXISTS ( SELECT 1
           FROM sel_expl
          WHERE sel_expl.expl_id = plan_netscenario.expl_id));
...
AS WITH plan_netscenario_current AS (
 ...
 SELECT v.netscenario_id,
  ...
  WHERE (EXISTS (SELECT 1 FROM vf_exploitation vfe WHERE vfe.expl_id = n.expl_id)) AND (EXISTS ( SELECT 1
           FROM sel_expl
          WHERE sel_expl.expl_id = n.expl_id));
