...
CREATE OR REPLACE FUNCTION SCHEMA_NAME.gw_fct_admin_sys_version_register(p_data json)
RETURNS json AS
$BODY$
...
BEGIN
	...
	SELECT * INTO v_prev FROM sys_version ORDER BY id DESC LIMIT 1;
	...
	IF v_isnew IS NOT TRUE AND v_prev IS NOT NULL THEN
		...
		v_creation_profile := COALESCE(
			v_creation_profile,
			v_prev.addparam -> 'environment' ->> 'creation_profile'
		);
	...
	IF v_merge IS NOT NULL AND v_merge <> '{}'::jsonb THEN
		IF v_merge ? 'satellites' THEN
			v_satellites := COALESCE(v_addparam -> 'satellites', '{}'::jsonb)
				|| (v_merge -> 'satellites');
	...
	ELSIF v_parent_schema IS NOT NULL THEN
		v_parent_arr := COALESCE(v_addparam -> 'parent_schemas', '[]'::jsonb);
		v_parent_arr := v_parent_arr || to_jsonb(v_parent_schema);
		v_addparam := jsonb_set(v_addparam, '{parent_schemas}', (
			SELECT COALESCE(jsonb_agg(DISTINCT elem), '[]'::jsonb)
			FROM jsonb_array_elements_text(v_parent_arr) AS elem
		), true);
	...
	INSERT INTO sys_version (giswater, project_type, postgres, postgis, language, epsg, addparam)
	VALUES (
		v_gwversion,
		upper(COALESCE(v_projecttype, v_prev.project_type, 'WS')),
		version(),
		postgis_version(),
		v_language,
		v_epsg,
