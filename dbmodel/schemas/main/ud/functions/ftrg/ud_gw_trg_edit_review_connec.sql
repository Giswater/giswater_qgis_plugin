...
--get value from edit_review_auto_field_checked
	...
	SELECT connec_id, y1, y2, connec.connec_type, conneccat_id, connec.matcat_id, annotation, observ, expl_id, the_geom INTO rec_connec
    ...
    IF TG_OP = 'INSERT' THEN
		...
		IF (NEW.expl_id IS NULL) THEN
			...
			IF (NEW.expl_id IS NULL) THEN
				...
				IF (NEW.expl_id IS NULL) THEN
					...
					"data":{"message":"2012", "function":"2474","parameters":{"feature_id":"'||NEW.connec_id::text||'"}}}$$);';
				END IF;
			END IF;
		END IF;
		...
		INSERT INTO review_connec (connec_id, y1, y2, connec_type, matcat_id, conneccat_id, annotation, observ,
				expl_id, the_geom, field_checked, field_date)
		VALUES (NEW.connec_id, NEW.y1, NEW.y2, NEW.connec_type, NEW.matcat_id, NEW.conneccat_id, NEW.annotation, NEW.observ,
		...
		--looking for insert values on audit table
	  	IF NEW.field_checked=TRUE THEN
				...
				(connec_id, old_y1, new_y1, old_y2, new_y2, old_connec_type, new_connec_type, old_matcat_id, new_matcat_id, old_conneccat_id,
				new_conneccat_id, old_annotation, new_annotation, old_observ, new_observ, review_obs, expl_id, the_geom, review_status_id, field_date, field_user)
				...
				NEW.matcat_id, rec_connec.conneccat_id, NEW.conneccat_id, rec_connec.annotation, NEW.annotation, rec_connec.observ, NEW.observ, NEW.review_obs, NEW.expl_id,
    ...
    ELSIF TG_OP = 'UPDATE' THEN
		...
		UPDATE review_connec SET y1=NEW.y1, y2=NEW.y2, connec_type=NEW.connec_type, matcat_id=NEW.matcat_id, conneccat_id=NEW.conneccat_id,
		...
		IF (NEW.field_checked is TRUE) THEN
			...
			IF EXISTS (SELECT connec_id FROM review_audit_connec WHERE connec_id=NEW.connec_id) THEN
				UPDATE review_audit_connec SET old_y1=rec_connec.y1, new_y1=NEW.y1, old_y2=rec_connec.y2,
       			...
       			new_matcat_id=NEW.matcat_id, old_conneccat_id=rec_connec.conneccat_id, new_conneccat_id=NEW.conneccat_id, old_annotation=rec_connec.annotation,
