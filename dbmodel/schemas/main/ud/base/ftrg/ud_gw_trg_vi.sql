...
BEGIN
    ...
    IF TG_OP = 'INSERT' THEN
		...
		ELSIF v_view='vi_orifices' THEN
			...
			INSERT INTO man_varc (arc_id) VALUES (NEW.arc_id);
			INSERT INTO inp_orifice (arc_id, ori_type, offsetval, cd, flap, orate)
			VALUES (NEW.arc_id, NEW.ori_type, NEW.offsetval, NEW.cd, NEW.flap, NEW.orate);
		...
		ELSIF v_view='vi_timeseries' THEN
			IF NEW.other1 ilike 'FILE' THEN
				IF NEW.timser_id NOT IN (SELECT id FROM inp_timeseries) THEN
					INSERT INTO inp_timeseries(id, idval, times_type,fname) VALUES (NEW.timser_id, NEW.timser_id,'FILE',NEW.other1) ;
				END IF;
			...
			ELSIF  NEW.other1 ilike '%:%' OR NEW.other1 ~ '^\d+$' OR NEW.other1 ilike '%:%' THEN
				...
				ELSE
					INSERT INTO inp_timeseries_value (timser_id, "time", value)  VALUES (NEW.timser_id, NEW.other2, NEW.other3::numeric);
				END IF;
			ELSE
				IF NEW.timser_id NOT IN (SELECT id FROM inp_timeseries) THEN
