...
SET search_path = ud36005 ,public;
...
drop view v_state_link;
drop view v_state_link_gully;
drop view v_state_link_connec;


CREATE MATERIALIZED VIEW v_state_link AS 
(
...
CREATE MATERIALIZED VIEW v_state_link_connec AS 
(
         SELECT DISTINCT link.link_id
           FROM selector_state,
            selector_expl,
            link
          WHERE link.state = selector_state.state_id AND (link.expl_id = selector_expl.expl_id OR link.expl_id2 = selector_expl.expl_id) AND selector_state.cur_user = "current_user"()::text 
          AND selector_expl.cur_user = "current_user"()::text AND link.feature_type::text = 'CONNEC'::text
        EXCEPT ALL
         SELECT plan_psector_x_connec.link_id
           FROM selector_psector,
            selector_expl,
          ...
          AND plan_psector.expl_id = selector_expl.expl_id AND selector_expl.cur_user = CURRENT_USER::text AND plan_psector_x_connec.active IS TRUE
) UNION ALL
 SELECT plan_psector_x_connec.link_id
   FROM selector_psector,
    selector_expl,
...
CREATE MATERIALIZED VIEW v_state_link_gully AS 
(
         SELECT DISTINCT link.link_id
           FROM selector_state,
            selector_expl,
...
DROP VIEW v_state_connec;
DROP VIEW v_state_gully;

CREATE MATERIALIZED VIEW v_state_connec AS
                 ...
                 SELECT plan_psector_x_connec.connec_id,
                   ...
                   FROM selector_psector,
                    ...
                    plan_psector_x_connec
                     JOIN plan_psector ON plan_psector.psector_id = plan_psector_x_connec.psector_id
                  WHERE plan_psector_x_connec.psector_id = selector_psector.psector_id AND selector_psector.cur_user = "current_user"()::text 
                  AND plan_psector_x_connec.state = 0 AND plan_psector.expl_id = selector_expl.expl_id AND selector_expl.cur_user = "current_user"()::text
        ) UNION
         SELECT plan_psector_x_connec.connec_id,
            plan_psector_x_connec.arc_id,
            2 AS flag
           FROM selector_psector,
            ...
            plan_psector_x_connec
             JOIN plan_psector ON plan_psector.psector_id = plan_psector_x_connec.psector_id
          WHERE plan_psector_x_connec.psector_id = selector_psector.psector_id AND selector_psector.cur_user = "current_user"()::text 
          AND plan_psector_x_connec.state = 1 AND plan_psector.expl_id = selector_expl.expl_id AND selector_expl.cur_user = "current_user"()::text
  ORDER BY 1, 3 DESC) a;
...
ALTER TABLE v_state_connec OWNER TO role_basic;
ALTER TABLE v_state_gully OWNER TO role_basic;
...
AS SELECT * FROM vu_arc;

CREATE OR REPLACE VIEW v_node
AS SELECT * FROM vu_node;

DROP VIEW v_state_node;
DROP VIEW v_expl_node;
DROP VIEW v_expl_arc;
DROP VIEW v_state_arc;

CREATE MATERIALIZED VIEW v_state_arc AS 
(         SELECT arc.arc_id
           FROM selector_state,
            arc
...
CREATE MATERIALIZED VIEW v_state_node AS 
(
         SELECT node.node_id
           FROM selector_state,
            node
...
CREATE MATERIALIZED VIEW v_expl_arc AS 
...
CREATE MATERIALIZED VIEW v_expl_node AS 
