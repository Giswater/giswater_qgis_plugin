...
UPDATE sys_fprocess SET fprocess_name='dqa-nodeparent acording with graphdelimiter', "source"='core', fprocess_type='Check graph-data', project_type='ws', except_level=2, except_msg='nodes with cat_feature_node.graph_delimiter=''DQA'' not configured on the dqa table. nodes with cat_feature_node.graph_delimiter=''DQA'' configured for unactive mapzone.', query_text='SELECT node_id, nodecat_id, the_geom, a.active,  v_prefix_node.expl_id FROM v_prefix_node JOIN cat_node c ON id=nodecat_id JOIN cat_feature_node n ON n.id=c.node_type  LEFT JOIN (SELECT node_id, active FROM v_prefix_node JOIN (SELECT (json_array_elements_text((graphconfig::json->>''use'')::json))::json->>''nodeParent'' as node_id, active FROM v_prefix_dqa WHERE graphconfig IS NOT NULL )a USING (node_id)) a USING (node_id) WHERE graph_delimiter=''DQA'' AND (a.node_id IS NULL  OR node_id NOT IN (SELECT (json_array_elements_text((graphconfig::json->>''ignore'')::json))::text FROM v_prefix_dqa WHERE active IS TRUE))  AND v_prefix_node.state > 0 and verified<>''2'' and (a.active is null or a.active is false)', info_msg='All nodes with cat_feature_node.graphdelimiter=''DMA'' are defined as nodeParent on dma.graphconfig', function_name='[gw_fct_graphanalytics_check_data, gw_fct_admin_check_data]' WHERE fid=181;
UPDATE sys_fprocess SET fprocess_name='presszone-nodeparent acording with graphdelimiter', "source"='core', fprocess_type='Check graph-data', project_type='ws', except_level=2, except_msg='nodes with cat_feature_node.graph_delimiter=''PRESSZONE'' not configured on the presszone table. nodes with cat_feature_node.graph_delimiter=''PRESSZONE'' configured for unactive mapzone.', query_text='SELECT node_id, nodecat_id, the_geom, a.active,v_prefix_node.expl_id FROM v_prefix_node JOIN cat_node c ON id=nodecat_id JOIN cat_feature_node n ON n.id=c.node_type  LEFT JOIN (SELECT node_id, active FROM v_prefix_node JOIN (SELECT (json_array_elements_text((graphconfig::json->>''use'')::json))::json->>''nodeParent'' as node_id, active FROM v_prefix_presszone WHERE graphconfig IS NOT NULL )a USING (node_id)) a USING (node_id) WHERE graph_delimiter=''PRESSZONE'' AND (a.node_id IS NULL  OR node_id NOT IN (SELECT (json_array_elements_text((graphconfig::json->>''ignore'')::json))::text FROM v_prefix_presszone WHERE active IS TRUE))  AND v_prefix_node.state > 0 and verified<>''2'' and (a.active is null or a.active is false)', info_msg='All nodes with cat_feature_node.graphdelimiter=''PRESSZONE'' are defined as nodeParent on presszone.graphconfig', function_name='[gw_fct_graphanalytics_check_data, gw_fct_admin_check_data]' WHERE fid=182;
...
UPDATE sys_fprocess SET fprocess_name='Null values on valv_type table', "source"='core', fprocess_type='Check epa-data', project_type='ws', except_level=3, except_msg='valves with null values on valv_type column.', query_text='SELECT * FROM v_edit_inp_valve WHERE valv_type IS NULL', info_msg='Valve valv_type checked. No mandatory values missed.', function_name='[gw_fct_pg2epa_check_data, gw_fct_admin_check_data]' WHERE fid=273;
...
UPDATE sys_fprocess SET fprocess_name='Check fluid_type values exists on man_ table', "source"='core', fprocess_type='Check om-data', project_type='ws', except_level=3, except_msg='features with fluid_type does not exists on man_type_fluid table.', query_text='SELECT ''ARC'', arc_id, fluid_type FROM v_prefix_arc WHERE fluid_type NOT IN (SELECT fluid_type FROM man_type_fluid WHERE feature_type is null or feature_type = ''ARC'' or featurecat_id IS NOT NULL) AND fluid_type IS NOT NULL
UNION
...
INSERT INTO sys_fprocess (fid, fprocess_name, "source", fprocess_type, project_type, except_level, except_msg, query_text, info_msg, function_name) VALUES(533, 'Non-mandatory nodarc with less than two arcs', 'core', 'Check epa-topology', 'ws', 2, 'NON-mandatory node2arcs with less than two arcs. It will be transformed on the fly using only one arc''', 'SELECT * FROM temp_anl_node WHERE fid = 292', 'No results found for NON-mandatory node2arcs with less than two arcs.', '[gw_fct_pg2epa_check_data, gw_fct_admin_check_data]');
...
INSERT INTO sys_fprocess (fid, fprocess_name, "source", fprocess_type, project_type, except_level, except_msg, query_text, info_msg, function_name) VALUES(547, 'Check fluid_type values exists on man_ table', 'core', 'Check om-data', 'ud', 3, 'features with fluid_type does not exists on man_type_fluid table.', 'SELECT ''ARC'', arc_id, fluid_type FROM v_prefix_arc WHERE fluid_type NOT IN (SELECT fluid_type FROM man_type_fluid WHERE feature_type is null or feature_type = ''ARC'' or featurecat_id IS NOT NULL) AND fluid_type IS NOT NULL
UNION
SELECT ''NODE'', node_id, fluid_type FROM v_prefix_node WHERE fluid_type NOT IN (SELECT fluid_type FROM man_type_fluid WHERE feature_type is null or feature_type = ''NODE'' or featurecat_id IS NOT NULL) AND fluid_type IS NOT NULL
UNION
...
INSERT INTO sys_fprocess (fid, fprocess_name, "source", fprocess_type, project_type, except_level, except_msg, query_text, info_msg, function_name) VALUES(576, 'Check flow regulator length fits on destination arc (orifice)', 'core', 'Check epa-data', 'ud', 3, 'orifice flow regulator wich his length do not respect the minimum length for target arc.', 'SELECT nodarc_id, f.the_geom FROM selector_sector s, v_edit_inp_flwreg_orifice f
JOIN node n USING (node_id) JOIN arc a ON a.arc_id = to_arc 
WHERE n.sector_id = s.sector_id 
AND cur_user=current_user AND 
flwreg_length + (SELECT value::numeric FROM config_param_user WHERE parameter = ''inp_options_minlength'' AND cur_user = current_user) > st_length(a.the_geom)', 'All orifice flow regulators has lengh wich fits target arc.', '[gw_fct_pg2epa_check_data]');
INSERT INTO sys_fprocess (fid, fprocess_name, "source", fprocess_type, project_type, except_level, except_msg, query_text, info_msg, function_name) VALUES(577, 'Check flow regulator length fits on destination arc (weir)', 'core', 'Check epa-data', 'ud', 3, 'weir flow regulator length do not respect the minimum length for target arc', 'SELECT nodarc_id, f.the_geom FROM selector_sector s, v_edit_inp_flwreg_weir f
JOIN node n USING (node_id) JOIN arc a ON a.arc_id = to_arc 
WHERE n.sector_id = s.sector_id AND cur_user=current_user AND flwreg_length + 
(SELECT value::numeric FROM config_param_user WHERE parameter = ''inp_options_minlength'' AND cur_user = current_user) > st_length(a.the_geom)', 'All weir flow regulators has lengh wich fits target arc.', '[gw_fct_pg2epa_check_data]');
INSERT INTO sys_fprocess (fid, fprocess_name, "source", fprocess_type, project_type, except_level, except_msg, query_text, info_msg, function_name) VALUES(578, 'Check flow regulator length fits on destination arc (outlet)', 'core', 'Check epa-data', 'ud', 3, 'outlet flow regulator length do not respect the minimum length for target arc', 'SELECT nodarc_id, f.the_geom FROM selector_sector s, v_edit_inp_flwreg_pump f
JOIN node n USING (node_id) JOIN arc a ON a.arc_id = to_arc 
WHERE n.sector_id = s.sector_id AND cur_user=current_user AND flwreg_length + 
(SELECT value::numeric FROM config_param_user WHERE parameter = ''inp_options_minlength'' AND cur_user = current_user) > st_length(a.the_geom)', 'All outlet flow regulators has lengh wich fits target arc.', '[gw_fct_pg2epa_check_data]');
...
JOIN node n USING (node_id) JOIN arc a ON a.arc_id = to_arc 
WHERE n.sector_id = s.sector_id AND cur_user=current_user AND flwreg_length + 
(SELECT value::numeric FROM config_param_user WHERE parameter = ''inp_options_minlength'' AND cur_user = current_user) > st_length(a.the_geom)', 'All pump flow regulators has lengh wich fits target arc.', '[gw_fct_pg2epa_check_data]');
...
INSERT INTO sys_fprocess (fid, fprocess_name, "source", fprocess_type, project_type, except_level, except_msg, query_text, info_msg, function_name) VALUES(594, 'Null values on valve_type of virtualvalve', 'core', 'Check epa-data', 'ws', 1, 'virtualvalves with null values on valv_type column.', 'SELECT n.node_id, n.nodecat_id, n.the_geom, expl_id FROM man_valve JOIN v_prefix_node n USING (node_id) 
...
INSERT INTO sys_fprocess (fid, fprocess_name, project_type, parameters, "source", isaudit, fprocess_type, addparam, except_level, except_msg, except_table, except_table_msg, query_text, info_msg, function_name, active) VALUES(636, 'dma-nodeparent acording with graphdelimiter (unactive dma)', 'ws', NULL, 'core', true, 'Check graph-config', NULL, 2, 'nodes with cat_feature_node.graph_delimiter=''DMA'' configured for unactive mapzone.', NULL, NULL, 'SELECT node_id, nodecat_id, the_geom, a.active, t_node.expl_id FROM t_node JOIN cat_node c ON id=nodecat_id JOIN cat_feature_node n ON n.id=c.node_type LEFT JOIN (SELECT node_id, active FROM t_node JOIN (SELECT (json_array_elements_text((graphconfig::json->>''use'')::json))::json->>''nodeParent'' as node_id,   active FROM t_dma WHERE graphconfig IS NOT NULL )a USING (node_id)) a USING (node_id) WHERE graph_delimiter=''DMA'' AND (a.node_id IS NULL  OR node_id NOT IN (SELECT (json_array_elements_text((graphconfig::json->>''ignore'')::json))::text FROM t_dma WHERE active IS TRUE)) AND t_node.state > 0 and verified<>''2'' and a.active is null', 'All nodes with cat_feature_node.graphdelimiter=''DMA'' are defined as nodeParent on dma.graphconfig', '[gw_fct_graphanalytics_check_data, gw_fct_om_check_data, gw_fct_admin_check_data]', true) ON CONFLICT (fid) DO NOTHING;
