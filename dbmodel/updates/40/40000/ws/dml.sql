...
VALUES ('edit_connec_linkcat_vdefault','config','Value default catalog for link connected to connec','role_edit','Default catalog for linkcat','SELECT cat_arc.id, cat_arc.id AS idval FROM cat_arc JOIN cat_feature ON cat_feature.id = cat_arc.arc_type WHERE cat_feature.feature_type = ''ARC''',true,16,'utils',false,'linkcat_id',false,'text','combo',true,'PVC25-PN16-DOM','lyt_connec',true);
...
SELECT id,
  CASE
    ...
    ELSE 'PIPE'
  END 
  AS connec_type, matcat_id, pnom, dnom, dint, dext, descript, link, brand_id, model_id, svg, estimated_depth, active,
"label"
FROM cat_connec ON CONFLICT DO NOTHING;

INSERT INTO cat_arc (id,arc_type,shape) VALUES ('UPDATE_LINK_40','PIPE','CIRCULAR');
