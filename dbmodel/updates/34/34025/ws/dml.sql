...
VALUES ('new_mapzone', 'form_catalog', 'mapzoneType', 1, 'string','combo', 'Type', TRUE, FALSE,TRUE, FALSE,'lyt_data_1',FALSE,
'select ''DMA'' as id, ''DMA'' as idval union select ''PRESSZONE'' as id, ''PRESSZONE'' as idval FROM exploitation WHERE expl_id IS NOT NULL',
...
VALUES ('new_mapzone', 'form_catalog', 'id', 2, 'string','text', 'Id', TRUE, FALSE,TRUE, FALSE,'lyt_data_1',FALSE,'DMA: integer, PRESSZONE: text')
ON CONFLICT (formname, formtype, columnname) DO NOTHING;

INSERT INTO config_form_fields(formname, formtype, columnname, layoutorder, datatype, widgettype,
label, ismandatory, isparent, iseditable, isautoupdate,layoutname, hidden)
VALUES ('new_mapzone', 'form_catalog', 'name', 3, 'string','text', 'Name', TRUE, FALSE,TRUE, FALSE,'lyt_data_1',FALSE)
ON CONFLICT (formname, formtype, columnname) DO NOTHING;
...
INSERT INTO config_form_fields(formname, formtype, columnname, layoutorder, datatype, widgettype,
label, ismandatory, isparent, iseditable, isautoupdate, layoutname, hidden)
VALUES ('new_mapzone', 'form_catalog', 'link', 5, 'string','text', 'Link', FALSE, FALSE,TRUE, FALSE,'lyt_data_1',FALSE)
ON CONFLICT (formname, formtype, columnname) DO NOTHING;

INSERT INTO config_form_fields(formname, formtype, columnname, layoutorder, datatype, widgettype,
label, ismandatory, isparent, iseditable, isautoupdate, layoutname, hidden,placeholder)
VALUES ('new_mapzone', 'form_catalog', 'stylesheet', 6, 'string','text', 'Stylesheet', FALSE, FALSE,TRUE, FALSE,'lyt_data_1',FALSE,
