...
CREATE OR REPLACE VIEW vi_status AS SELECT rpt_inp_arc.arc_id,
  ...
  WHERE rpt_inp_arc.status::text = 'ACTIVE_VALVE'::text OR rpt_inp_arc.status::text = 'CLOSED_VALVE'::text OR rpt_inp_arc.status::text = 'OPEN_VALVE'::text AND rpt_inp_arc.result_id::text = inp_selector_result.result_id::text AND inp_selector_result.cur_user = "current_user"()::text
UNION
 SELECT rpt_inp_arc.arc_id,
   idval
    ...
    rpt_inp_arc
     ...
     JOIN inp_typevalue ON inp_typevalue.id=rpt_inp_arc.status
 WHERE inp_pump.status::text = 'OPEN_PUMP'::text OR inp_pump.status::text = 'CLOSED_PUMP'::text AND rpt_inp_arc.result_id::text = inp_selector_result.result_id::text AND inp_selector_result.cur_user = "current_user"()::text
UNION
 SELECT rpt_inp_arc.arc_id,
   idval
    FROM inp_selector_result,
