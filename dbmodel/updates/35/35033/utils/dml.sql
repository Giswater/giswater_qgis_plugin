...
SELECT  'v_edit_node','form_feature', 'data', 'parent_id', 'lyt_data_1', max(layoutorder)+1, 'string', 'typeahead', 'parent_id', 'parent_id - Identificador de su nodo pariente. Un nodo padre puede tener varios otros nodos contenidos dentro de él',
...
INSERT INTO config_form_fields (formname, formtype, tabname, columnname, layoutname, layoutorder, 
datatype, widgettype, label, tooltip,  ismandatory, isparent, iseditable, isautoupdate, isfilter, dv_querytext, dv_isnullvalue, hidden)
select 've_node','form_feature', 'data', 'parent_id', 'lyt_data_1', max(layoutorder)+1, 'string', 'typeahead', 'parent_id', 'parent_id - Identificador de su nodo pariente. Un nodo padre puede tener varios otros nodos contenidos dentro de él',
false, false,true,false,false, 'SELECT node_id AS id, node_id AS idval FROM node WHERE node_id IS NOT NULL', true, true
...
WHERE feature_type = 'NODE' AND  (formname ilike 'v_edit_node' and columnname = 'parent_id')  group by child_layer,formtype, tabname, columnname, layoutname,
datatype, widgettype, label, tooltip,  ismandatory, isparent, iseditable, isautoupdate, isfilter, dv_querytext, dv_orderby_id, dv_isnullvalue, dv_parent_id, dv_querytext_filterc,  hidden 
ON CONFLICT (formname, formtype, columnname, tabname) DO NOTHING;
