...
CREATE OR REPLACE VIEW vu_arc AS 
 ...
 SELECT arc.arc_id,
    ...
    arc.elev1,
    arc.custom_elev1,
    CASE when arc.sys_elev1 IS NULL THEN a.sys_elev::numeric(12,3) ELSE arc.sys_elev1 END AS sys_elev1,
    ...
    arc.elev2,
    arc.custom_elev2,
    CASE when arc.sys_elev2 IS NULL THEN b.sys_elev::numeric(12,3) ELSE arc.sys_elev2 END AS sys_elev2,
    ...
    b.sys_top_elev - arc.sys_elev2 - cat_arc.geom1 AS r2,
        ...
        END AS z2,
    CASE WHEN arc.sys_slope IS NULL AND st_length(arc.the_geom) > 1 THEN
	((a.sys_elev-b.sys_elev)/st_length(arc.the_geom))::numeric(12,4)
	ELSE arc.sys_slope::numeric(12,4) END AS slope,
    arc.arc_type,
