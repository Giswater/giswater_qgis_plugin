...
UPDATE config_form_fields SET dv_querytext='SELECT category_type as id, category_type as idval FROM man_type_category WHERE ((featurecat_id is null AND ''ELEMENT''=ANY(feature_type))) AND active IS TRUE' WHERE formname='ve_element' AND formtype='form_feature' AND columnname='category_type' AND tabname='tab_data';