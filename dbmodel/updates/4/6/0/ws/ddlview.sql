...
JOIN selector_sector ssec ON ssec.sector_id = a.sector_id  AND ssec.cur_user = CURRENT_USER
JOIN selector_municipality sm ON sm.muni_id = a.muni_id AND sm.cur_user = CURRENT_USER
JOIN selector_expl se ON se.expl_id = ANY (array_append(a.expl_visibility::integer[], a.expl_id)) AND se.cur_user = CURRENT_USER
...
LEFT JOIN LATERAL (
    ...
    FROM plan_psector_x_connec pp
    WHERE pp.connec_id = c.connec_id
    AND pp.psector_id IN (
        SELECT sp.psector_id
        FROM selector_psector sp
        WHERE sp.cur_user = CURRENT_USER
...
JOIN selector_sector ssec ON ssec.sector_id = c.sector_id AND ssec.cur_user = CURRENT_USER
JOIN selector_municipality sm ON sm.muni_id = c.muni_id AND sm.cur_user = CURRENT_USER
JOIN selector_expl se ON se.expl_id = ANY (array_append(c.expl_visibility::integer[], c.expl_id)) AND se.cur_user = CURRENT_USER
...
AS SELECT
    ...
    e.uuid
FROM element e
JOIN selector_state ss ON ss.state_id = e.state AND ss.cur_user = CURRENT_USER
JOIN selector_sector ssec ON ssec.sector_id = e.sector_id AND ssec.cur_user = CURRENT_USER
JOIN selector_municipality sm  ON sm.muni_id = e.muni_id AND sm.cur_user = CURRENT_USER
JOIN selector_expl se ON se.expl_id = e.expl_id AND se.cur_user = CURRENT_USER
