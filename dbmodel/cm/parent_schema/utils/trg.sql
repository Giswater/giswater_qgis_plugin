...
FOR EACH ROW EXECUTE FUNCTION cm.gw_trg_lot_x_feature('gully');
...
FOR EACH ROW EXECUTE FUNCTION cm.gw_trg_lot_x_feature_check_campaign('gully');
...
BEGIN
    ...
    LOOP
        ...
        IF mission_type IS NOT NULL THEN
            EXECUTE format(
                ...
                'FOR EACH ROW EXECUTE PROCEDURE cm.gw_trg_cm_log(%L, %L, %L)',
    ...
    LOOP
        ...
        EXECUTE format(
            'CREATE OR REPLACE TRIGGER %I ' ||
            'AFTER INSERT OR UPDATE OR DELETE ON cm.%I ' ||
            'FOR EACH ROW EXECUTE PROCEDURE cm.gw_trg_log_cm(%L, %L, %L)',
            trigger_name, rec.table_name, feature_type, mission_type, mission_id_column
        );
    END LOOP;
