...
CREATE OR REPLACE FUNCTION SCHEMA_NAME.gw_fct_getformfieldscm(
...
AS $BODY$
/*EXAMPLE
SELECT SCHEMA_NAME.gw_fct_getformfieldscm( 've_arc_pipe','form_feature','tab_data','ve_arc_pipe','arc_id',	'2088','character varying(16)',	'UPDATE',	1,	'4',	'{}' )
SELECT SCHEMA_NAME.gw_fct_getformfieldscm('visit_arc_insp', 'form_visit', 'tab_data', NULL,	 NULL, 		NULL, 	NULL, 			'INSERT', 	null, 	3,	null)
SELECT "SCHEMA_NAME".gw_fct_getformfieldscm('go2epa', 'form_generic', 'tab_data', null, null, null, null, null, null,null, '{}')
SELECT "SCHEMA_NAME".gw_fct_getformfieldscm('ve_arc_conduit', 'form_feature', 'tab_data', 've_arc_conduit', 'arc_id', '2001', NULL, 'SELECT', null, 4, NULL)
SELECT "SCHEMA_NAME".gw_fct_getformfieldscm('ve_arc_pipe', 'form_feature', NULL, NULL, NULL, NULL, NULL, 'INSERT', null, 4, '{}')
SELECT "SCHEMA_NAME".gw_fct_getformfieldscm( 'print', 'form_generic', 'tab_data', null, null, null, null, 'SELECT', null, 3);

PERFORM gw_fct_debug(concat('{"data":{"msg":"----> INPUT FOR gw_fct_getformfieldscm: ", "variables":"',v_debug,'"}}')::json);
PERFORM gw_fct_debug(concat('{"data":{"msg":"<---- OUTPUT FOR gw_fct_getformfieldscm: ", "variables":"',v_debug,'"}}')::json);
...
BEGIN
	...
	IF v_debug = TRUE THEN
		...
		PERFORM gw_fct_debug(concat('{"data":{"msg":"----> INPUT FOR gw_fct_getformfieldscm: ", "variables":',v_debug_var,'}}')::json);
	END IF;
	...
	IF p_formname ILIKE 'v_edit_inp%' THEN
		...
		v_debug_sql := json_build_object('querystring', v_querystring, 'vars', v_debug_vars, 'funcname', 'gw_fct_getformfieldscm', 'flag', 10);
	...
	ELSIF p_formname='infoplan' THEN
		...
		v_debug_sql := json_build_object('querystring', v_querystring, 'vars', v_debug_vars, 'funcname', 'gw_fct_getformfieldscm', 'flag', 20);
		SELECT gw_fct_debugsql(v_debug_sql) INTO v_msgerr;
		EXECUTE v_querystring INTO fields_array;
	...
	LOOP
		...
		IF  (aux_json->>'queryText') is not null and (aux_json->>'queryText') != '' then
			...
			v_debug_sql := json_build_object('querystring', v_querystring, 'vars', v_debug_vars, 'funcname', 'gw_fct_getformfieldscm', 'flag', 30);
			SELECT gw_fct_debugsql(v_debug_sql) INTO v_msgerr;
		...
		IF  (aux_json->>'queryText') is not null and (aux_json->>'queryText') != '' then
			...
			v_debug_vars := json_build_object('aux_json->>''queryText''', (aux_json->>'queryText'), 'v_orderby', v_orderby);
			v_debug_sql := json_build_object('querystring', v_querystring, 'vars', v_debug_vars, 'funcname', 'gw_fct_getformfieldscm', 'flag', 40);
			SELECT gw_fct_debugsql(v_debug_sql) INTO v_msgerr;
			EXECUTE v_querystring INTO v_array;
			-- Enable null values
			IF (aux_json->>'isNullValue')::boolean IS TRUE THEN
	...
	LOOP
		...
		IF p_tgop ='INSERT' THEN
			...
			ELSE
				...
				v_debug_sql := json_build_object('querystring', v_querystring, 'vars', v_debug_vars, 'funcname', 'gw_fct_getformfieldscm', 'flag', 50);
				SELECT gw_fct_debugsql(v_debug_sql) INTO v_msgerr;
		...
		IF (aux_json->>'queryTextFilter') IS NOT NULL AND v_selected_id IS NOT NULL THEN
			...
			v_debug_sql := json_build_object('querystring', v_querystring, 'vars', v_debug_vars, 'funcname', 'gw_fct_getformfieldscm', 'flag', 60);
			SELECT gw_fct_debugsql(v_debug_sql) INTO v_msgerr;
		...
		ELSE
			v_querystring = concat('SELECT (array_agg(id)) FROM (',(aux_json->>'queryText'),' ORDER BY ',v_orderby,')a');
			v_debug_vars := json_build_object('aux_json->>''queryText''', (aux_json->>'queryText'), 'v_orderby', v_orderby);
			v_debug_sql := json_build_object('querystring', v_querystring, 'vars', v_debug_vars, 'funcname', 'gw_fct_getformfieldscm', 'flag', 70);
			SELECT gw_fct_debugsql(v_debug_sql) INTO v_msgerr;
			EXECUTE v_querystring INTO v_array;
		...
		IF (aux_json->>'queryTextFilter') IS NOT NULL AND v_selected_id IS NOT NULL THEN
			...
			,' ORDER BY ',v_orderby,') a');
			v_debug_vars := json_build_object('aux_json->>''queryText''', (aux_json->>'queryText'), 'aux_json->>''queryTextFilter''', (aux_json->>'queryTextFilter'), 'v_selected_id', v_selected_id, 'v_orderby', v_orderby);
			v_debug_sql := json_build_object('querystring', v_querystring, 'vars', v_debug_vars, 'funcname', 'gw_fct_getformfieldscm', 'flag', 80);
			SELECT gw_fct_debugsql(v_debug_sql) INTO v_msgerr;
			EXECUTE v_querystring INTO v_array;
		ELSE
			v_querystring = concat('SELECT (array_agg(idval)) FROM (',(aux_json->>'queryText'),' ORDER BY ',v_orderby,')a');
			v_debug_vars := json_build_object('aux_json->>''queryText''', (aux_json->>'queryText'), 'v_orderby', v_orderby);
			v_debug_sql := json_build_object('querystring', v_querystring, 'vars', v_debug_vars, 'funcname', 'gw_fct_getformfieldscm', 'flag', 90);
			SELECT gw_fct_debugsql(v_debug_sql) INTO v_msgerr;
			EXECUTE v_querystring INTO v_array;
		END IF;
	...
	*/
	...
	PERFORM gw_fct_debug(concat('{"data":{"msg":"<---- OUTPUT FOR gw_fct_getformfieldscm: ", "variables":""}}')::json);
