...
CREATE OR REPLACE FUNCTION SCHEMA_NAME.gw_fct_getworkorder(p_data json)
  RETURNS json AS
$BODY$
...
BEGIN
    ...
    EXECUTE
      'SELECT row_to_json(row) FROM (SELECT value FROM SCHEMA_NAME.config_param_system WHERE parameter=''admin_version'') row'
    INTO v_version;
    ...
    ELSE
        SELECT nextval('SCHEMA_NAME.workorder_workorder_id_seq') INTO v_id;
