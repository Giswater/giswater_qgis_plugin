...
CREATE OR REPLACE FUNCTION SCHEMA_NAME.gw_fct_getSCHEMA_NAMEteam(p_data json)
 RETURNS json
 LANGUAGE plpgsql
AS $function$
DECLARE
...
BEGIN
   ...
   JOIN SCHEMA_NAME.cat_organization co ON co.organization_id = oc.organization_id
  ...
  -- Assumed that OWNER text is the administrators
  	IF v_orgname = 'OWNER' THEN
	    ...
	    INTO v_result
	    FROM SCHEMA_NAME.cat_team AS ct
	    WHERE ct.role_id <> 'role_SCHEMA_NAME_admin'; -- ASSUMED
	ELSE
		-- Return teams that match the criteria
	    SELECT json_build_object(
	        'status', 'Accepted',
	    ...
	    INTO v_result
	    FROM SCHEMA_NAME.cat_team AS ct
	    JOIN SCHEMA_NAME.om_campaign AS oc ON ct.organization_id = oc.organization_id
	    WHERE oc.campaign_id = v_campaign_id 
	   	AND ct.role_id = 'role_SCHEMA_NAME_field'; -- ASSUMED
     END IF;
