	...
	IF v_tablename IS NOT NULL AND (v_idname IS NOT NULL OR v_id IS NOT NULL) THEN
		...
		LOOP
			...
			IF (v_aux_json->>'widgettype')='combo' THEN
				v_fields_array[v_array_index] := gw_fct_json_object_set_key_cm(v_fields_array[v_array_index], 'selectedId', COALESCE(v_field_value, ''));
			ELSIF (v_aux_json->>'widgettype')='button' and json_extract_path_text(v_aux_json,'widgetcontrols','text') IS NOT NULL THEN
				v_fields_array[v_array_index] := gw_fct_json_object_set_key_cm(v_fields_array[v_array_index], 'value', json_extract_path_text(v_aux_json,'widgetcontrols','text'));
			ELSE
				v_fields_array[v_array_index] := gw_fct_json_object_set_key_cm(v_fields_array[v_array_index], 'value', COALESCE(v_field_value, ''));
			END IF;
