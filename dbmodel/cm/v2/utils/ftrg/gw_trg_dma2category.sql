	 ...
	 */ 
	 ELSIF v_trigger_from = 'om_category' and (v_block != 'dma2category' or v_block is null)  THEN
			...
			IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
	            ...
	            v_querytext = 'UPDATE v_edit_'||v_feature_type||' SET dma_id = '||v_dma_id||' WHERE '||v_feature_type||'_id='||quote_literal(v_feature_id)||';';
	                EXECUTE v_querytext;
	            ELSE
			...
			ELSIF TG_OP = 'DELETE' THEN
				...
				IF (SELECT count(*) FROM om_category_x_gully join om_category using (category_id) where gully_id=v_feature_id and macrocategory_id in (1,2,3)) = 0 then
			   		v_querytext = 'UPDATE v_edit_'||v_feature_type||' SET dma_id = 0 WHERE '||v_feature_type||'_id='||quote_literal(v_feature_id)||';';
			   		EXECUTE v_querytext;
			   	end if;
