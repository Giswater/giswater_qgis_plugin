...
CREATE OR REPLACE FUNCTION PARENT_SCHEMA.gw_fct_cm_check_arc_duplicated(p_data json)
 RETURNS json
 LANGUAGE plpgsql
AS $function$
...
BEGIN
    ...
    'WITH mec AS (
		...
		WHERE the_geom IS NOT NULL AND lot_id = %L
	)
	...
	SELECT 999, 1, concat('There are ', v_count, ' duplicated arcs in the lot ', v_lot_id, '.');
