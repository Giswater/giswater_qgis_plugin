...
ALTER TABLE "SCHEMA_NAME".cat_arc
	ADD COLUMN z1 numeric (12,2);
ALTER TABLE "SCHEMA_NAME".cat_arc
	ADD COLUMN z2 numeric (12,2);
ALTER TABLE "SCHEMA_NAME".cat_arc
	ADD COLUMN width numeric (12,2);	
ALTER TABLE "SCHEMA_NAME".cat_arc
	ADD COLUMN area numeric (12,4);	
ALTER TABLE "SCHEMA_NAME".cat_arc
	ADD COLUMN estimated_depth numeric (12,2);
ALTER TABLE "SCHEMA_NAME".cat_arc
	ADD COLUMN 'bulk' numeric (12,2);
ALTER TABLE "SCHEMA_NAME".cat_arc
	ADD COLUMN cost_unit varchar (3);
ALTER TABLE "SCHEMA_NAME".cat_arc
	ADD COLUMN cost varchar (16);
ALTER TABLE "SCHEMA_NAME".cat_arc
	ADD COLUMN m2bottom_cost varchar (16);
ALTER TABLE "SCHEMA_NAME".cat_arc
	ADD COLUMN m3protec_cost varchar (16);

ALTER TABLE "SCHEMA_NAME".cat_node
	ADD COLUMN estimated_y numeric (12,2);
ALTER TABLE "SCHEMA_NAME".cat_node
	ADD COLUMN cost_unit varchar (3);
ALTER TABLE "SCHEMA_NAME".cat_node
	ADD COLUMN cost varchar (16);
...
CREATE TABLE "SCHEMA_NAME".plan_economic_selection  -- Used to show economic data
(
...
SELECT 
arc.arc_id,
v_inp_arc_x_node.depth1,
v_inp_arc_x_node.depth2,
(CASE WHEN (v_inp_arc_x_node.depth1*v_inp_arc_x_node.depth2 =0::numeric) THEN v_price_x_catarc.estimated_depth::numeric(12,2) ELSE ((v_inp_arc_x_node.depth1+v_inp_arc_x_node.depth2)/2)::numeric(12,2) END) AS mean_depth,
arc.arccat_id,
...
FROM "SCHEMA_NAME".arc
	JOIN "SCHEMA_NAME".v_inp_arc_x_node ON ((((arc.arc_id)::text = (v_inp_arc_x_node.arc_id)::text)))
	...
	JOIN "SCHEMA_NAME".v_price_x_catsoil ON ((((arc_dat.soilcat_id)::text = (v_price_x_catsoil.id)::text)))
	...
	GROUP BY arc.arc_id, v_inp_arc_x_node.depth1, v_inp_arc_x_node.depth2, mean_depth,arc.arccat_id, dint,z1,z2,area,width,bulk, cost_unit, arc_cost, m2bottom_cost,m3protec_cost,v_price_x_catsoil.id, y_param, b, trenchlining, m3exc_cost, m3fill_cost, m3excess_cost, m2trenchl_cost,arc.state, arc.the_geom;
...
FROM "SCHEMA_NAME".v_plan_ml_arc
	...
	JOIN "SCHEMA_NAME".plan_economic_selection ON (((v_plan_ml_arc."state")::text = (plan_economic_selection.id)::text));
...
(CASE WHEN (v_price_x_catnode.cost_unit='ut'::text) THEN NULL ELSE ((CASE WHEN (node.depth*1=0::numeric) THEN v_price_x_catnode.estimated_depth::numeric(12,2) ELSE ((node.depth)/2)END)) END)::numeric(12,2) AS calculated_depth,
v_price_x_catnode.cost,
(CASE WHEN (v_price_x_catnode.cost_unit='ut'::text) THEN v_price_x_catnode.cost ELSE ((CASE WHEN (node.depth*1=0::numeric) THEN v_price_x_catnode.estimated_depth::numeric(12,2) ELSE ((node.depth)/2)::numeric(12,2) END)*v_price_x_catnode.cost) END)::numeric(12,2) AS budget,
node."state",
node.the_geom

FROM ("SCHEMA_NAME".node
JOIN "SCHEMA_NAME".v_price_x_catnode ON ((((node.node_type)::text = (v_price_x_catnode.id)::text))))
JOIN "SCHEMA_NAME".plan_economic_selection ON (((node."state")::text = (plan_economic_selection.id)::text));
