...
-- FEATURE INSERT      
				...
				INSERT INTO dimensions (id, distance, depth, the_geom, coordx, coordy, feature_id, feature_type)
				VALUES (NEW.id, NEW.distance, NEW.depth, NEW.the_geom, NEW.coordx, NEW.coordy, NEW.feature_id, NEW.feature_type);
...
-- UPDATE
    ...
    ELSIF TG_OP = 'UPDATE' THEN
			...
			UPDATE dimensions
			SET id=NEW.id, distance=NEW.distance, depth=NEW.depth, the_geom=NEW.the_geom, coordx=NEW.coordx, coordy=NEW.coordy, feature_id=NEW.feature_id, feature_type=NEW.feature_type
			WHERE id=NEW.id;
