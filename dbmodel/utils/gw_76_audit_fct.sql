...
CREATE OR REPLACE FUNCTION SCHEMA_NAME.audit_function(    p_audit_cat_error_id integer,    p_audit_cat_function_id integer,    p_debug_info text)
RETURNS smallint AS
$BODY$
...
BEGIN
    ...
    SET search_path = "test_ws_0815", public; 
    ...
    IF p_audit_cat_error_id >= 1 AND p_audit_cat_error_id < 99 THEN  									-- function and trigger function trace log
		RETURN p_audit_cat_error_id;

    ELSIF (p_audit_cat_error_id >= 100 AND p_audit_cat_error_id < 500) AND audit_cat_error_rec.log_level=0 THEN			-- trigger function return
		...
		RETURN p_audit_cat_error_id;
    ELSIF (p_audit_cat_error_id >= 100 AND p_audit_cat_error_id < 500) AND audit_cat_error_rec.log_level=1 THEN 			-- trigger function return
		...
		RETURN p_audit_cat_error_id;	
ELSIF p_audit_cat_error_id >= 501 AND p_audit_cat_error_id < 998 THEN														-- function return
		RETURN p_audit_cat_error_id;
    END IF;
