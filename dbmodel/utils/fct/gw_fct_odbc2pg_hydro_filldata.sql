...
BEGIN
	...
	*/
	...
	SELECT a.feature_id , log_message::json->>'connec_id' FROM audit_log_data a
	WHERE log_message::json->>'year' IS NOT NULL AND log_message::json->>'period' IS NOT NULL AND fprocesscat_id=74 and log_message::json->>'connec_id' is not null and log_message::json->>'connec_id' !='None'
	ON CONFLICT (hydrometer_id) DO NOTHING;
	...
	JOIN ext_rtc_hydrometer b ON b.id=feature_id::integer
	...
	FROM audit_log_data 
	left JOIN ext_rtc_hydrometer a ON a.id=feature_id::integer
	left JOIN ext_hydrometer_category_x_pattern c ON a.category_id=c.category_id::integer
	left JOIN ext_cat_period b ON CONCAT (log_message::json->>'year', '-', log_message::json->>'period') = b.id
	WHERE log_message::json->>'year' IS NOT NULL AND log_message::json->>'period' IS NOT NULL AND fprocesscat_id=74
	...
	SELECT sum(value::numeric(12,5)) as lps, connec_id, expl_id::integer FROM query group by expl_id, connec_id) a WHERE connec.expl_id = a.expl_id and connec.connec_id = a.connec_id;
