...
BEGIN 
	...
	ELSE
		...
		VALUES (15, v_result_id, 1,'INFO: There is/are no row(s) without values on cat_arc.active column.');
	END IF;
	...
	ELSE
		INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
		VALUES (15, v_result_id, 1,'INFO: There is/are no row(s) without values on cat_arc.cost column.');
	END IF;
	...
	ELSE
		INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
		VALUES (15, v_result_id, 1,'INFO: There is/are no row(s) without values on cat_arc.m2bottom_cost column.');
	END IF;
	...
	ELSE
		INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
		VALUES (15, v_result_id, 1,'INFO: There is/are no row(s) without values on cat_arc.m3protec_cost column.');
	END IF;
	...
	ELSE
		INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
		VALUES (15, v_result_id, 1,'INFO: There is/are no row(s) without values on cat_node.active column.');
	END IF;
	...
	ELSE
		INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
		VALUES (15, v_result_id, 1,'INFO: There is/are no row(s) row(s) without values on cat_node.cost column.');
	END IF;
	...
	ELSE
		INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
		VALUES (15, v_result_id, 1,'INFO: There is/are no row(s) without values on cat_node.cost_unit column.');
	END IF;
	...
	IF v_project_type='WS' THEN 
		...
		ELSE
			INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
			VALUES (15, v_result_id, 1,'INFO: There is/are no row(s) without values on cat_node.estimated_depth column.');
		END IF;
	...
	ELSIF v_project_type='UD' THEN 
		...
		ELSE
			INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
			VALUES (15, v_result_id, 1,'INFO: There is/are no row(s) without values on cat_node.estimated_y column.');
		END IF;
	END IF;
	...
	ELSE
		INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
		VALUES (15, v_result_id, 1,'INFO: There is/are no row(s) without values on cat_connec.active column column.');
	END IF;
	...
	ELSE
		INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
		VALUES (15, v_result_id, 1,'INFO: There is/are no row(s) without values on cat_connec.cost_ut column.');
	END IF;
	...
	ELSE
		INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
		VALUES (15, v_result_id, 1,'INFO: There is/are no row(s) without values on cat_connec.cost_ml column.');
	END IF;
	...
	ELSE
		INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
		VALUES (15, v_result_id, 1,'INFO: There is/are no row(s) without values on cat_connec.cost_m3 column.');
	END IF;
	...
	ELSE
		INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
		VALUES (15, v_result_id, 1,'INFO: There is/are no row(s) without values on cat_pavement.thickness column.');
	END IF;
	...
	ELSE
		INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
		VALUES (15, v_result_id, 1,'INFO: There is/are no row(s) without values on cat_pavement.m2_cost column.');
	END IF;
	...
	ELSE
		INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
		VALUES (15, v_result_id, 1,'INFO: There is/are no row(s) without values on cat_soil.y_param column.');
	END IF;
	...
	ELSE
		INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
		VALUES (15, v_result_id, 1,'INFO: There is/are no row(s) without values on cat_soil.b column.');
	END IF;
	...
	ELSE
		INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
		VALUES (15, v_result_id, 1,'INFO: There is/are no row(s) without values on cat_soil.m3exc_cost column.');
	END IF;
	...
	ELSE
		INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
		VALUES (15, v_result_id, 1,'INFO: There is/are no row(s) without values on cat_soil.m3fill_cost column.');
	END IF;
	...
	ELSE
		INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
		VALUES (15, v_result_id, 1,'INFO: There is/are no row(s) without values on cat_soil.m3excess_cost column.');
	END IF;
	...
	ELSE
		INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
		VALUES (15, v_result_id, 1,'INFO: There is/are no row(s) without values on cat_soil.m2trenchl_cost column.');
	END IF;
	...
	IF v_project_type='UD' THEN
		...
		ELSE
			INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
			VALUES (15, v_result_id, 1,'INFO: There is/are no row(s) without values on cat_grate.active column.');
		END IF;
		...
		ELSE
			INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
			VALUES (15, v_result_id, 1,'INFO: There is/are no row(s) without values on cat_grate.cost_ut column.');
		END IF;
	...
	ELSE
		INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
		VALUES (15, v_result_id, 1,'INFO: The number of rows of row(s) of the plan_arc_x_pavement table is same than the arc table.');
	END IF;
	...
	ELSE
		INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) 
		VALUES (15, v_result_id, 1,'INFO: There is/are no row(s) without values on row(s) without values on plan_arc_x_pavement.pavcat_id column.');
	END IF;
