...
-- FUNCTION: SCHEMA_NAME.gw_fct_pg2epa_check_network(json)

-- DROP FUNCTION IF EXISTS SCHEMA_NAME.gw_fct_pg2epa_check_network(json);

CREATE OR REPLACE FUNCTION SCHEMA_NAME.gw_fct_pg2epa_check_network(
	p_data json)
    RETURNS json
    LANGUAGE 'plpgsql'
    COST 100
...
BEGIN
	...
	INSERT INTO audit_check_data (fid, result_id, criticity, error_message) VALUES (v_fid, v_result_id, 2,concat(''));
	INSERT INTO audit_check_data (fid, result_id, criticity, error_message) VALUES (v_fid, v_result_id, 2,concat('BASIC STATS'));
	INSERT INTO audit_check_data (fid, result_id, criticity, error_message) VALUES (v_fid, v_result_id, 2,concat('-------------------'));
	...
	IF v_project_type =  'WS' THEN
		...
		SELECT count(*) INTO v_connec_num FROM connec WHERE arc_id IN (SELECT arc_id FROM temp_arc);

		SELECT sum(st_length(the_geom)) INTO v_connec_meters FROM link WHERE feature_id IN (SELECT connec_id FROM connec WHERE arc_id IN (SELECT arc_id FROM temp_arc));

		v_networkstats = concat('{"Total length (Km) ":"',v_sumlength,'", "connec_num": "',v_connec_num,'", "connec_meters": "',v_connec_meters,'"}');
