...
CREATE OR REPLACE FUNCTION ud_inp.gw_fct_utils_csv2pg_import_swmm_rpt(p_result_id text)
  RETURNS integer AS
$BODY$

/*EXAMPLE
SELECT ud_inp.gw_fct_utils_csv2pg_import_epanet_rpt(result1)
*/
...
BEGIN
	...
	LOOP
		IF (SELECT tablename FROM sys_csv2pg_config WHERE header_text=concat(rpt_rec.csv1,' ',rpt_rec.csv2) AND pg2csvcat_id=11) IS NOT NULL THEN
			type_aux=(SELECT tablename FROM sys_csv2pg_config WHERE header_text=concat(rpt_rec.csv1,' ',rpt_rec.csv2) AND pg2csvcat_id=11);
		END IF;	
