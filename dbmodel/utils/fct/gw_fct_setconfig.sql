...
BEGIN
   ...
   END LOOP;
	...
	RETURN ('{"status":"Failed","message":' || to_json(SQLERRM) || ', "message":{"level":'||to_json(right(SQLSTATE, 1))||', "text":"'||to_json(SQLERRM)||'"}, "version":'|| v_version ||',"SQLSTATE":' || to_json(SQLSTATE) || '}')::json;
