...
BEGIN
	...
	INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) VALUES (73, NULL, 3, 'CRITICAL ERRORS');	
	INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) VALUES (73, NULL, 3, '----------------------');	
	...
	INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) VALUES (73, NULL, 1, 'INFO');
	INSERT INTO audit_check_data (fprocesscat_id, result_id, criticity, error_message) VALUES (73, NULL, 1, '-------');	
	...
	IF v_count > 0 THEN
		DELETE FROM anl_node WHERE fprocesscat_id=90 and cur_user=current_user;
		...
		VALUES (73, 2, concat('WARNING: There is/are ',v_count,' arc(s) NOT exported through the ODBC system. Please check your data before continue'));
		INSERT INTO audit_check_data (fprocesscat_id, criticity, error_message) 
	...
	EXECUTE concat('SELECT count(*) FROM (',v_querytext,')a') INTO v_count;
	IF v_count > 0 THEN
		DELETE FROM anl_node WHERE fprocesscat_id=92 and cur_user=current_user;
	...
	-- get relation dma-number of exported connecs
	INSERT INTO audit_check_data (fprocesscat_id, criticity, error_message) 
	VALUES (73, 1, 'DMA - NUMBER OF CONNECS EXPORTED');
	INSERT INTO audit_check_data (fprocesscat_id, criticity, error_message)  
	SELECT 73, 1, concat(dma_id, ' - ', count(*)) FROM v_edit_connec 
	WHERE expl_id=v_expl
	GROUP BY dma_id; 

	-- get relation dma-number of imported connecs
	INSERT INTO audit_check_data (fprocesscat_id, criticity, error_message) 
	...
	FROM (SELECT id, error_message as message FROM audit_check_data WHERE user_name="current_user"() AND fprocesscat_id=73 order by criticity desc, id asc) row; 
	...
	v_result = null;
	SELECT array_to_json(array_agg(row_to_json(row))) INTO v_result 
	FROM (SELECT id, node_id, nodecat_id, state, expl_id, descript, the_geom FROM anl_node WHERE cur_user="current_user"() AND fprocesscat_id=92) row; 
	v_result := COALESCE(v_result, '{}'); 
