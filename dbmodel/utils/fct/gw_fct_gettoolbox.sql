...
--    Control NULL's
	...
	RETURN ('{"status":"Failed","SQLERR":' || to_json(SQLERRM) || ', "message":{"level":'||to_json(right(SQLSTATE, 1))||', "text":"'||to_json(SQLERRM)||'"}, "version":'|| v_version || ',"SQLSTATE":' || to_json(SQLSTATE) || ',"MSGERR": '|| to_json(v_msgerr::json ->> 'MSGERR') ||'}')::json;
