...
BEGIN
	...
	INSERT INTO audit_check_data (fid, result_id, error_message) VALUES (v_fid, v_result_id, concat('IMPORT CATALOG'));
		...
		IF v_fid = 450 THEN
	    	...
	    	INSERT INTO plan_price(id, unit, descript)
   			...
   			SELECT csv22, 'u', csv22 FROM temp_csv WHERE cur_user=current_user AND fid=v_fid AND csv22 IS NOT NULL  ON CONFLICT (id) DO NOTHING;
				...
				INSERT INTO cat_arc(id, arctype_id, matcat_id, pnom, dnom, dint, dext, descript, link, brand, model, svg, z1, z2, width, area, estimated_depth, 
		...
		ELSIF v_fid=448 THEN
		    ...
		    INSERT INTO cat_brand_model(id, catbrand_id)
   			...
   			WHERE cur_user=current_user AND csv16 IS NOT NULL AND fid=v_fid ON CONFLICT (id) DO NOTHING; 
				...
				INSERT INTO cat_node(id, nodetype_id, matcat_id, pnom, dnom, dint, dext, shape, descript, 
