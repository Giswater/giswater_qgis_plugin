...
BEGIN
	...
	SELECT array_agg(row_to_json(d)) FROM (SELECT context, alias as "layerName", st.id as "tableName",CASE WHEN c.column_name IS NULL THEN 'None' ELSE c.column_name END AS "geomField", i.column_name as "tableId" FROM sys_table st
