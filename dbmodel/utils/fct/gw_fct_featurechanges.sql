			...
			CASE v_featuretype
			    WHEN 'FEATURE' THEN
			        ...
			        v_select_node = 'SELECT '||v_feature||'_id as "'||lower(v_feature)||'Id", '||
			                          lower(v_feature)||'cat_id as "featureClass", macrosector_code as "macrosectorCode", comment as "aresepId", asset_id as "assetId", state FROM vu_'||v_feature|| ' WHERE lastupdate >= '''||v_lastfeeding||'''';
					
					-- CONNEC
					v_feature = 'connec';
					v_select_connec = 'SELECT '||v_feature||'_id as "'||lower(v_feature)||'Id", '||
					-- Combine the query parts and execute it
			                          lower(v_feature)||'at_id as "featureClass", macrosector_code as "macrosectorCode", comment as "aresepId", asset_id as "assetId", state FROM vu_'||v_feature|| ' WHERE lastupdate >= '''||v_lastfeeding||'''';
		...
		WHEN 'UPDATE' THEN
			CASE v_featuretype
				WHEN 'FEATURE' THEN
					...
					v_feature = 'node';
					v_select_node = 'SELECT '||v_feature||'_id as "'||lower(v_feature)||'Id", '||
				    				lower(v_feature)||'cat_id as "featureClass", macrosector_code as "macrosectorCode", comment as "aresepId", asset_id as "assetId", state FROM vu_'||v_feature|| ' WHERE lastupdate >= '''||v_lastfeeding||''' and '||v_feature||'_id IN (SELECT newdata->>''node_id'' as id FROM audit.log WHERE exists (SELECT 1 FROM unnest(array[''macrosector_id'', ''state'', ''nodecat_id'', ''asset_id'']) AS key WHERE olddata->key IS NOT NULL AND newdata->key IS NOT NULL and table_name ilike ''%_element_x_%'' AND schema = ''SCHEMA_NAME''))';
					-- CONNEC
					v_feature = 'connec';
					v_select_connec = 'SELECT '||v_feature||'_id as "'||lower(v_feature)||'Id", '||
					-- Combine the query parts and execute it
		                   			lower(v_feature)||'at_id as "featureClass", macrosector_code as "macrosectorCode", comment as "aresepId", asset_id as "assetId", state FROM vu_'||v_feature|| ' WHERE lastupdate >= '''||v_lastfeeding||''' and '||v_feature||'_id IN (SELECT newdata->>''connec_id'' as id FROM audit.log WHERE exists (SELECT 1 FROM unnest(array[''macrosector_id'', ''state'', ''nodecat_id'', ''asset_id'']) AS key WHERE olddata->key IS NOT NULL AND newdata->key IS NOT NULL and table_name ilike ''%_element_x_%'' AND schema = ''SCHEMA_NAME''))';
