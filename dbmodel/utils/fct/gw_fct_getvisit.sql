...
SELECT ws_36.gw_fct_getvisit($${"client":{"device":5, "lang":"es_ES", "cur_user": "bgeo", "infoType":1, "epsg":25831}, "form":{"visit_id": null, "visit_type": 1}, "feature":{}, "data":{"filterFields":{}, "pageInfo":{}, "coordinates": {"xcoord": 418980.68916666665, "ycoord": 4576904.783750001, "zoomRatio": 10000}}}$$);
...
-- Sensibility factor
	...
	END IF;
	-- Get feature
	v_sql = concat('SELECT DISTINCT(layer_id), orderby+100 orderby, addparam->>''geomType'' as geomtype FROM  ',quote_ident(v_config_layer),' JOIN cat_feature ON parent_layer=layer_id  ORDER BY orderby');
	...
	v_querystring = concat('SELECT array_agg(row_to_json(a)) FROM (SELECT DISTINCT ON (tabname, orderby) tabname as "tabName", label as "tabLabel", tooltip as "tooltip",NULL as "tabFunction", NULL AS "tabactions",  orderby  
							FROM config_form_tabs WHERE formname =''visit'' AND device = ', v_device,' AND orderby IS NOT NULL ORDER BY orderby, tabname)a');
