...
BEGIN
	...
	SELECT node_id, state, nodecat_id, expl_id, the_geom FROM node WHERE state=1 AND node_id NOT IN (SELECT node_id FROM plan_psector_x_node WHERE psector_id = '||v_psector||' AND state=0)
