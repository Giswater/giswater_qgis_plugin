...
BEGIN
	...
	IF v_feature_list IS NOT NULL THEN
		...
		SELECT DISTINCT connec_id, v_new_psector_id, state, doable, descript,link_geom FROM plan_psector_x_connec 
	...
	IF v_project_type='UD' THEN
		...
		IF v_feature_list IS NOT NULL THEN
			...
			SELECT DISTINCT gully_id, v_new_psector_id, state, doable, descript,link_geom FROM plan_psector_x_gully 
			WHERE psector_id=v_old_psector_id AND (state=0 OR (state=1 and doable = false));
