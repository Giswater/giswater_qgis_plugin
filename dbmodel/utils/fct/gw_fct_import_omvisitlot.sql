...
BEGIN
	...
	v_parameter1 = (SELECT parameter_id FROM config_visit_parameter_x_parameter cxp JOIN config_api_visit cav ON cav.visitclass_id=cxp.class_id WHERE cav.tablename = v_featuretable LIMIT 1);
	v_parameter2 = (SELECT parameter_id FROM config_visit_parameter_x_parameter cxp JOIN config_api_visit cav ON cav.visitclass_id=cxp.class_id WHERE cav.tablename = v_featuretable LIMIT 1 OFFSET 1);
	...
	INSERT INTO audit_check_data (fprocesscat_id, result_id, error_message) VALUES (54, v_result_id, concat('------------------------------'));
 	...
 	-- starting process
	FOR v_visit IN SELECT * FROM temp_csv2pg WHERE cur_user=current_user AND csv2pgcat_id=21
	LOOP 
	...
	DELETE FROM temp_csv2pg WHERE cur_user=current_user AND csv2pgcat_id=21;
	...
	FROM (SELECT id, error_message as message FROM audit_check_data WHERE cur_user="current_user"() AND fprocesscat_id=54) row; 
