...
BEGIN
	...
	FROM (SELECT id, error_message as message FROM audit_check_data WHERE user_name="current_user"() AND fprocesscat_id=3 order by id) row; 
	...
	v_result = null;
	SELECT array_to_json(array_agg(row_to_json(row))) INTO v_result 
	FROM (SELECT id, node_id, arccat_id, state, expl_id, descript, the_geom_p, fprocesscat_id FROM anl_arc_x_node WHERE cur_user="current_user"() AND fprocesscat_id=3) row; 
	v_result := COALESCE(v_result, '{}'); 
