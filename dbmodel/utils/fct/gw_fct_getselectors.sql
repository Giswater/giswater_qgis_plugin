...
BEGIN
	...
	ELSE
		...
		WHERE n.sector_id is null AND s.active and s.sector_id > 0
 			UNION
		select distinct on (s.sector_id) s.sector_id, s.name, s.macrosector_id, s.descript, s.parent_id, s.active from sector s
		JOIN (SELECT DISTINCT node.sector_id, unnest(node.expl_visibility) as expl_visibility FROM node WHERE node.state > 0)n USING (sector_id)
		JOIN exploitation e ON e.expl_id=n.expl_visibility
		JOIN config_user_x_expl c ON c.expl_id=n.expl_visibility WHERE s.active and s.sector_id > 0 and username = current_user
		order by 1;
