...
SELECT SCHEMA_NAME.gw_fct_arc_divide($${
"client":{"device":4, "infoType":1, "lang":"ES"},
"feature":{"id":"SELECT array_to_json(array_agg(node_id)) FROM ve_node_filter WHERE state=1"},
"data":{}}$$)
...
BEGIN
	...
	v_feature_text = lower(((p_data ->>'feature')::json->>'id')::text);
...
IF v_feature_array IS NOT NULL THEN
	...
	FOREACH v_node_id in array(v_feature_array) loop
				...
				IF v_arc_id IS NOT NULL THEN 
					...
					ELSE
						...
						ELSIF (v_state=1 AND v_state_node=2) THEN
							...
							INSERT INTO audit_check_data (fid,  criticity, error_message)
							VALUES (212, 1,concat('Insert arcs into current psector: ',v_psector,'.'));
