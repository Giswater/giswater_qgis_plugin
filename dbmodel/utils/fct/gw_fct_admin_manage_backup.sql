...
INSTRUCTIONS
------------
CREATE, create a backup for all data for related table into temp_table with fid = 434
...
BEGIN 
	...
	IF v_backupname IS NULL OR length(v_backupname) < 1 THEN
		...
		"data":{"message":"3146", "function":"2792","parameters":null, "is_process":true}}$$);' INTO v_audit_result;
	...
	ELSE
		...
		IF v_action ='CREATE' THEN
			...
			IF (SELECT count(*) FROM temp_table WHERE text_column::json->>'name' = v_backupname AND fid = 434) > 0 THEN
				...
				EXECUTE 'SELECT gw_fct_getmessage($${"client":{"device":4, "infoType":1, "lang":"ES"},"feature":{},
				"data":{"message":"3148", "function":"2792","parameters":{"backup_name":"'||v_backupname||'"}, "is_process":true}}$$);' INTO v_audit_result;
			END IF;
		...
		ELSIF v_action ='RESTORE' THEN
			SELECT text_column::json->>'table' INTO v_tablename_check FROM temp_table WHERE text_column::json->>'name' = v_backupname AND fid=434 LIMIT 1;

			-- check consistency on tablename
			IF v_tablename != v_tablename_check THEN
				EXECUTE 'SELECT gw_fct_getmessage($${"client":{"device":4, "infoType":1, "lang":"ES"},"feature":{},
				"data":{"message":"3150", "function":"2792","parameters":{"table_name":"'||v_tablename||'"}, "is_process":true}}$$);' INTO v_audit_result;
			END IF;
			...
			IF (SELECT count(*) FROM temp_table WHERE text_column::json->>'name' = v_newbackupname AND fid=434 ) > 0 THEN
				EXECUTE 'SELECT gw_fct_getmessage($${"client":{"device":4, "infoType":1, "lang":"ES"},"feature":{},
				"data":{"message":"3148", "function":"2792","parameters":{"backup_name":"'||v_newbackupname||'"}, "is_process":true}}$$);' INTO v_audit_result;
			END IF;
			...
			(SELECT addparam FROM temp_table WHERE fid =434 AND text_column::json->>'name' = v_backupname limit 1)a)b;
			...
			-- build querytext (end)
			v_querytext = concat (v_querytext, ' FROM temp_table WHERE fid=434 AND text_column::json->>''name'' = ',quote_literal(v_backupname));

			EXECUTE 'SELECT count(*) FROM temp_table WHERE fid=434 AND text_column::json->>''name'' = '||quote_literal(v_backupname)
				INTO v_count;
		...
		ELSIF v_action= 'DELETE' THEN
			...
			-- get table from backup
			SELECT text_column::json->>'table' INTO v_tablename FROM temp_table WHERE text_column::json->>'name' = v_backupname AND fid=434 LIMIT 1;

			EXECUTE 'SELECT count(*) FROM temp_table WHERE text_column::json->>''name'' = '|| quote_literal(v_backupname) ||' AND fid=434'
			INTO v_count;

			DELETE FROM temp_table WHERE text_column::json->>'name' = v_backupname AND fid=434;
