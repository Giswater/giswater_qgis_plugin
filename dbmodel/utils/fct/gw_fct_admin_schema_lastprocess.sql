...
BEGIN
	...
	ELSE
		...
		IF v_isnew IS TRUE THEN
			...
			IF v_projecttype = 'WS' THEN
				UPDATE cat_feature SET active = false WHERE feature_class NOT IN ('VALVE', 'WJOIN', 'JUNCTION', 'TANK', 'PIPE'); -- ws projects
			ELSE
				UPDATE cat_feature SET active = false WHERE feature_class NOT IN ('CONDUIT', 'JUNCTION', 'CONNEC', 'GULLY', 'OUTFALL'); -- ud projects
			END IF;
		...
		ELSIF v_isnew IS FALSE THEN
			...
			IF v_oldversion IN ('3.6.007', '3.6.008', '3.6.008.1', '3.6.008.2') THEN
				...
				LOOP
					...
					VALUES (rec_feature.child_layer, concat('Custom edit view for ', rec_feature.child_layer), 'role_edit', concat('{"level_1":"INVENTORY","level_2":"NETWORK","level_3":"',
