...
--  Search path
	...
	v_parameter = (SELECT parameter_id2 FROM om_visit_event JOIN config_visit_param_x_param ON parameter_id=parameter_id1
	...
	IF v_visit_type = 1 THEN
		...
		LOOP
			...
			-- insert parameters
			FOR rec_parameter IN SELECT * FROM om_visit_event JOIN config_visit_param_x_param ON parameter_id=parameter_id1
			...
			LOOP 
				...
				FOR rec_parameter_child IN SELECT * FROM config_visit_param_x_param WHERE parameter_id1=v_parameter AND action_type=4
				LOOP 
				...
				FOR rec_parameter_child IN SELECT * FROM config_visit_param_x_param WHERE parameter_id1=v_parameter AND action_type=5
				LOOP 

					work_aux := (select action_value::text from config_visit_param_x_param WHERE parameter_id1=rec_parameter.parameter_id2 AND action_type=5);
		...
		UPDATE om_visit SET class_id=v_newclass WHERE om_visit.id=visit_id_aux;
		 ...
		 IF (SELECT count(*) FROM om_visit_event JOIN config_visit_param_x_param ON parameter_id=parameter_id1
		...
		IF (SELECT om_visit_event.visit_id FROM SCHEMA_NAME.om_visit_event 
			...
			JOIN SCHEMA_NAME.config_visit_param_x_param ON parameter_id=config_visit_param_x_param.parameter_id2
			WHERE visit_id = ANY(v_visit_id)
			AND action_type = 2
			AND parameter_id = config_visit_param_x_param.parameter_id2
