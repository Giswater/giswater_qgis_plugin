...
BEGIN
    ...
    -- Initialize key values from fields
    v_class := NULLIF(v_fields->>'class_id', '')::integer;
    v_visitextcode := v_fields->>'ext_code';
    v_visitcat := NULLIF(v_fields->>'visitcat_id', '')::smallint;
    v_status := NULLIF(v_fields->>'status', '')::smallint;
    v_lot := NULLIF(v_fields->>'lot_id', '')::integer;
    v_unit_id := NULLIF(v_fields->>'unit_id', '')::integer;
    v_node_id := NULLIF(v_fields->>'node_id', '')::integer;
    v_arc_id := NULLIF(v_fields->>'arc_id', '')::integer;
    v_gully_id := NULLIF(v_fields->>'gully_id', '')::integer;

    -- Get class metadata
    SELECT feature_type, tablename, formname, ui_tablename
    INTO v_class_feature_type, v_tablename, v_formname, v_ui_tablename
    ...
    IF upper(v_class_feature_type) = 'ALL' THEN
        ...
        ELSIF v_arc_id IS NOT NULL THEN
            v_real_feature := 'arc';
        ELSIF NULLIF(v_fields->>'connec_id', '') IS NOT NULL THEN
        ...
        v_effective_feature_type := v_real_feature;
    ELSE
        v_effective_feature_type := lower(v_class_feature_type);
    END IF;

    -- Get id_name from resolved table_name
    ...
    PERFORM setval('"SCHEMA_NAME".doc_x_visit_id_seq', (SELECT max(doc_id) FROM doc_x_visit), true);
    ...
    IF v_visit_id IS NULL THEN
        ...
        "data":{"message":"3118", "function":"2622","parameters":null, "is_process":true}}$$);'
        INTO v_message;
    ...
    ELSE
        ...
        IF v_ckeckchangeclass IS NULL THEN
            ...
            INSERT INTO om_visit_event (parameter_id, visit_id)
            SELECT parameter_id, v_id
            FROM config_visit_class_x_parameter
        ...
        EXECUTE 'SELECT gw_fct_getmessage($${"client":{"device":4, "infoType":1, "lang":"ES"},"feature":{},
        "data":{"message":"3120", "function":"2662","parameters":null, "is_process":true}}$$);'
        INTO v_message;
    ...
    FROM (SELECT json_array_elements(v_addphotos)) a
    INTO v_addphotos_array;
    ...
    IF v_addphotos_array IS NOT NULL THEN
        ...
        LOOP
            v_querystring = concat(
                'INSERT INTO om_visit_event_photo (visit_id, event_id, tstamp, value)
            ...
            v_debug_vars := json_build_object('v_id', v_id, 'v_event_id', v_event_id, 'v_addphotos', v_addphotos);
        ...
        v_querystring = concat(
            'UPDATE om_visit_event SET value = ''true'' WHERE visit_id = ', quote_nullable(v_id), ' AND parameter_id = ''photo'''
        );
        v_debug_vars := json_build_object('v_id', v_id);
    ...
    ELSE
        v_querystring = concat(
            'UPDATE om_visit_event SET value = ''false'' WHERE visit_id = ', quote_nullable(v_id), ' AND parameter_id = ''photo'''
        );
        v_debug_vars := json_build_object('v_id', v_id);
    ...
    UPDATE om_visit_event
    SET xcoord = (((p_data ->>'data')::json->>'deviceTrace')::json->>'xcoord')::float,
    ...
    IF v_parent_id IS NOT NULL THEN
        ...
        FOR rec_arc IN
            SELECT * FROM om_visit_lot_x_arc WHERE unit_id = v_unit_id AND lot_id = v_lot AND arc_id::integer <> v_feature_id::integer
        LOOP
            INSERT INTO om_visit (startdate, enddate, expl_id, user_name, lot_id, class_id, status, visit_type, the_geom, unit_id)
            SELECT startdate, enddate, expl_id, user_name, lot_id, v_visitclass_arc, status, visit_type, the_geom, unit_id
            FROM om_visit
            WHERE id = v_id
            RETURNING id INTO id_last;

            INSERT INTO om_visit_x_arc (visit_id, arc_id) VALUES(id_last, rec_arc.arc_id);
            INSERT INTO om_visit_event (visit_id, parameter_id, value, xcoord, ycoord)
            SELECT id_last, parameter_id, value, xcoord, ycoord
            FROM om_visit_event
            WHERE visit_id = v_id;
        END LOOP;
        ...
        FOR rec_node IN
            SELECT * FROM om_visit_lot_x_node WHERE unit_id = v_unit_id AND lot_id = v_lot AND node_id::integer <> v_feature_id::integer
        LOOP
            INSERT INTO om_visit (startdate, enddate, expl_id, user_name, lot_id, class_id, status, visit_type, the_geom, unit_id)
            SELECT startdate, enddate, expl_id, user_name, lot_id, v_visitclass_node, status, visit_type, the_geom, unit_id
            FROM om_visit
            WHERE id = v_id
            RETURNING id INTO id_last;
        ...
        FOR rec_link IN
            SELECT * FROM om_visit_lot_x_link WHERE unit_id = v_unit_id AND lot_id = v_lot AND link_id::integer <> v_feature_id::integer
        LOOP
            INSERT INTO om_visit (startdate, enddate, expl_id, user_name, lot_id, class_id, status, visit_type, the_geom, unit_id)
            SELECT startdate, enddate, expl_id, user_name, lot_id, v_visitclass_link, status, visit_type, the_geom, unit_id
            FROM om_visit
            WHERE id = v_id
            RETURNING id INTO id_last;
        ...
        FOR rec_gully IN
            SELECT * FROM om_visit_lot_x_gully WHERE unit_id = v_unit_id AND lot_id = v_lot AND gully_id::integer <> v_feature_id::integer
        LOOP
            INSERT INTO om_visit (startdate, enddate, expl_id, user_name, lot_id, class_id, status, visit_type, the_geom, unit_id)
            SELECT startdate, enddate, expl_id, user_name, lot_id, v_visitclass_gully, status, visit_type, the_geom, unit_id
            FROM om_visit
            WHERE id = v_id
            RETURNING id INTO id_last;
    ...
    IF v_id IS NOT NULL THEN
        v_querystring = concat(
            'SELECT row_to_json(a)
             FROM (SELECT St_AsText(St_simplify(the_geom,0)) FROM om_visit WHERE id=', quote_nullable(v_id), ') a'
        );
        v_debug_vars := json_build_object('v_id', v_id);
