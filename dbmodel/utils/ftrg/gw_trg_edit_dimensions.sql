...
BEGIN
	...
	IF TG_OP = 'INSERT' THEN
		...
		-- Insert
		INSERT INTO dimensions (id, distance, depth, the_geom, x_label, y_label, rotation_label, offset_label, direction_arrow, x_symbol, y_symbol, feature_id, feature_type, state, expl_id, observ)
		VALUES (NEW.id, NEW.distance, NEW.depth, NEW.the_geom, NEW.x_label, NEW.y_label, NEW.rotation_label, NEW.offset_label, NEW.direction_arrow, NEW.x_symbol, NEW.y_symbol, 
			NEW.feature_id, NEW.feature_type, NEW.state, NEW.expl_id, NEW.observ);
	...
	ELSIF TG_OP = 'UPDATE' THEN
		...
		direction_arrow=NEW.direction_arrow, x_symbol=NEW.x_symbol, y_symbol=NEW.y_symbol, feature_id=NEW.feature_id, feature_type=NEW.feature_type, expl_id=NEW.expl_id, observ=NEW.observ
		WHERE id=NEW.id;
