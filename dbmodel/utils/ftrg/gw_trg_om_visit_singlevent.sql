    ...
    ELSIF TG_OP = 'UPDATE' THEN
	    -- visit table
  	    ...
  	    -- Delete parameters in case of inconsistency againts visitclass and events (due class of visit have been changed)
   	    DELETE FROM om_visit_event WHERE visit_id=NEW.visit_id AND parameter_id NOT IN (SELECT parameter_id FROM config_visit_parameter_action WHERE class_id=NEW.class_id);
