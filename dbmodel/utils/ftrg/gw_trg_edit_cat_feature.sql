...
BEGIN
	...
	IF TG_OP = 'INSERT' THEN
			INSERT INTO cat_feature(id, system_id, shortcut_key, descript, link_path, code_autofill, active, config)
  	  VALUES (NEW.id, NEW.system_id, NEW.shortcut_key, NEW.descript, NEW.link_path, NEW.code_autofill, NEW.active, NEW.config::json);
    ...
    ELSIF TG_OP = 'UPDATE' THEN
    	UPDATE cat_feature SET id=NEW.id, system_id=NEW.system_id, shortcut_key=NEW.shortcut_key, descript=NEW.descript, link_path=NEW.link_path, 
			code_autofill=NEW.code_autofill, active=NEW.active, config=NEW.config::json WHERE id=OLD.id;
