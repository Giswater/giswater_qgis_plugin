		...
		IF (OLD.status != NEW.status) AND (NEW.status = 4) THEN
			...
			ELSIF v_projectype = 'UD' THEN
				...
				INSERT INTO archived_psector_arc_traceability (
					...
					streetname2, dwfzone_id, initoverflowpath, omunit_id, registration_date, hydraulic_capacity, meandering, conserv_state, om_state, last_visitdate, negativeoffset
				)
				SELECT
					nextval('SCHEMA_NAME.archived_psector_arc_traceability_id_seq'), pa.psector_id, pa.state, pa.doable, pa.addparam, now(), current_user, 'Execute psector',
					...
					a.streetname2, a.dwfzone_id, a.initoverflowpath, a.omunit_id, a.registration_date, a.hydraulic_capacity, a.meandering, a.conserv_state, a.om_state, a.last_visitdate, a.negativeoffset
				FROM plan_psector_x_arc pa
				JOIN arc a USING (arc_id)
				WHERE psector_id=NEW.psector_id;
