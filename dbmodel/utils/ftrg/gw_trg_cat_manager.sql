...
BEGIN 
	...
	ELSIF TG_OP = 'UPDATE'THEN
		...
		--if username was empty insert exploitation_x_user or combinations for this manager 
			...
			FOREACH rec_user IN ARRAY OLD.username LOOP
				...
				FOREACH rec_expl IN ARRAY OLD.expl_id LOOP
					...
					IF (SELECT count(id) FROM cat_manager WHERE rec_user = ANY(username) AND rec_expl::integer = ANY(expl_id))>0 THEN
						...
						--if not, remove it	
						IF (SELECT count(id) FROM exploitation_x_user WHERE username=rec_user and expl_id=rec_expl) > 0 THEN

							UPDATE exploitation_x_user set manager_id = v_new_id WHERE username=rec_user and expl_id=rec_expl;
						END IF;
					ELSE
						DELETE FROM exploitation_x_user WHERE rec_user=username and expl_id=rec_expl and manager_id=NEW.id;
					END IF;
				END LOOP;
			END LOOP;
		--if user was added insetr new relation into exploitation_x_user
		...
		--if exploitation was added insert new posible relations into exploitation_x_user
