...
class GwToolBoxButton(GwParentAction):
    ...
    def load_settings_values(self, dialog, function):
        ...
        if dialog.cmb_geom_type.property('selectedId') in (None, '', 'NULL'):
            geom_type = tools_gw.get_config_parser('btn_toolbox', f"{function_name}_cmb_geom_type")
        else:
        ...
        if dialog.cmb_layers.property('selectedId') in (None, '', 'NULL'):
            layer = tools_gw.get_config_parser('btn_toolbox', f"{function_name}_cmb_layers")
        else:
            layer = dialog.cmb_layers.property('selectedId')
        tools_qt.set_combo_value(dialog.cmb_layers, layer, 0)

        if tools_gw.get_config_parser('btn_toolbox', f"{function_name}_rbt_previous") == 'True':
