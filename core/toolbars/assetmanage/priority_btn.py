...
from ....libs import lib_vars, tools_qgis, tools_db, tools_os, tools_qt
    ...
    def __init__(self, type):
        try:
            ...
            config_path = os.path.join(
                lib_vars.plugin_dir, f"config{os.sep}config.config"
            )
...
class CalculatePriority:
    ...
    def clicked_event(self):
        self.dlg_priority = PriorityUi(self)
        ...
        key = "arccat_id" #QUITAR ESTA LINEA CUANDO SE DECOMENTE LA LINEA DE ARRIBA (QUE DA ERROR DE MOMENTO)
        ...
        if self.mode == "new":
            rows = tools_db.get_rows(
                ...
                from asset.config_engine_def
                 -- descomentar esta linea: where method = '{self.config.method}'
                """
            )
        else:
            rows = tools_db.get_rows(
                ...
                where c.result_id = {self.result["id"]}
                 -- descomentar esta linea:    and d.method = '{self.config.method}'
                """
            )
