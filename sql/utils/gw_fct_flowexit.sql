-- Function: SCHEMA_NAME.gw_fct_flowexit(character varying)

-- DROP FUNCTION SCHEMA_NAME.gw_fct_flowexit(character varying);

CREATE OR REPLACE FUNCTION SCHEMA_NAME.gw_fct_flowexit(node_id_arg character varying)
  RETURNS json AS
$BODY$DECLARE
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION SCHEMA_NAME.gw_fct_flowexit(character varying)
  OWNER TO geoserver;
...
-- Function: SCHEMA_NAME.gw_fct_flowexit_recursive(character varying)

-- DROP FUNCTION SCHEMA_NAME.gw_fct_flowexit_recursive(character varying);

CREATE OR REPLACE FUNCTION SCHEMA_NAME.gw_fct_flowexit_recursive(node_id_arg character varying)
  RETURNS void AS
$BODY$DECLARE
...
$BODY$
  LANGUAGE plpgsql VOLATILE
  COST 100;
ALTER FUNCTION SCHEMA_NAME.gw_fct_flowexit_recursive(character varying)
  OWNER TO geoserver;
